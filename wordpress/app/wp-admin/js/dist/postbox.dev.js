  ...
  window.postboxes = {
    ...
    handleOrder: function handleOrder() {
      ...
      if ('true' === button.attr('aria-disabled')) {
        firstOrLastPositionMessage = button.hasClass('handle-order-higher') ? __('The box is on the first position') : __('The box is on the last position');
     ...
     * @return {void}
     */
    handleOrderBetweenSortables: function handleOrderBetweenSortables(position, button, postbox) {
      var closestSortablesId = button.closest('.meta-box-sortables').attr('id'),
          sortablesIds = [],
          sortablesIndex,
          detachedPostbox; // Get the list of sortables within the page.
      ...
      sortablesIndex = $.inArray(closestSortablesId, sortablesIds); // Detach the postbox to be moved.

      detachedPostbox = postbox.detach(); // Move the detached postbox to its new position.
    ...
    updateOrderButtonsProperties: function updateOrderButtonsProperties() {
      var firstSortablesId = $('.meta-box-sortables:visible:first').attr('id'),
          ...
          moveDownButtons = $('.handle-order-lower'); // Enable all buttons as a reset first.

      moveUpButtons.attr('aria-disabled', 'false').removeClass('hidden');
      moveDownButtons.attr('aria-disabled', 'false').removeClass('hidden'); // When there's only one "sortables" area (e.g. in the block editor) and only one visible postbox, hide the buttons.
    ...
    add_postbox_toggles: function add_postbox_toggles(page, args) {
      ...
      this.page = page;
      this.init(page, args);
      $handles.on('click.postboxes', this.handle_click); // Handle the order of the postboxes.
    ...
    init: function init(page, args) {
      ...
      $('.meta-box-sortables').sortable({
        ...
        start: function start() {
          $('body').addClass('is-dragging-metaboxes'); // Refresh the cached positions of all the sortable items so that the min-height set while dragging works.
      ...
      this._mark_area(); // Update the "move" buttons properties.


      this.updateOrderButtonsProperties();
      $document.on('postbox-toggled', this.updateOrderButtonsProperties); // Set the handle buttons `aria-expanded` attribute initial value on page load.
     ...
     * @return {void}
     */
    save_state: function save_state(page) {
      var closed, hidden; // Return on the nav-menus.php screen, see #35112.
     ...
     * @return {void}
     */
    save_order: function save_order(page) {
      var postVars,
     ...
     * @return {void}
     */
    updateEmptySortablesText: function updateEmptySortablesText(visibleSortables, areAllVisibleSortablesEmpty) {
