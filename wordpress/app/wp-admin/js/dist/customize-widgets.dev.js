...
(function (wp, $) {
  ...
  api.Widgets.savedWidgetIds = {}; // Link settings.
  ...
  /** @lends wp.customize.Widgets.WidgetCollection.prototype */
  {
    ...
    search: function search(term) {
      var match, haystack; // Escape the term string for RegExp meta characters.
  ...
  /** @lends wp.customize.Widgets.AvailableWidgetsPanelView.prototype */
  {
    ...
    initialize: function initialize() {
      ...
      this.updateList(); // Set the initial search count to the number of available widgets.
    ...
    search: _.debounce(function (event) {
      var firstVisible;
      this.collection.doSearch(event.target.value); // Update the search matches count.

      this.updateSearchMatchesCount(); // Announce how many search results.

      this.announceSearchMatches(); // Remove a widget from being selected if it is no longer visible.
     ...
     * Highlights a widget.
    	 */
    select: function select(widgetTpl) {
     ...
     * Adds a selected widget to the sidebar.
    	 */
    submit: function submit(widgetTpl) {
      ...
      widget = this.collection.findWhere({
        id: widgetId
      });
     ...
     * Opens the panel.
     */
    open: function open(sidebarControl) {
      this.currentSidebarControl = sidebarControl; // Wide widget controls appear over the preview, and so they need to be collapsed when the panel opens.
      ...
      this.$el.find('.selected').removeClass('selected'); // Reset search.
  ...
  api.Widgets.formSyncHandlers = {
     ...
     */
    rss: function rss(e, widget, newForm) {
  ...
  /** @lends wp.customize.Widgets.WidgetControl.prototype */
  {
    ...
    ready: function ready() {
      ...
      if (!control.section()) {
        ...
        api.section(control.section(), function (section) {
          var onExpanded = function onExpanded(isExpanded) {
            if (isExpanded) {
              control.embedWidgetControl();
    ...
    embedWidgetContent: function embedWidgetContent() {
      ...
      control.widgetContentEmbedded = true; // Update the notification container element now that the widget content has been embedded.
    ...
    _setupModel: function _setupModel() {
      var self = this,
          rememberSavedWidgetId; // Remember saved widgets so we know which to trash (move to inactive widgets sidebar).

      rememberSavedWidgetId = function rememberSavedWidgetId() {
      ...
      this.liveUpdateMode = true; // Update widget whenever model changes.
    ...
    _setupWideWidget: function _setupWideWidget() {
      ...
      /* max-width breakpoint in customize-controls.css */
      ) {
          return;
        }
      ...
      positionWidget = function positionWidget() {
        ...
        top = Math.max(0, // Prevent top from going off screen.
        Math.min(Math.max(offsetTop, 0), // Distance widget in panel is from top of screen.
    ...
    _setupReorderUI: function _setupReorderUI() {
      ...
      selectSidebarItem = function selectSidebarItem(li) {
      ...
      updateAvailableSidebars = function updateAvailableSidebars() {
    ...
    _setupHighlightEffects: function _setupHighlightEffects() {
      var self = this; // Highlight whenever hovering or clicking over the form.
    ...
    _setupUpdateUI: function _setupUpdateUI() {
      ...
      $widgetContent = $widgetRoot.find('.widget-content:first'); // Configure update button.
      ...
      $saveBtn.on('click', function (e) {
        ...
        self.updateWidget({
          disable_form: true
        }); // @todo disable_form is unused?
      });
      updateWidgetDebounced = _.debounce(function () {
        self.updateWidget();
      }, 250); // Trigger widget form update when hitting Enter within an input.
    ...
    _setupRemoveUI: function _setupRemoveUI() {
      var self = this,
          $removeBtn,
          replaceDeleteWithRemove; // Configure remove button.
      ...
      replaceDeleteWithRemove = function replaceDeleteWithRemove() {
     ...
     * @private
     */
    _getInputs: function _getInputs(container) {
     ...
     * @private
     */
    _getInputsSignature: function _getInputsSignature(inputs) {
     ...
     * @private
     */
    _getInputState: function _getInputState(input) {
      input = $(input);
     ...
     * @private
     */
    _setInputState: function _setInputState(input, state) {
      input = $(input);
    ...
    updateWidget: function updateWidget(args) {
      var self = this,
          ...
          processing,
          jqxhr,
          isChanged; // The updateWidget logic requires that the form fields to be fully present.
      ...
      $widgetRoot = this.container.find('.widget:first');
      $widgetContent = $widgetRoot.find('.widget-content:first'); // Remove a previous error message.
      ...
      jqxhr.done(function (r) {
        var message,
            ...
            isLiveUpdateAborted = false; // Check if the user is logged out.
        ...
        if (r.success) {
          ...
          hasSameInputsInResponse = self._getInputsSignature($inputs) === self._getInputsSignature($sanitizedInputs); // Restore live update mode if sanitized fields are now aligned with the existing fields.
          ...
          if (hasSameInputsInResponse && self.liveUpdateMode) {
            ...
            $(document).trigger('widget-synced', [$widgetRoot, r.data.form]); // Otherwise, if sanitized fields are not aligned with existing fields, disable live update mode if enabled.
          } else if (self.liveUpdateMode) {
            self.liveUpdateMode = false;
            self.container.find('input[name="savewidget"]').show();
            isLiveUpdateAborted = true; // Otherwise, replace existing form with the sanitized form.
          } else {
     ...
     */
    toggleForm: function toggleForm(showOrHide) {
    ...
    onChangeExpanded: function onChangeExpanded(expanded, args) {
      ...
      expandControl = function expandControl() {
     ...
     */
    _moveWidgetByOne: function _moveWidgetByOne(offset) {
     ...
     */
    toggleWidgetMoveArea: function toggleWidgetMoveArea(showOrHide) {
      var self = this,
          $moveWidgetArea;
  ...
  /** @lends wp.customize.Widgets.WigetsPanel.prototype */
  {
    ...
    ready: function ready() {
      ...
      panel.deferred.embedded.done(function () {
        ...
        panelMetaContainer = panel.container.find('.panel-meta'); // @todo This should use the Notifications API introduced to panels. See <https://core.trac.wordpress.org/ticket/38794>.
        ...
        getActiveSectionCount = function getActiveSectionCount() {
  ...
  /** @lends wp.customize.Widgets.SidebarControl.prototype */
  {
    ...
    _setupModel: function _setupModel() {
      ...
      this.setting.bind(function (newWidgetIds, oldWidgetIds) {
        ...
        removedWidgetIds = _(oldWidgetIds).difference(newWidgetIds); // Filter out any persistent widget IDs for widgets which have been deactivated.
        ...
        self._applyCardinalOrderClassNames(); // If the widget was dragged into the sidebar, make sure the sidebar_id param is updated.
        ...
        _(removedWidgetIds).each(function (removedWidgetId) {
          ...
          setTimeout(function () {
            var removedControl,
                ...
                removedIdBase,
                widget,
                isPresentInAnotherSidebar = false; // Check if the widget is in another sidebar.
            ...
            removedControl = api.Widgets.getWidgetFormControlForWidget(removedWidgetId); // Detect if widget control was dragged to another sidebar.

            wasDraggedToAnotherSidebar = removedControl && $.contains(document, removedControl.container[0]) && !$.contains(self.$sectionContent[0], removedControl.container[0]); // Delete any widget form controls for removed widgets.
    ...
    _setupSortable: function _setupSortable() {
      ...
      this.$controlSection.find('.accordion-section-title').droppable({
        ...
        over: function over() {
          ...
          section.expand({
            allowMultiple: true,
            // Prevent the section being dragged from to be collapsed.
    ...
    _applyCardinalOrderClassNames: function _applyCardinalOrderClassNames() {
      ...
      $(widgetControls).each(function () {
        $(this.container).removeClass('first-widget').removeClass('last-widget').find('.move-widget-down, .move-widget-up').prop('tabIndex', 0);
      });

      _.first(widgetControls).container.addClass('first-widget').find('.move-widget-up').prop('tabIndex', -1);

      _.last(widgetControls).container.addClass('last-widget').find('.move-widget-down').prop('tabIndex', -1);
    },
    ...
    toggleReordering: function toggleReordering(showOrHide) {
      ...
      if (showOrHide) {
        ...
        wp.a11y.speak(l10n.reorderModeOn); // Hide widget titles while reordering: title is already in the reorder controls.
     ...
     */
    addWidget: function addWidget(widgetId) {
      var self = this,
          controlHtml,
          $widget,
      ...
      $widget = $(controlHtml);
      controlContainer = $('<li/>').addClass('customize-control').addClass('customize-control-' + controlType).append($widget); // Remove icon which is visible inside the panel.
      ...
      controlContainer.attr('id', 'customize-control-' + settingId.replace(/\]/g, '').replace(/\[/g, '-')); // Only create setting if it doesn't already exist (if we're adding a pre-existing inactive widget).
      ...
      api.control.add(widgetFormControl); // Make sure widget is removed from the other sidebars.
  ...
  api.bind('ready', function () {
    ...
    api.previewer.bind('highlight-widget-control', api.Widgets.highlightWidgetFormControl); // Open and focus widget control.
  ...
  api.Widgets.getSidebarWidgetControlContainingWidget = function (widgetId) {
    var foundControl = null; // @todo This can use widgetIdToSettingId(), then pass into wp.customize.control( x ).getSidebarWidgetsControl().
  ...
  api.Widgets.getWidgetFormControlForWidget = function (widgetId) {
    var foundControl = null; // @todo We can just use widgetIdToSettingId() here.
