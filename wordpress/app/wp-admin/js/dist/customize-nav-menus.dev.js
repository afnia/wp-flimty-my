...
(function (api, wp, $) {
  ...
  api.Menus = api.Menus || {}; // Link settings.
  ...
  /** @lends wp.customize.Menus.AvailableItemCollection.prototype */
  {
    ...
    sortByField: function sortByField(fieldName) {
  ...
  /** @lends wp.customize.Menus.AvailableMenuItemsPanelView.prototype */
  {
    ...
    initList: function initList() {
      var self = this; // Render the template for each item by type.
     ...
     * @return {void}
     */
    loadItems: function loadItems(itemTypes, deprecated) {
      var self = this,
          _itemTypes,
          requestItemTypes = [],
          params,
        ...
        _.each(data.items, function (typeItems, name) {
          if (0 === typeItems.length) {
            if (0 === self.pages[name]) {
              availableMenuItemContainers[name].find('.accordion-section-title').addClass('cannot-expand').removeClass('loading').find('.accordion-section-title > button').prop('tabIndex', -1);
            }
    ...
    select: function select(menuitemTpl) {
    ...
    submit: function submit(menuitemTpl) {
      ...
      menu_item = this.collection.findWhere({
        id: menuitemId
      });
    ...
    _submitLink: function _submitLink(event) {
    ...
    submitLink: function submitLink() {
      ...
      this.currentMenuControl.addItemToMenu(menuItem); // Reset the custom link form.
     ...
     * @return {void}
     */
    _submitNew: function _submitNew(event) {
      var container; // Only proceed with keypress if it is Enter.
     ...
     * @return {void}
     */
    submitNew: function submitNew(container) {
      var panel = this,
      ...
      promise.done(function (data) {
        ...
        panel.currentMenuControl.addItemToMenu(availableItem.attributes); // Add the new item to the list of available items.
        ...
        $content.scrollTop(); // Reset the create content form.
    ...
    // Opens the panel.
    open: function open(menuControl) {
      var panel = this,
          _close;
      ...
      _close = function close() {
        panel.close();
        $(this).off('click', _close);
      };

      $('#customize-preview').on('click', _close); // Collapse all controls.
    ...
    keyboardAccessible: function keyboardAccessible(event) {
      var isEnter = 13 === event.which,
          isEsc = 27 === event.which,
          isBackTab = 9 === event.which && event.shiftKey,
          isSearchFocused = $(event.target).is(this.$search); // If enter pressed but nothing entered, don't do anything.
  ...
  /** @lends wp.customize.Menus.MenusPanel.prototype */
  {
    attachEvents: function attachEvents() {
      ...
      button.on('click keydown', function (event) {
        ...
        event.preventDefault(); // Hide description.
  ...
  /** @lends wp.customize.Menus.MenuSection.prototype */
  {
    ...
    ready: function ready() {
      ...
      handleFieldActiveToggle = function handleFieldActiveToggle() {
    ...
    populateControls: function populateControls() {
      var section = this,
          ...
          menuAutoAddControl,
          menuDeleteControl; // Add the control for managing the menu name.
     ...
     */
    updateAssignedLocationsInSectionTitle: function updateAssignedLocationsInSectionTitle(themeLocationSlugs) {
      var section = this,
          $title;
    ...
    onChangeExpanded: function onChangeExpanded(expanded, args) {
      ...
      if (expanded) {
        ...
        wpNavMenu.targetList = wpNavMenu.menuList; // Add attributes needed by wpNavMenu.
  ...
  api.Menus.createNavMenu = function createNavMenu(name) {
    ...
    customizeId = 'nav_menu[' + String(placeholderId) + ']'; // Register the menu control setting.
    ...
    setting.set($.extend({}, api.Menus.data.defaultSettingValues.nav_menu, {
      name: name || ''
    }));
    /*
  ...
  /** @lends wp.customize.Menus.NewMenuSection.prototype */
  {
    ...
    attachEvents: function attachEvents() {
      ...
      section.headContainer.find('.accordion-section-title').replaceWith(wp.template('nav-menu-create-menu-section-title'));
      /*
    ...
    submit: function submit() {
      ...
      menuSection = api.Menus.createNavMenu(name); // Clear name field.
      ...
      contentContainer.find('.assigned-menu-location input[type=checkbox]').each(function () {
        ...
        if (checkbox.prop('checked')) {
          ...
          navMenuLocationSetting.set(menuSection.params.menu_id); // Reset state for next new menu.

          checkbox.prop('checked', false);
        }
      });
      wp.a11y.speak(api.Menus.data.l10n.menuAdded); // Focus on the new menu section.
     ...
     * @return {void}
     */
    selectDefaultLocation: function selectDefaultLocation(locationId) {
  ...
  /** @lends wp.customize.Menus.MenuLocationControl.prototype */
  {
    ...
    ready: function ready() {
      var control = this,
          navMenuIdRegex = /^nav_menu\[(-?\d+)]/; // @todo It would be better if this was added directly on the setting itself, as opposed to the control.
  ...
  /** @lends wp.customize.Menus.MenuItemControl.prototype */
  {
    ...
    _setupReorderUI: function _setupReorderUI() {
      var control = this,
          template,
          $reorderNav;
      template = wp.template('menu-item-reorder-nav'); // Add the menu item reordering elements to the menu item control.

      control.container.find('.item-controls').after(template); // Handle clicks for up/down/left-right on the reorder nav.
    ...
    _setupUpdateUI: function _setupUpdateUI() {
      ...
      control.elements.description = new api.Element(control.container.find('.edit-menu-item-description')); // @todo Allow other elements, added by plugins, to be automatically picked up here;
      ...
      control.setting.bind(function (to, from) {
        ...
        if (false === to) {
          ...
          control.container.find('.menu-item-data-parent-id').val(to.menu_item_parent); // Handle UI updates when the position or depth (parent) change.
    ...
    _setupRemoveUI: function _setupRemoveUI() {
      var control = this,
          $removeBtn; // Configure delete button.
      ...
      $removeBtn.on('click', function () {
        ...
        var addingItems = true,
            ...
            $prev,
            instanceCounter = 0,
            // Instance count of the menu item deleted.
        ...
        _.each(addedItems, function (addedItem) {
          var menuItemId, menuItemControl, matches; // This is because menu item that's deleted is just hidden.
          ...
          menuItemControl = api.control('nav_menu_item[' + String(menuItemId) + ']'); // Check for duplicate menu items.
    ...
    _setupLinksUI: function _setupLinksUI() {
      var $origBtn; // Configure original link.
    ...
    _setupTitleUI: function _setupTitleUI() {
      var control = this,
          titleEl; // Ensure that whitespace is trimmed on blur so placeholder can be shown.
    ...
    renderContent: function renderContent() {
      ...
      containerClasses = ['menu-item', 'menu-item-depth-' + String(control.params.depth), 'menu-item-' + settingValue.object, 'menu-item-edit-inactive'];
     ...
     */
    expandForm: function expandForm(params) {
      this.expand(params);
    },
     ...
     * @param {Function} [params.completeCallback] - Function to call when the form toggle has finished animating.
     */
    collapseForm: function collapseForm(params) {
     ...
     * @param {Function} [params.completeCallback] - Function to call when the form toggle has finished animating.
     */
    toggleForm: function toggleForm(showOrHide, params) {
     ...
     * @param {Function} [params.completeCallback] - Function to call when the form toggle has finished animating.
     */
    onChangeExpanded: function onChangeExpanded(showOrHide, params) {
      var self = this,
          $menuitem,
          $inside,
          complete;
    ...
    focus: function focus(params) {
      ...
      focusControl = function focusControl() {
        ...
        params.completeCallback = function () {
          var focusable; // Note that we can't use :focusable due to a jQuery UI issue. See: https://github.com/jquery/jquery-ui/pull/1583
    ...
    _changePosition: function _changePosition(offset) {
      ...
      if (adjacentSetting) {
        adjacentSetting.set($.extend(_.clone(adjacentSetting()), {
     ...
     * @param {number} offset 1|-1
     */
    _changeDepth: function _changeDepth(offset) {
      if (1 !== offset && -1 !== offset) {
        throw new Error('Offset changes by 1 are only supported.');
      }
      ...
      var control = this,
          ...
          realPosition,
          siblingControl,
          parentControl; // Locate the other items under the same parent (siblings).
      ...
      if (-1 === offset) {
        ...
        parentControl = api.control('nav_menu_item[' + settingValue.menu_item_parent + ']'); // Make this control the parent of all the following siblings.

        _(siblingControls).chain().slice(realPosition).each(function (siblingControl, i) {
          siblingControl.setting.set($.extend({}, siblingControl.setting(), {
        ...
        _(control.getMenuControl().getMenuItemControls()).each(function (otherControl) {
          ...
          isControlToBeShifted = otherControl.setting().menu_item_parent === parentControl.setting().menu_item_parent && otherControl.setting().position > parentControl.setting().position;

          if (isControlToBeShifted) {
            otherControlSettingValue = _.clone(otherControl.setting());
            otherControl.setting.set($.extend(otherControlSettingValue, {
      ...
      control.container.find('.assigned-menu-location').each(function () {
        var container = $(this),
          ...
          navMenuLocationSetting.set(checked ? control.params.menu_id : 0);
            ...
            updateSelectedMenuLabel = function updateSelectedMenuLabel(selectedMenuId) {
  ...
  /** @lends wp.customize.Menus.MenuControl.prototype */
  {
    ...
    ready: function ready() {
      ...
      this._setupTitle(); // Add menu to Navigation Menu widgets.


      if (menu) {
        name = displayNavMenuName(menu.name); // Add the menu to the existing controls.
     ...
     */
    _setupSortable: function _setupSortable(menuList) {
      var control = this;
      ...
      menuList.on('sortstop', function () {
        setTimeout(function () {
          ...
          var menuItemContainerIds = control.$sectionContent.sortable('toArray'),
              ...
              position = 0,
              priority = 10;
          control.isSorting = false; // Reset horizontal scroll position when done dragging.
          ...
          _.each(menuItemControls, function (menuItemControl) {
            ...
            menuItemControl.priority(priority); // Note that wpNavMenu will be setting this .menu-item-data-parent-id input's value.
    ...
    _setupTitle: function _setupTitle() {
      ...
      control.setting.bind(function (menu) {
        ...
        var section = api.section(control.section()),
            ...
            name = displayNavMenuName(menu.name); // Update the control title.
     ...
     */
    toggleReordering: function toggleReordering(showOrHide) {
    ...
    reflowMenuItems: function reflowMenuItems() {
      ...
      _reflowRecursively = function reflowRecursively(context) {
        ...
        _.each(context.menuItemControls, function (menuItemControl) {
          if (thisParent === menuItemControl.setting().menu_item_parent) {
            currentMenuItemControls.push(menuItemControl); // @todo We could remove this item from menuItemControls now, for efficiency.
          }
        });
        ...
        _.each(currentMenuItemControls, function (menuItemControl) {
          ...
          menuItemControl.container.data('item-depth', context.currentDepth); // Process any children items.

          context.currentDepth += 1;
          context.currentParent = menuItemControl.params.menu_item_id;

          _reflowRecursively(context);
        ...
        if (currentMenuItemControls.length) {
          ...
          currentMenuItemControls[0].container.addClass('move-up-disabled').addClass('move-right-disabled').toggleClass('move-down-disabled', 1 === currentMenuItemControls.length);
          currentMenuItemControls[currentMenuItemControls.length - 1].container.addClass('move-down-disabled').toggleClass('move-up-disabled', 1 === currentMenuItemControls.length);
        }
      };
     ...
     */
    addItemToMenu: function addItemToMenu(item) {
      var menuControl = this,
          customizeId,
          settingArgs,
          setting,
     ...
     */
    updateInvitationVisibility: function updateInvitationVisibility(optionalMenuItemControls) {
  ...
  api.Menus.applySavedData = function (data) {
    ...
    _(data.nav_menu_updates).each(function (update) {
      ...
      if ('inserted' === update.status) {
        ...
        api.section.add(newSection); // Update the values for nav menus in Navigation Menu controls.
        ...
        oldSetting.preview();
        newSetting.preview();
        oldSetting._dirty = false; // Remove nav_menu section.

        oldSection.container.remove();
        api.section.remove(oldCustomizeId); // Update the nav_menu widget to reflect removed placeholder menu.
        ...
        widgetTemplate.find('.nav-menu-widget-form-controls:first').toggle(0 !== navMenuCount);
        widgetTemplate.find('.nav-menu-widget-no-menus-message:first').toggle(0 === navMenuCount);
        widgetTemplate.find('option[value=' + String(update.previous_term_id) + ']').remove(); // Update the nav_menu_locations[...] controls to remove the placeholder menus from the dropdown options.
    ...
    _(data.nav_menu_item_updates).each(function (update) {
      ...
      if ('inserted' === update.status) {
        ...
        settingValue = _.clone(settingValue); // If the parent menu item was also inserted, update the menu_item_parent to the new ID.
        ...
        api.control.remove(oldCustomizeId); // Add new control to take its place.

        api.control.add(newControl); // Delete the placeholder and preview the new setting.
