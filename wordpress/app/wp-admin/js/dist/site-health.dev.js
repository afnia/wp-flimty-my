...
jQuery(function ($) {
  var __ = wp.i18n.__,
      ...
      successTimeout; // Debug information copy section.

  clipboard.on('success', function (e) {
    var triggerElement = $(e.trigger),
        successElement = $('.success', triggerElement.closest('div')); // Clear the selection and move focus back to the trigger.

    e.clearSelection(); // Handle ClipboardJS focus bug, see https://github.com/zenorocha/clipboard.js/issues/680
  ...
  function validateIssueData(issue) {
    ...
    var minimumExpected = {
      ...
      description: 'string'
        ...
        value,
        subKey,
        subValue; // If the issue passed is not an object, return a `false` state early.

    if ('object' !== _typeof(issue)) {
      return false;
    ...
    for (key in minimumExpected) {
      ...
      if ('object' === _typeof(value)) {
        for (subKey in value) {
          ...
          if ('undefined' === typeof issue[key] || 'undefined' === typeof issue[key][subKey] || subValue !== _typeof(issue[key][subKey])) {
            passed = false;
          }
        }
      } else {
        if ('undefined' === typeof issue[key] || value !== _typeof(issue[key])) {
          passed = false;
        }
      }
    }
  ...
  function appendIssue(issue) {
    ...
    count = SiteHealth.site_status.issues[issue.status]; // If no test name is supplied, append a placeholder for markup references.
  ...
  function getDirectorySizes() {
    var timestamp = new Date().getTime(); // After 3 seconds announce that we're still waiting for directory sizes.
