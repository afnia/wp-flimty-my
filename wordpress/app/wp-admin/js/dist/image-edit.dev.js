...
(function ($) {
  ...
  var imageEdit = window.imageEdit = {
     ...
     * Enable crop tool.
     */
    toggleCropTool: function toggleCropTool(postid, nonce, cropButton) {
      var img = $('#image-preview-' + postid),
          selection = this.iasapi.getSelection();
      imageEdit.toggleControls(cropButton);
      var $el = $(cropButton);
      var state = $el.attr('aria-expanded') === 'true' ? 'true' : 'false'; // Crop tools have been closed.
      ...
      if ('false' === state) {
        ...
        imageEdit.setDisabled($('.imgedit-crop-clear'), 1); // Get values from inputs to restore previous selection.
        ...
        var height = $('#imgedit-sel-height-' + postid).val() ? $('#imgedit-sel-height-' + postid).val() : img.innerHeight(); // Ensure selection is available, otherwise reset to full image.
     ...
     */
    handleCropToolClick: function handleCropToolClick(postid, nonce, cropButton) {
     ...
     * @return {void}
     */
    setDisabled: function setDisabled(el, s) {
      /*
    ...
    init: function init(postid) {
      ...
      $('#imgedit-panel-' + postid).on('keypress', 'input[type="text"]', function (e) {
        var k = e.keyCode; // Key codes 37 through 40 are the arrow keys.
     ...
     * @return {void}
     */
    toggleEditor: function toggleEditor(postid, toggle, triggerUIReady) {
     ...
     */
    togglePopup: function togglePopup(el) {
      var $el = $(el);
      var $targetEl = $(el).attr('aria-controls');
      var $target = $('#' + $targetEl);
      $el.attr('aria-expanded', 'false' === $el.attr('aria-expanded') ? 'true' : 'false'); // Open menu and set z-index to appear above image crop area if it is enabled.

      $target.toggleClass('imgedit-popup-menu-open').slideToggle('fast').css({
        'z-index': 200000
    ...
    monitorPopup: function monitorPopup() {
      ...
      setTimeout(function () {
        ...
        var $contains = $parent.contains($focused); // If $focused is defined and not inside the menu container, close the popup.
     ...
     * @return {boolean} Always returns false.
     */
    browsePopup: function browsePopup(el) {
      var $el = $(el);
     ...
     * @return {boolean} Always returns false.
     */
    toggleHelp: function toggleHelp(el) {
      var $el = $(el);
      $el.attr('aria-expanded', 'false' === $el.attr('aria-expanded') ? 'true' : 'false').parents('.imgedit-group-top').toggleClass('imgedit-help-toggled').find('.imgedit-help').slideToggle('fast');
      return false;
    },
     ...
     * @return {boolean} Always returns false.
     */
    toggleControls: function toggleControls(el) {
      var $el = $(el);
     ...
     */
    getTarget: function getTarget(postid) {
     ...
     * @return {void}
     */
    scaleChanged: function scaleChanged(postid, x, el) {
     ...
     */
    getSelRatio: function getSelRatio(postid) {
      var x = this.hold.w,
          y = this.hold.h,
     ...
     */
    filterHistory: function filterHistory(postid, setSize) {
      ...
      if (history !== '') {
        ...
        if (setSize) {
          ...
          o = history[history.length - 1]; // c = 'crop', r = 'rotate', f = 'flip'.

          o = o.c || o.r || o.f || false;

          if (o) {
            // fw = Full image width.
            this.hold.w = o.fw; // fh = Full image height.
     ...
     * @return {void}
     */
    refreshEditor: function refreshEditor(postid, nonce, callback) {
      var t = this,
          data,
          img;
      ...
      img = $('<img id="image-preview-' + postid + '" alt="" />').on('load', {
        ...
        var max1,
            ...
            t = imageEdit,
            historyObj; // Checks if there already is some image-edit history.

        if ('' !== event.data.history) {
          historyObj = JSON.parse(event.data.history); // If last executed action in history is a crop action.
          ...
          if (historyObj[historyObj.length - 1].hasOwnProperty('c')) {
             ...
             */
            t.setDisabled($('#image-undo-' + postid), true); // Move focus to the undo button to avoid a focus loss.

            $('#image-undo-' + postid).trigger('focus');
          }
        }

        parent.empty().append(img); // w, h are the new full size dimensions.
        ...
        $('#imgedit-crop-' + postid).empty().append('<div class="notice notice-error" tabindex="-1" role="alert"><p>' + errorMessage + '</p></div>');
     ...
     */
    action: function action(postid, nonce, _action) {
      var t = this,
          data,
          w,
          h,
      ...
      if ('scale' === _action) {
        w = $('#imgedit-scale-width-' + postid), h = $('#imgedit-scale-height-' + postid), fw = t.intval(w.val()), fh = t.intval(h.val());
      ...
      $.post(ajaxurl, data, function (response) {
        ...
        t.toggleEditor(postid, 0, true); // Refresh the attachment model so that changes propagate.
     ...
     */
    save: function save(postid, nonce) {
      var data,
      ...
      $.post(ajaxurl, data, function (response) {
        ...
        if (response.data.error) {
          $('#imgedit-response-' + postid).html('<div class="notice notice-error" tabindex="-1" role="alert"><p>' + response.data.error + '</p></div>');
        ...
        if (response.data.msg) {
          $('#imgedit-response-' + postid).html('<div class="notice notice-success" tabindex="-1" role="alert"><p>' + response.data.msg + '</p></div>');
     ...
     */
    open: function open(postid, nonce, view) {
      this._view = view;
      var dfd,
          data,
     ...
     * @return {void}
     */
    imgLoaded: function imgLoaded(postid) {
      var img = $('#image-preview-' + postid),
          parent = $('#imgedit-crop-' + postid); // Ensure init has run even when directly loaded.
     ...
     * @return {void}
     */
    initCrop: function initCrop(postid, image, parent) {
      var t = this,
      ...
      t.iasapi = $image.imgAreaSelect({
         ...
         * @return {Object}
         */
        onSelectEnd: function onSelectEnd(img, c) {
         ...
         * @return {void}
         */
        onSelectChange: function onSelectChange(img, c) {
     ...
     * @return {boolean}
     */
    setCropSelection: function setCropSelection(postid, c) {
      var sel;
      c = c || 0;
     ...
     */
    close: function close(postid, warn) {
      warn = warn || false;
      ...
      this.iasapi = {};
      this.hold = {}; // If we've loaded the editor in the context of a Media Modal,
     ...
     */
    notsaved: function notsaved(postid) {
     ...
     * @return {void}
     */
    addStep: function addStep(op, postid, nonce) {
      var t = this,
     ...
     * @return {boolean}
     */
    rotate: function rotate(angle, postid, nonce, t) {
     ...
     * @return {boolean}
     */
    flip: function flip(axis, postid, nonce, t) {
      if ($(t).hasClass('disabled')) {
        return false;
      }
     ...
     */
    crop: function crop(postid, nonce, t) {
     ...
     */
    undo: function undo(postid, nonce) {
      var t = this,
      ...
      t.refreshEditor(postid, nonce, function () {
        ...
        t.setDisabled(button, pop < history.length); // When undo gets disabled, move focus to the redo button to avoid a focus loss.
     ...
     * @return {void}
     */
    redo: function redo(postid, nonce) {
      var t = this,
      ...
      t.refreshEditor(postid, nonce, function () {
        t.setDisabled($('#image-undo-' + postid), true);
        t.setDisabled(button, pop > 0); // When redo gets disabled, move focus to the undo button to avoid a focus loss.
     ...
     *                        is successful.
     */
    setNumSelection: function setNumSelection(postid, el) {
      var sel,
     ...
     * @return {void}
     */
    setRatioSelection: function setRatioSelection(postid, n, el) {
      var sel,
          r,
          x = this.intval($('#imgedit-crop-width-' + postid).val()),
          y = this.intval($('#imgedit-crop-height-' + postid).val()),
      ...
      if (x && y) {
        ...
        if (sel = this.iasapi.getSelection(true)) {
          ...
          if (r > h) {
            ...
            $('#imgedit-crop-' + postid).prepend('<div class="notice notice-error" tabindex="-1" role="alert"><p>' + errorMessage + '</p></div>');
            wp.a11y.speak(errorMessage, 'assertive');
     ...
     *                        void when it is.
     */
    validateNumeric: function validateNumeric(el) {
