...
(function ($) {
  ...
  api = window.wpNavMenu = {
    ...
    init: function init() {
      ...
      if (menus.oneThemeLocationNoMenus) $('#posttype-page').addSelectedToMenu(api.addMenuItemToBottom);
    ...
    jQueryExtensions: function jQueryExtensions() {
      ...
      $.fn.extend({
        ...
        updateDepthClass: function updateDepthClass(current, prev) {
        ...
        shiftDepthClass: function shiftDepthClass(change) {
          return this.each(function () {
            var t = $(this),
        ...
        shiftHorizontally: function shiftHorizontally(dir) {
          return this.each(function () {
            var t = $(this),
                depth = t.menuItemDepth(),
                newDepth = depth + dir; // Change .menu-item-depth-n class.

            t.moveHorizontally(newDepth, depth);
          });
        },
        moveHorizontally: function moveHorizontally(newDepth, depth) {
          return this.each(function () {
            var t = $(this),
                children = t.childMenuItems(),
                diff = newDepth - depth,
                subItemText = t.find('.is-submenu'); // Change .menu-item-depth-n class.

            t.updateDepthClass(newDepth, depth).updateParentMenuItemDBId(); // If it has children, move those too.
            ...
            if (0 === newDepth) subItemText.hide();else subItemText.show();
          });
        },
         ...
         */
        addSelectedToMenu: function addSelectedToMenu(processMethod) {
          ...
          return this.each(function () {
            var t = $(this),
                ...
                re = /menu-item\[([^\]]*)/;
            processMethod = processMethod || api.addMenuItemToBottom; // If no items are checked, bail.

            if (!checkboxes.length) return false; // Show the Ajax spinner.

            t.find('.button-controls .spinner').addClass('is-active'); // Retrieve menu item data.
        ...
        getItemData: function getItemData(itemType, id) {
          itemType = itemType || 'menu-item';
          var itemData = {},
              i,
              fields = ['menu-item-db-id', 'menu-item-object-id', 'menu-item-object', 'menu-item-parent-id', 'menu-item-position', 'menu-item-type', 'menu-item-title', 'menu-item-url', 'menu-item-description', 'menu-item-attr-title', 'menu-item-target', 'menu-item-classes', 'menu-item-xfn'];
          ...
          this.find('input').each(function () {
            ...
            while (i--) {
              if (itemType == 'menu-item') field = fields[i] + '[' + id + ']';else if (itemType == 'add-menu-item') field = 'menu-item[' + id + '][' + fields[i] + ']';
        ...
        setItemData: function setItemData(itemData, itemType, id) {
          ...
          this.find('input').each(function () {
            ...
            $.each(itemData, function (attr, val) {
              if (itemType == 'menu-item') field = attr + '[' + id + ']';else if (itemType == 'add-menu-item') field = 'menu-item[' + id + '][' + attr + ']';
    ...
    countMenuItems: function countMenuItems(depth) {
      return $('.menu-item-depth-' + depth).length;
    },
    moveMenuItem: function moveMenuItem($this, dir) {
      var items,
          newItemPosition,
          newDepth,
      ...
      switch (dir) {
        case 'up':
          newItemPosition = thisItemPosition - 1; // Already at top.

          if (0 === thisItemPosition) break; // If a sub item is moved to top, shift it to 0 depth.

          if (0 === newItemPosition && 0 !== thisItemDepth) thisItem.moveHorizontally(0, thisItemDepth); // If prev item is sub item, shift to match depth.

          if (0 !== prevItemDepth) thisItem.moveHorizontally(prevItemDepth, thisItemDepth); // Does this item have sub items?

          if (thisItemChildren) {
            items = thisItem.add(thisItemChildren); // Move the entire block.
        ...
        case 'down':
          ...
          if (thisItemChildren) {
            items = thisItem.add(thisItemChildren), nextItem = menuItems.eq(items.length + thisItemPosition), nextItemChildren = 0 !== nextItem.childMenuItems().length;
            ...
            if (0 !== nextItemChildren.length) thisItem.moveHorizontally(nextItemDepth, thisItemDepth); // Have we reached the bottom?
        ...
        case 'top':
          // Already at top.
          if (0 === thisItemPosition) break; // Does this item have sub items?
        ...
        case 'right':
          ...
          if (0 === thisItemPosition) break; // Already sub item of prevItem.
    ...
    initAccessibility: function initAccessibility() {
      ...
      api.refreshKeyboardAccessibility();
      api.refreshAdvancedAccessibility(); // Refresh the accessibility when the user comes close to the item in any way.
     ...
     */
    refreshAdvancedAccessibilityOfItem: function refreshAdvancedAccessibilityOfItem(itemToRefresh) {
      ...
      menuItem.find('.field-move').toggle(totalMenuItems > 1); // Where can they move this menu item?
      ...
      if (!isPrimaryMenuItem) {
        thisLink = menuItem.find('.menus-move-left'), thisLinkText = menus.outFrom.replace('%s', prevItemNameLeft);
      ...
      if (0 !== position) {
        if (menuItem.find('.menu-item-data-parent-id').val() !== menuItem.prev().find('.menu-item-data-db-id').val()) {
          thisLink = menuItem.find('.menus-move-right'), thisLinkText = menus.under.replace('%s', prevItemNameRight);
          thisLink.attr('aria-label', menus.moveUnder.replace('%s', prevItemNameRight)).text(thisLinkText).css('display', 'inline');
        }
      }

      if (isPrimaryMenuItem) {
        primaryItems = $('.menu-item-depth-0'), itemPosition = primaryItems.index(menuItem) + 1, totalMenuItems = primaryItems.length, // String together help text for primary menu items.
      ...
      $this.attr('aria-label', title); // Mark this item's accessibility as refreshed.
    ...
    refreshAdvancedAccessibility: function refreshAdvancedAccessibility() {
      ...
      $('.menu-item-settings .field-move .menus-move').hide(); // Mark all menu items as unprocessed.

      $('a.item-edit').data('needs_accessibility_refresh', true); // All open items have to be refreshed or they will show no links.
      ...
      $('a.item-edit').off('focus').on('focus', function () {
        $(this).off('keydown').on('keydown', function (e) {
          var arrows,
              $this = $(this),
              thisItem = $this.parents('li.menu-item'),
              thisItemData = thisItem.getItemData(); // Bail if it's not an arrow key.

          if (37 != e.which && 38 != e.which && 39 != e.which && 40 != e.which) return; // Avoid multiple keydown events.

          $this.off('keydown'); // Bail if there is only one menu item.

          if (1 === $('#menu-to-edit li').length) return; // If RTL, swap left/right arrows.
    ...
    initPreviewing: function initPreviewing() {
      ...
      $('#menu-to-edit').on('change input', '.edit-menu-item-title', function (e) {
        var input = $(e.currentTarget),
            ...
            titleEl;
        title = input.val();
        titleEl = input.closest('.menu-item').find('.menu-item-title'); // Don't update to empty title.
    ...
    initToggles: function initToggles() {
      // Init postboxes.
      postboxes.add_postbox_toggles('nav-menus'); // Adjust columns functions for menus UI.
    ...
    initSortables: function initSortables() {
      ...
      if (0 !== $('#menu-to-edit li').length) $('.drag-instructions').show(); // Use the right edge if RTL.
      ...
      api.menuList.sortable({
        ...
        start: function start(e, ui) {
          var height, width, parent, children, tempHolder; // Handle placement for RTL orientation.

          if (api.isRTL) ui.item[0].style.right = 'auto';
          transport = ui.item.children('.menu-item-transport'); // Set depths. currentDepth must be set before children are located.

          originalDepth = ui.item.menuItemDepth();
          updateCurrentDepth(ui, originalDepth); // Attach child elements to parent.
          // Skip the placeholder.

          parent = ui.item.next()[0] == ui.placeholder[0] ? ui.item.next() : ui.item;
          children = parent.childMenuItems();
          transport.append(children); // Update the height of the placeholder to match the moving item.

          height = transport.outerHeight(); // If there are children, account for distance between top of children and parent.
          ...
          ui.placeholder.height(height); // Update the width of the placeholder to match the moving item.
          ...
          ui.placeholder.width(width); // Update the list of menu items.
        ...
        stop: function stop(e, ui) {
          var children,
              subMenuTitle,
              depthChange = currentDepth - originalDepth; // Return child elements to the list.

          children = transport.children().insertAfter(ui.item); // Add "sub menu" description.

          subMenuTitle = ui.item.find('.item-title .is-submenu');
          if (0 < currentDepth) subMenuTitle.show();else subMenuTitle.hide(); // Update depth classes.
          ...
          api.registerChange(); // Update the item data.

          ui.item.updateParentMenuItemDBId(); // Address sortable's incorrectly-calculated top in Opera.

          ui.item[0].style.top = 0; // Handle drop placement for rtl orientation.
        ...
        change: function change(e, ui) {
          ...
          if (!ui.placeholder.parent().hasClass('menu')) prev.length ? prev.after(ui.placeholder) : api.menuList.prepend(ui.placeholder);
          updateSharedVars(ui);
        },
        sort: function sort(e, ui) {
          ...
          if (depth != currentDepth) updateCurrentDepth(ui, depth); // If we overlap the next element, manually shift downwards.
      ...
      function updateSharedVars(ui) {
        ...
        next = ui.placeholder.next('.menu-item'); // Make sure we don't select the moving item.
        ...
        if (prev.length) maxDepth = (depth = prev.menuItemDepth() + 1) > api.options.globalMaxDepth ? api.options.globalMaxDepth : depth;else maxDepth = 0;
      }
    ...
    initManageLocations: function initManageLocations() {
      ...
      $('.menu-location-menus select').on('change', function () {
        ...
        if ($(this).find('option:selected').data('orig')) editLink.show();else editLink.hide();
      });
    },
    ...
    attachQuickSearchListeners: function attachQuickSearchListeners() {
      var searchTimer; // Prevent form submission.
    ...
    updateQuickSearchResults: function updateQuickSearchResults(input) {
      var panel,
          params,
          minSearchLength = 2,
          q = input.val();
    ...
    addCustomLink: function addCustomLink(processMethod) {
      ...
      this.addLinkToMenu(url, label, processMethod, function () {
        ...
        $('.customlinkdiv .spinner').removeClass('is-active'); // Set custom link form back to defaults.

        $('#custom-menu-item-name').val('').trigger('blur');
        $('#custom-menu-item-url').val('').attr('placeholder', 'https://');
      });
    },
    addLinkToMenu: function addLinkToMenu(url, label, processMethod, callback) {
      processMethod = processMethod || api.addMenuItemToBottom;
    ...
    addItemToMenu: function addItemToMenu(menuItem, processMethod, callback) {
      var menu = $('#menu').val(),
      ...
      $.post(ajaxurl, params, function (menuMarkup) {
        ...
        processMethod(menuMarkup, params); // Make it stand out a bit more visually, by adding a fadeIn.
     ...
     */
    addMenuItemToBottom: function addMenuItemToBottom(menuMarkup) {
     ...
     * @fires document#menu-item-added Passes menuMarkup as a jQuery object.
     */
    addMenuItemToTop: function addMenuItemToTop(menuMarkup) {
      var $menuMarkup = $(menuMarkup);
    ...
    attachUnsavedChangesListener: function attachUnsavedChangesListener() {
      ...
      if (0 !== $('#menu-to-edit').length || 0 !== $('.menu-location-menus select').length) {
        window.onbeforeunload = function () {
          if (api.menusChanged) return wp.i18n.__('The changes you made will be lost if you navigate away from this page.');
        };
      } else {
    ...
    attachTabsPanelListeners: function attachTabsPanelListeners() {
      $('#menu-settings-column').on('click', function (e) {
        ...
        if (target.hasClass('nav-tab-link')) {
          ...
          wrapper = target.parents('.accordion-section-content').first(); // Upon changing tabs, we want to uncheck all checkboxes.

          $('input', wrapper).prop('checked', false);
          $('.tabs-panel-active', wrapper).removeClass('tabs-panel-active').addClass('tabs-panel-inactive');
          $('#' + panelId, wrapper).removeClass('tabs-panel-inactive').addClass('tabs-panel-active');
          $('.tabs', wrapper).removeClass('tabs');
          target.parent().addClass('tabs'); // Select the search bar.

          $('.quick-search', wrapper).trigger('focus'); // Hide controls in the search tab if no items found.
          ...
          api.registerChange();
          if (e.target.id && 'submit-customlinkdiv' == e.target.id) api.addCustomLink(api.addMenuItemToBottom);else if (e.target.id && -1 != e.target.id.indexOf('submit-')) $('#' + e.target.id.replace(/submit-/, '')).addSelectedToMenu(api.addMenuItemToBottom);
          return false;
        }
      });
      /*
      ...
      $('#nav-menu-meta').on('click', 'a.page-numbers', function () {
        ...
        $.post(ajaxurl, this.href.replace(/.*\?/, '').replace(/action=([^&]*)/, '') + '&action=menu-get-metabox', function (resp) {
    ...
    eventOnClickEditLink: function eventOnClickEditLink(clickedEl) {
      var settings,
          item,
    ...
    eventOnClickCancelLink: function eventOnClickCancelLink(clickedEl) {
      ...
      settings.setItemData(settings.data('menu-item-data')).hide(); // Restore the title of the currently active/expanded menu item.
    ...
    eventOnClickMenuSave: function eventOnClickMenuSave() {
      var locs = '',
          menuName = $('#menu-name'),
          menuNameVal = menuName.val(); // Cancel and warn if invalid menu name.
      ...
      $('#update-nav-menu').append(locs); // Update menu item position data.
    ...
    eventOnClickMenuItemDelete: function eventOnClickMenuItemDelete(clickedEl) {
     ...
     */
    processQuickSearchQueryResponse: function processQuickSearchQueryResponse(resp, req, panel) {
      var matched,
          newID,
          takenIDs = {},
      ...
      $items.each(function () {
        $item = $(this); // Make a unique DB ID number.
     ...
     */
    removeMenuItem: function removeMenuItem(el) {
    ...
    depthToPx: function depthToPx(depth) {
      return depth * api.options.menuItemDepthPerLevel;
    },
    pxToDepth: function pxToDepth(px) {
      return Math.floor(px / api.options.menuItemDepthPerLevel);
    }
  };
  $(function () {
    wpNavMenu.init(); // Prevent focused element from being hidden by the sticky footer.
