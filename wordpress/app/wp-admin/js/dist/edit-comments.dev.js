...
(function ($) {
  ...
  updateApproved = function updateApproved(diff, commentPostId) {
  ...
  updateCountText = function updateCountText(selector, diff) {
  ...
  updateDashboardText = function updateDashboardText(response) {
  ...
  updateInModerationText = function updateInModerationText(response) {
    if (!response || !response.i18n_moderation_text) {
      return;
    } // Update the "comment in moderation" text across the UI.


    $('.comments-in-moderation-text').text(response.i18n_moderation_text); // Hide the "comment in moderation" text in the Dashboard "At a Glance" widget.
  ...
  updateHtmlTitle = function updateHtmlTitle(diff) {
    var newTitle, regExMatch, titleCount, commentFrag;
    /* translators: %s: Comments count. */

    titleRegEx = titleRegEx || new RegExp(__('Comments (%s)').replace('%s', '\\([0-9' + thousandsSeparator + ']+\\)') + '?'); // Count funcs operate on a $'d element.
  ...
  updatePending = function updatePending(diff, commentPostId) {
    var postSelector = '.post-com-count-' + commentPostId,
  ...
  window.setCommentsList = function () {
    ...
    updateTotalCount = function updateTotalCount(total, time, setConfidentTime) {
    ...
    dimAfter = function dimAfter(r, settings) {
      var editRow,
          replyID,
          replyButton,
          response,
      ...
      if (c.is('.unapproved')) {
        ...
        c.find('.row-actions span.view').addClass('hidden').end().find('div.comment_status').html('0');
      } else {
        if (settings.data.id == replyID) replyButton.text(__('Reply'));
        c.find('.row-actions span.view').removeClass('hidden').end().find('div.comment_status').html('1');
      }
    ...
    delBefore = function delBefore(settings, list) {
      var note,
          id,
          el,
          n,
      ...
      if (wpListsData.indexOf(':trash=1') != -1) action = 'trash';else if (wpListsData.indexOf(':spam=1') != -1) action = 'spam';
    ...
    delAfter = function delAfter(r, settings) {
      ...
      if (targetParent.is('span.undo')) {
        ...
        undoing = true; // User clicked "Spam".
      } else if (targetParent.is('span.spam')) {
        // The comment is currently approved.
        if (approved) {
          approvedDiff = -1; // The comment is currently pending.
        } else if (unapproved) {
          pendingDiff = -1; // The comment was in the Trash.
        } else if (trashed) {
          trashDiff = -1;
        } // You can't spam an item on the Spam screen.


        spamDiff = 1; // User clicked "Unspam".
        ...
        if (approved) {
          ...
          if (targetParent.hasClass('approve')) {
            approvedDiff = 1; // The comment was previously pending.
        ...
        spamDiff = -1; // User clicked "Trash".
      } else if (targetParent.is('span.trash')) {
        if (approved) {
          approvedDiff = -1;
        } else if (unapproved) {
          pendingDiff = -1; // The comment was in the spam queue.
        } else if (spammed) {
          spamDiff = -1;
        } // You can't trash an item on the Trash screen.


        trashDiff = 1; // User clicked "Restore".
      ...
      if ('trash' === settings.data.comment_status && !getCount($('span.trash-count')) || 'spam' === settings.data.comment_status && !getCount($('span.spam-count'))) {
        $('#delete_all').hide();
      }

      if (!isDashboard) {
        total = totalInput.val() ? parseInt(totalInput.val(), 10) : 0;
        if ($(settings.target).parent().is('span.undo')) total++;else total--;
        if (total < 0) total = 0;
      ...
      animatedCallback = function animatedCallback() {
    ...
    refillTheExtraList = function refillTheExtraList(ev) {
      var args = $.query.get(),
      ...
      args.paged++; // $.query.get() needs some correction to be sent into an Ajax request.
    ...
    window.theList = $('#the-comment-list').wpList({
      ...
      if (wpListsData.indexOf(':trash=1') != -1 || wpListsData.indexOf(':spam=1') != -1) $('#undo-' + id).fadeIn(300, function () {
        $(this).show();
      });
    });
  };
  ...
  window.commentReply = {
     ...
     * @return {void}
     */
    addEvents: function addEvents(r) {
      r.each(function () {
    ...
    close: function close() {
      var commentRow = $(),
          replyRow = $('#replyrow'); // Return if the replyrow is not showing.
     ...
     */
    open: function open(comment_id, post_id, action) {
      var editRow,
          rowData,
          act,
          replyButton,
      ...
      colspanVal = $('> th:visible, > td:visible', c).length; // Make sure it's actually a table and there's a `colspan` value to apply.
      ...
      setTimeout(function () {
        ...
        if (scrollBottom - 20 < rbottom) window.scroll(0, rbottom - vp + 35);else if (rtop - 20 < scrollTop) window.scroll(0, rtop - 35);
  ...
  $(function () {
    ...
    if (typeof $.table_hotkeys != 'undefined') {
       ...
       */
      make_hotkeys_redirect = function make_hotkeys_redirect(which) {
        return function () {
          var first_last, l;
          first_last = 'next' == which ? 'first' : 'last';
          l = $('.tablenav-pages .' + which + '-page:not(.disabled)');
          if (l.length) window.location = l[0].href.replace(/\&hotkeys_highlight_(first|last)=1/g, '') + '&hotkeys_highlight_' + first_last + '=1';
        };
      };
      /**
      ...
      edit_comment = function edit_comment(event, current_row) {
      ...
      make_bulk = function make_bulk(value) {
        return function () {
      ...
      $.table_hotkeys($('table.widefat'), ['a', 'u', 's', 'd', 'r', 'q', 'z', ['e', edit_comment], ['shift+x', toggle_all], ['shift+a', make_bulk('approve')], ['shift+s', make_bulk('spam')], ['shift+d', make_bulk('delete')], ['shift+t', make_bulk('trash')], ['shift+z', make_bulk('untrash')], ['shift+u', make_bulk('unapprove')]], {
