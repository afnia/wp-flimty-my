...
(function (exports, $) {
  ...
  /** @lends wp.customize.OverlayNotification.prototype */
  {
     ...
     */
    initialize: function initialize(code, params) {
  ...
  /** @lends wp.customize.Notifications.prototype */
  {
    ...
    initialize: function initialize(options) {
      ...
      _.bindAll(collection, 'constrainFocus'); // Keep track of the order in which the notifications were added for sorting purposes.


      collection._addedIncrement = 0;
      collection._addedOrder = {}; // Trigger change event when notification is added or removed.
     ...
     */
    add: function add(notification, notificationObject) {
      var collection = this,
          code,
          instance;
    ...
    render: function render() {
      var collection = this,
          ...
          listElement,
          focusableElements; // Short-circuit if there are no container to render into.
      ...
      collection.container.toggle(0 !== notifications.length); // Short-circuit if there are no changes to the notifications.
    ...
    constrainFocus: function constrainFocus(event) {
      var collection = this,
          focusableElements; // Prevent keys from escaping.
  ...
  /** @lends wp.customize.Setting.prototype */
  {
     ...
     */
    initialize: function initialize(id, value, options) {
      var setting = this,
          params;
      params = _.extend({
      ...
      setting.notifications = new api.Notifications(); // Whenever the setting's value changes, refresh the preview.
  ...
  api.dirtyValues = function dirtyValues(options) {
    ...
    api.each(function (setting) {
      ...
      settingRevision = api._latestSettingRevisions[setting.id]; // Skip including settings that have already been included in the changeset, if only requesting unsaved.
  ...
  api.requestChangesetUpdate = function requestChangesetUpdate(changes, args) {
    var deferred,
        ...
        data,
        submittedArgs;
    deferred = new $.Deferred(); // Prevent attempting changeset update while request is being made.
    ...
    api.trigger('changeset-save', submittedChanges, submittedArgs); // Short-circuit when there are no pending changes.
    ...
    request.done(function requestChangesetUpdateDone(data) {
      var savedChangesetValues = {}; // Ensure that all settings updated subsequently will be included in the next changeset update request.
      ...
      api.previewer.send('changeset-saved', _.extend({}, data, {
  ...
  api.utils.areElementListsEqual = function (listA, listB) {
    var equal = listA.length === listB.length && // If lists are different lengths, then naturally they are not equal.
  ...
  /** @lends wp.customize~Container.prototype */
  {
    ...
    initialize: function initialize(id, options) {
      ...
      $.extend(container, {
        params: _.defaults(options.params || options, // Passing the params is deprecated.
        container.defaults)
      });
    ...
    setupNotifications: function setupNotifications() {
      var container = this,
          renderNotifications;
      container.notifications.container = container.getNotificationsContainerElement(); // Render notifications when they change and when the construct is expanded.
     ...
     * @return {Array}
     */
    _children: function _children(parentType, childType) {
      var parent = this,
          children = [];
     ...
     */
    onChangeActive: function onChangeActive(active, args) {
      var construct = this,
     ...
     */
    _toggleActive: function _toggleActive(active, params) {
      var self = this;
      params = params || {};
     ...
     */
    _toggleExpanded: function _toggleExpanded(expanded, params) {
      var instance = this,
          previousCompleteCallback;
      params = params || {};
      previousCompleteCallback = params.completeCallback; // Short-circuit expand() if the instance is not active.
     ...
     */
    collapse: function collapse(params) {
     ...
     * @return {void}
     */
    _animateChangeExpanded: function _animateChangeExpanded(completeCallback) {
      ...
      var construct = this,
          ...
          transitionParentPane; // Determine set of elements that are affected by the animation.
      ...
      _transitionEndCallback = function transitionEndCallback(e) {
        if (2 !== e.eventPhase || !$(e.target).is(content)) {
          return;
        }

        content.off(normalizedTransitionendEventName, _transitionEndCallback);
      ...
      content.on(normalizedTransitionendEventName, _transitionEndCallback);
      elements.addClass('busy'); // Prevent screen flicker when pane has been scrolled before expanding.
    ...
    getContainer: function getContainer() {
      ...
      if (template && container.container) {
        return template(_.extend({
          id: container.id
        }, container.params)).toString().trim();
      }
  ...
  /** @lends wp.customize.Section.prototype */
  {
    ...
    initialize: function initialize(id, options) {
      var section = this,
          params;
      params = options.params || options; // Look up the type if one was not supplied.
    ...
    embed: function embed() {
      var inject,
          section = this;
      section.containerParent = api.ensure(section.containerParent); // Watch for changes to the panel state.

      inject = function inject(panelId) {
        var parentContainer;
  ...
  /** @lends wp.customize.ThemesSection.prototype */
  {
    ...
    embed: function embed() {
      var inject,
          section = this; // Watch for changes to the panel state.
    ...
    ready: function ready() {
      ...
      section.template = wp.template('customize-themes-details-view'); // Bind global keyboard events.
    ...
    attachEvents: function attachEvents() {
      var section = this,
          debounced; // Expand/Collapse accordion sections on click.
      ...
      section.headerContainer = $('#accordion-section-' + section.id); // Expand section/panel. Only collapse when opening another section.
      ...
      section.headerContainer.on('click', '.customize-themes-section-title', function () {
        ...
        if (section.headerContainer.find('.filter-details').length) {
          section.headerContainer.find('.customize-themes-section-title').toggleClass('details-open').attr('aria-expanded', function (i, attr) {
            return 'true' === attr ? 'false' : 'true';
          });
      ...
      section.contentContainer.on('click', '.feature-filter-toggle', function (e) {
        ...
        $filterToggle.toggleClass('open').attr('aria-expanded', function (i, attr) {
          return 'true' === attr ? 'false' : 'true';
      ...
      updateSelectedState(); // Move section controls to the themes area.
    ...
    onChangeExpanded: function onChangeExpanded(expanded, args) {
      ...
      var section = this,
          container = section.contentContainer.closest('.customize-themes-full-container'); // Immediately call the complete callback if there were no changes.
        ...
        api.section.each(function (otherSection) {
          ...
          if (otherSection !== section) {
            ...
            if ('themes' === otherSection.params.type) {
              ...
              section.contentContainer.find('.wp-filter-search').val(searchTerm); // Directly initialize an empty remote search to avoid a race condition.
      ...
      if (expanded) {
        ...
        section.contentContainer.removeClass('current-section'); // Always hide, even if they don't exist or are already hidden.
    ...
    loadThemes: function loadThemes() {
      ...
      request.done(function (data) {
        var themes = data.themes; // Stop and try again if the term changed while loading.
     ...
     * @return {void}
     */
    loadControls: function loadControls(themes, page) {
      var newThemeControls = [],
          section = this; // Add controls for each theme.
    ...
    loadMore: function loadMore() {
      ...
      if (!section.fullyLoaded && !section.loading) {
        ...
        bottom = container.scrollTop() + container.height(); // Use a fixed distance to the bottom of loaded results to avoid unnecessarily
     ...
     * @return {void}
     */
    filterSearch: function filterSearch(term) {
      var count = 0,
          visible = false,
          section = this,
      ...
      api.reflowPaneContents(); // Update theme count.
     ...
     * @return {void}
     */
    checkTerm: function checkTerm(section) {
      var newTerm;
     ...
     * @return {void}
     */
    initializeNewQuery: function initializeNewQuery(newTerm, newTags) {
      var section = this; // Clear the controls in the section.
      ...
      section.screenshotQueue = null; // Run a new query, with loadThemes handling paging, etc.
    ...
    renderScreenshots: function renderScreenshots() {
      var section = this; // Fill queue initially, or check for more if empty.
     ...
     */
    loadThemePreview: function loadThemePreview(themeId) {
     ...
     * @return {void}
     */
    showDetails: function showDetails(theme, callback) {
      var section = this,
     ...
     * @return {void}
     */
    containFocus: function containFocus(el) {
      var tabbables;
      el.on('keydown', function (event) {
        ...
        tabbables = $(':tabbable', el); // Keep focus within the overlay.
  ...
  /** @lends wp.customize.Panel.prototype */
  {
    ...
    attachEvents: function attachEvents() {
      var meta,
          panel = this; // Expand/Collapse accordion sections on click.
    ...
    renderContent: function renderContent() {
      var template,
          panel = this; // Add the content to the container.
  ...
  /** @lends wp.customize.ThemsPanel.prototype */
  {
     ...
     */
    attachEvents: function attachEvents() {
      var panel = this; // Attach regular panel events.

      api.Panel.prototype.attachEvents.apply(panel); // Temporary since supplying SFTP credentials does not work yet. See #42184.
      ...
      api.state('changesetStatus').bind(toggleDisabledNotifications); // Collapse panel to customize the current theme.
    ...
    onChangeExpanded: function onChangeExpanded(expanded, args) {
      var panel = this,
          overlay,
          sections,
          hasExpandedSection = false; // Expand/collapse the panel normally.

      api.Panel.prototype.onChangeExpanded.apply(this, [expanded, args]); // Immediately call the complete callback if there were no changes.
      ...
      if (expanded) {
        overlay.addClass('in-themes-panel').delay(200).find('.customize-themes-full-container').addClass('animate');

        _.delay(function () {
          overlay.addClass('themes-panel-expanded');
        }, 200); // Automatically open the first section (except on small screens), if one isn't already expanded.
        ...
        overlay.removeClass('in-themes-panel themes-panel-expanded').find('.customize-themes-full-container').removeClass('animate');
      }
    },
     ...
     * @return {jQuery.promise} Promise.
     */
    installTheme: function installTheme(event) {
      var panel = this,
          preview,
          onInstallSuccess,
          ...
          deferred = $.Deferred(),
          request;
      preview = $(event.target).hasClass('preview'); // Temporary since supplying SFTP credentials does not work yet. See #42184.
      ...
      onInstallSuccess = function onInstallSuccess(response) {
        var theme = false,
            themeControl;
        ...
        if (preview) {
          ...
          api.control.add(themeControl);
          api.control(themeControl.id).container.trigger('render-screenshot'); // Close the details modal if it's open to the installed theme.
    ...
    loadThemePreview: function loadThemePreview(themeId) {
      var panel = this,
          deferred = $.Deferred(),
          _onceProcessingComplete,
          urlParser,
          queryParams; // Prevent loading a non-active theme preview when there is a drafted/scheduled changeset.
      ...
      urlParser.search = $.param(queryParams); // Update loading message. Everything else is handled by reloading the page.
      ...
      _onceProcessingComplete = function onceProcessingComplete() {
        var request;
        ...
        api.state('processing').unbind(_onceProcessingComplete);
      ...
      if (0 === api.state('processing').get()) {
        ...
        api.state('processing').bind(_onceProcessingComplete);
      }
     ...
     * @return {void}
     */
    updateTheme: function updateTheme(event) {
      wp.updates.maybeRequestFilesystemCredentials(event);
     ...
     * @return {void}
     */
    deleteTheme: function deleteTheme(event) {
      var theme, section;
      theme = $(event.target).data('slug');
      section = api.section('installed_themes');
      event.preventDefault(); // Temporary since supplying SFTP credentials does not work yet. See #42184.
      ...
      $(document).one('wp-theme-delete-success', function () {
        var control = api.control('installed_theme_' + theme); // Remove theme control.

        control.container.remove();
        api.control.remove(control.id); // Update installed count.

        section.loaded = section.loaded - 1;
        section.updateCount(); // Rerender any other theme controls as uninstalled.
  ...
  /** @lends wp.customize.Control.prototype */
  {
    ...
    initialize: function initialize(id, options) {
      var control = this,
          ...
          settings,
          gatherSettings;
      control.params = _.extend({}, control.defaults, control.params || {}, // In case subclass already defines.
      ...
      _.extend(settings, control.params.settings); // Note: Settings can be an array or an object, with values being either setting IDs or Setting (or Value) objects.
      ...
      gatherSettings = function gatherSettings() {
    ...
    embed: function embed() {
      var control = this,
          inject; // Watch for changes to the section state.

      inject = function inject(sectionId) {
        var parentContainer;
    ...
    setupNotifications: function setupNotifications() {
      var control = this,
          ...
          onSectionAssigned; // Add setting notifications to the control notification.
      ...
      renderNotificationsIfVisible = function renderNotificationsIfVisible() {
      ...
      control.notifications.bind('rendered', function () {
        ...
        control.container.toggleClass('has-error', 0 !== _.where(notifications, {
          type: 'error'
        }).length);
      });

      onSectionAssigned = function onSectionAssigned(newSectionId, oldSectionId) {
    ...
    renderNotifications: function renderNotifications() {
      ...
      container.empty().append(control.notificationsTemplate({
    ...
    renderContent: function renderContent() {
      var control = this,
          ...
          templateId,
          sectionId;
      standardTypes = ['button', 'checkbox', 'date', 'datetime-local', 'email', 'month', 'number', 'password', 'radio', 'range', 'search', 'select', 'tel', 'time', 'text', 'textarea', 'week', 'url'];
      templateId = control.templateSelector; // Use default content template when a standard HTML type is used,
      // there isn't a more specific template existing, and the control container is empty.

      if (templateId === 'customize-control-' + control.params.type + '-content' && _.contains(standardTypes, control.params.type) && !document.getElementById('tmpl-' + templateId) && 0 === control.container.children().length) {
    ...
    addNewPage: function addNewPage() {
      ...
      input.attr('disabled', 'disabled'); // The menus functions add the page, publish when appropriate,
      ...
      promise.done(function (data) {
        var availableItem, $content, itemTemplate; // Prepare the new page as an available menu item.
        ...
        $content.prepend(itemTemplate(availableItem.attributes)); // Focus the select control.
  ...
  /** @lends wp.customize.MediaControl.prototype */
  {
     ...
     */
    ready: function ready() {
      var control = this; // Shortcut so that we don't have to use _.bind every time we add a callback.

      _.bindAll(control, 'restoreDefault', 'removeFile', 'openFrame', 'select', 'pausePlayer'); // Bind events, with delegation to facilitate re-rendering.
      ...
      control.container.on('click keydown', '.remove-button', control.cleanupPlayer); // Resize the player controls when it becomes visible (ie when section is expanded).
      ...
      function setAttachmentDataAndRenderContent(value) {
        ...
        if ('pending' === hasAttachmentData.state()) {
          wp.media.attachment(value).fetch().done(function () {
            ...
            hasAttachmentData.resolve(); // Send attachment information to the preview for possible use in `postMessage` transport.
      ...
      setAttachmentDataAndRenderContent(control.setting()); // Update the attachment data and re-render the control when the setting changes.
    ...
    select: function select() {
      ...
      this.params.attachment = attachment; // Set the Customizer setting; the callback takes care of rendering.

      this.setting(attachment.id);
      node = this.container.find('audio, video').get(0); // Initialize audio/video previews.
     ...
     */
    restoreDefault: function restoreDefault(event) {
      if (api.utils.isKeydownButNotEnterEvent(event)) {
        return;
      }
  ...
  /** @lends wp.customize.HeaderControl.prototype */
  {
    ready: function ready() {
      ...
      api.HeaderTool.combinedList = api.HeaderTool.CombinedList = new api.HeaderTool.CombinedList([api.HeaderTool.UploadsList, api.HeaderTool.DefaultsList]); // Ensure custom-header-crop Ajax requests bootstrap the Customizer to activate the previewed theme.
     ...
     */
    onSkippedCrop: function onSkippedCrop(selection) {
     ...
     * @param {number} height
     */
    setImageFromURL: function setImageFromURL(url, attachmentId, width, height) {
      var choice,
          data = {};
      data.url = url;
  ...
  /** @lends wp.customize.ThemeControl.prototype */
  {
     ...
     */
    filter: function filter(terms) {
      var control = this,
          matchCount = 0,
          haystack = control.params.theme.name + ' ' + control.params.theme.description + ' ' + control.params.theme.tags + ' ' + control.params.theme.author + ' ';
      haystack = haystack.toLowerCase().replace('-', ' '); // Back-compat for behavior in WordPress 4.2.0 to 4.8.X.
      ...
      if (control.params.theme.name.toLowerCase() === terms.join(' ')) {
        ...
        matchCount = matchCount + 10 * (haystack.split(terms.join(' ')).length - 1); // Search for each term individually (as whole-word and partial match) and sum weighted match counts.
     ...
     * @return {void}
     */
    rerenderAsInstalled: function rerenderAsInstalled(installed) {
      var control = this,
          section;
  ...
  /** @lends wp.customize.CodeEditorControl.prototype */
  {
    ...
    initialize: function initialize(id, options) {
      ...
      api.Control.prototype.initialize.call(control, id, options); // Note that rendering is debounced so the props will be used when rendering happens after add event.
      ...
      api.section(control.section(), function (section) {
        section.deferred.embedded.done(function () {
          ...
          if (section.expanded()) {
            ...
            _onceExpanded = function onceExpanded(isExpanded) {
              if (isExpanded) {
                control.initEditor();
                section.expanded.unbind(_onceExpanded);
              }
            };

            section.expanded.bind(_onceExpanded);
          }
        });
      });
    },
    ...
    initEditor: function initEditor() {
      var control = this,
          element,
          editorSettings = false; // Obtain editorSettings for instantiation.
     ...
     * @return {void}
     */
    initSyntaxHighlightingEditor: function initSyntaxHighlightingEditor(codeEditorSettings) {
      var control = this,
      ...
      control.editor = wp.codeEditor.initialize($textarea, settings); // Improve the editor accessibility.
  ...
  /** @lends wp.customize.DateTimeControl.prototype */
  {
    ...
    ready: function ready() {
      ...
      control.container.find('.date-input').each(function () {
        ...
        control.elements.push(element); // Add invalid date error once user changes (and has blurred the input).
      ...
      control.populateDateInputs();
      control.setting.bind(control.populateDateInputs); // Start populating setting after inputs have been populated.
    ...
    convertInputDateToString: function convertInputDateToString() {
      ...
      pad = function pad(number, padding) {
        var zeros;
      ...
      getElementValue = function getElementValue(component) {
  ...
  api.control = new api.Values({
    defaultConstructor: api.Control
  });
  /**
  ...
  api.section = new api.Values({
    defaultConstructor: api.Section
  });
  /**
  ...
  api.panel = new api.Values({
    defaultConstructor: api.Panel
  });
  /**
  ...
  /** @lends wp.customize.PreviewFrame.prototype */
  {
     ...
     */
    initialize: function initialize(params, options) {
    ...
    run: function run(deferred) {
      ...
      previewFrame.bind('iframe-loading-error', function (error) {
        previewFrame.iframe.remove(); // Check if the user is not logged in.
  ...
  /** @lends wp.customize.Previewer.prototype */
  {
    ...
    initialize: function initialize(params, options) {
      ...
      previewer.refresh = _.debounce(function (originalRefresh) {
        return function () {
          var isProcessingComplete, _refreshOnceProcessingComplete;

          isProcessingComplete = function isProcessingComplete() {
          ...
          if (isProcessingComplete()) {
            ...
            _refreshOnceProcessingComplete = function refreshOnceProcessingComplete() {
              if (isProcessingComplete()) {
                originalRefresh.call(previewer);
                api.state('processing').unbind(_refreshOnceProcessingComplete);
              }
            };

            api.state('processing').bind(_refreshOnceProcessingComplete);
          }
        };
      }(previewer.refresh), previewer.refreshBuffer);
      ...
      previewer.add('previewUrl', params.previewUrl).setter(function (to) {
        var result = null,
            ...
            parsedCandidateUrls = [];
        urlParser = document.createElement('a');
        urlParser.href = to; // Abort if URL is for admin or (static) files in wp-includes or wp-content.
        ...
        parsedCandidateUrls.push(urlParser); // Prepend list with URL that matches the scheme/protocol of the iframe.
      ...
      previewer.bind('ready', previewer.ready); // Start listening for keep-alive messages when iframe first loads.
    ...
    ready: function ready(data) {
      ...
      previewer.send('sync', synced); // Set the previewUrl without causing the url to set the iframe.
    ...
    keepPreviewAlive: function keepPreviewAlive() {
      ...
      scheduleKeepAliveCheck = function scheduleKeepAliveCheck() {
      ...
      keepAliveTick = function keepAliveTick() {
      ...
      handleMissingKeepAlive = function handleMissingKeepAlive() {
    ...
    refresh: function refresh() {
      var previewer = this,
          onSettingChange; // Display loading indicator.
      ...
      onSettingChange = function onSettingChange(setting) {
      ...
      previewer.loading.done(function (readyData) {
        ...
        _onceSynced = function onceSynced() {
          loadingFrame.unbind('synced', _onceSynced);
        ...
        loadingFrame.bind('synced', _onceSynced); // This event will be received directly by the previewer in normal navigation; this is only needed for seamless refresh.
    ...
    cheatin: function cheatin() {
      $(document.body).empty().addClass('cheatin').append('<h1>' + api.l10n.notAllowedHeading + '</h1>' + '<p>' + api.l10n.notAllowed + '</p>');
    },
  ...
  api._handleSettingValidities = function handleSettingValidities(args) {
    var invalidSettingControls,
        invalidSettings = [],
        wasFocused = false; // Find the controls that correspond to each invalid setting.
      ...
      if (setting) {
        ...
        if (_.isObject(validity)) {
          _.each(validity, function (params, code) {
            ...
            notification = new api.Notification(code, _.extend({
              fromServer: true
            }, params)); // Remove existing notification if already exists for code but differs in parameters.
    ...
    if (args.focusInvalidControl) {
      invalidSettingControls = api.findControlsForSettings(invalidSettings); // Focus on the first control that is inside of an expanded section (one that is visible).
  ...
  }, api); // Define state values.

  api.state = new api.Values();

  _.each(['saved', 'saving', 'trashing', 'activated', 'processing', 'paneVisible', 'expandedPanel', 'expandedSection', 'changesetDate', 'selectedChangesetDate', 'changesetStatus', 'selectedChangesetStatus', 'remainingTimeToPublish', 'previewerAlive', 'editShortcutVisibility', 'changesetLocked', 'previewedDevice'], function (name) {
    api.state.create(name);
  });

  $(function () {
    api.settings = window._wpCustomizeSettings;
    api.l10n = window._wpCustomizeControlsL10n; // Check if we can run the Customizer.
    ...
    var parent,
        ...
        footerActions = $('#customize-footer-actions'); // Add publish settings section in JS instead of PHP since the Customizer depends on it to function.
    ...
    api.section('publish_settings', function (section) {
      ...
      isSectionActive = function isSectionActive() {
      ...
      updateSectionActive = function updateSectionActive() {
      ...
      updateSectionActive(); // Bind visibility of the publish settings button to whether the section is active.
      ...
      publishWhenTime = function publishWhenTime() {
        api.state('selectedChangesetStatus').set('publish');
        api.previewer.save();
      }; // Start countdown for when the dateTime arrives, or clear interval when it is .


      updateTimeArrivedPoller = function updateTimeArrivedPoller() {
        var shouldPoll = 'future' === api.state('changesetStatus').get() && 'future' === api.state('selectedChangesetStatus').get() && api.state('changesetDate').get() && api.state('selectedChangesetDate').get() === api.state('changesetDate').get() && api.utils.getRemainingTime(api.state('changesetDate').get()) >= 0;
      ...
      updateTimeArrivedPoller(); // Ensure dateControl only appears when selected status is future.

      dateControl.active.validate = function () {
        return 'future' === api.state('selectedChangesetStatus').get();
      };

      toggleDateControl = function toggleDateControl(value) {
        dateControl.active.set('future' === value);
      };

      toggleDateControl(api.state('selectedChangesetStatus').get());
      api.state('selectedChangesetStatus').bind(toggleDateControl); // Show notification on date control when status is future but it isn't a future date.
    ...
    /** @lends wp.customize.previewer */
    {
      ...
      save: function save(args) {
        ...
        submit = function submit() {
          ...
          request.fail(function (response) {
            ...
            api.trigger('error', response); // Start a new changeset if the underlying changeset was published.
          ...
          request.done(function (response) {
            ...
            api.trigger('saved', response); // Restore the global dirty state if any settings were modified during save.
        ...
        if (0 === processing()) {
          ...
          _submitWhenDoneProcessing = function submitWhenDoneProcessing() {
            if (0 === processing()) {
              api.state.unbind('change', _submitWhenDoneProcessing);
              submit();
            }
          };

          api.state.bind('change', _submitWhenDoneProcessing);
        }
      ...
      trash: function trash() {
        ...
        success = function success() {
          ...
          api.state('saved').set(true); // Go back to Customizer without changeset.
    ...
    $.each(api.settings.panels, function (id, data) {
      var Constructor = api.panelConstructor[data.type] || api.Panel,
          options; // Inclusion of params alias is for back-compat for custom panels that expect to augment this property.
    ...
    $.each(api.settings.sections, function (id, data) {
      var Constructor = api.sectionConstructor[data.type] || api.Section,
          options; // Inclusion of params alias is for back-compat for custom sections that expect to augment this property.
    ...
    $.each(api.settings.controls, function (id, data) {
      var Constructor = api.controlConstructor[data.type] || api.Control,
          options; // Inclusion of params alias is for back-compat for custom controls that expect to augment this property.
    ...
    (function (state) {
      ...
      defaultSelectedChangesetStatus = api.settings.changeset.currentUserCanPublish ? 'publish' : 'draft'; // Set default states.
      ...
      populateChangesetUuidParam = function populateChangesetUuidParam(isIncluded) {
        var urlParser, queryParams; // Abort on IE9 which doesn't support history management.
    ...
    (function checkAndDisplayLockNotice() {
      ...
      /** @lends wp.customize~LockedNotification.prototype */
      {
        ...
        render: function render() {
          ...
          li = api.OverlayNotification.prototype.render.call(data); // Try to autosave the changeset now.
    ...
    (function () {
      ...
      function addAutosaveRestoreNotification() {
        var code = 'autosave_available',
            _onStateChange; // Since there is an autosave revision and the user hasn't loaded with autosaved, add notification to prompt to load autosaved version.
        ...
        api.notifications.add(new api.Notification(code, {
          ...
          render: function render() {
            var li = api.Notification.prototype.render.call(this),
                link; // Handle clicking on restoration link.
        ...
        _onStateChange = function onStateChange() {
          dismissAutosave();
          api.notifications.remove(code);
          api.unbind('change', _onStateChange);
          api.state('changesetStatus').unbind(_onStateChange);
        };

        api.bind('change', _onStateChange);
        api.state('changesetStatus').bind(_onStateChange);
      }
    ...
    (function initStickyHeaders() {
      ...
      changeContainer = function changeContainer(container) {
        ...
        if (activeHeader && activeHeader.element) {
          ...
          releaseStickyHeader(activeHeader.element); // Remove event listener in the previous panel or section.
      ...
      api.state('expandedPanel').bind(changeContainer); // Throttled scroll event handler.
      ...
      }, 8)); // Update header position on sidebar layout change.
      ...
      releaseStickyHeader = function releaseStickyHeader(headerElement) {
        if (!headerElement.hasClass('is-sticky')) {
          return;
        }

        headerElement.removeClass('is-sticky').addClass('maybe-sticky is-in-view').css('top', parentContainer.scrollTop() + 'px');
      }; // Reset position of the sticky header.


      resetStickyHeader = function resetStickyHeader(headerElement, headerParent) {
      ...
      positionStickyHeader = function positionStickyHeader(header, scrollTop, scrollDirection) {
        var headerElement = header.element,
            ...
            isScrollingUp = -1 === scrollDirection; // When scrolling down, gradually hide sticky header.
    ...
    api.previewedDevice = api.state('previewedDevice'); // Set the default device.
    ...
    api.previewedDevice.bind(function (newDevice) {
      ...
      footerActions.find('.devices .preview-' + newDevice).addClass('active').attr('aria-pressed', true);
    ...
    (function () {
      ...
      function startPromptingBeforeUnload() {
        ...
        api.state('selectedChangesetDate').unbind(startPromptingBeforeUnload); // Prompt user with AYS dialog if leaving the Customizer with unsaved changes.
      ...
      function requestClose() {
        ...
        if (isCleanState()) {
          ...
          dismissLock = true; // Mark all settings as clean to prevent another call to requestChangesetUpdate.
    ...
    parent.send('ready'); // Control visibility for default controls.
    ...
    api.control('background_preset', function (control) {
      ...
      defaultValues = [_wpCustomizeBackground.defaults['default-position-x'], _wpCustomizeBackground.defaults['default-position-y'], _wpCustomizeBackground.defaults['default-size'], _wpCustomizeBackground.defaults['default-repeat'], _wpCustomizeBackground.defaults['default-attachment']];
      values = {
      ...
      toggleVisibility = function toggleVisibility(preset) {
      ...
      updateSettings = function updateSettings(preset) {
    ...
    (function () {
      ...
      sectionReady.done(function setupSectionDescription(section) {
        var control = api.control('custom_css'); // Hide redundant label for visual users.

        control.container.find('.customize-control-title:first').addClass('screen-reader-text'); // Close the section description when clicking the close button.

        section.container.find('.section-description-buttons .section-description-close').on('click', function () {
          section.container.find('.section-meta .customize-section-description:first').removeClass('open').slideUp();
        ...
        if (control && !control.setting.get()) {
          section.container.find('.section-meta .customize-section-description:first').addClass('open').show().trigger('toggled');
    ...
    api.control('header_video', function (headerVideoControl) {
      headerVideoControl.deferred.embedded.done(function () {
        var toggleNotice = function toggleNotice() {
    ...
    function startAutosaving() {
      ...
      updateChangesetWithReschedule = function updateChangesetWithReschedule() {
        if (!updatePending) {
          updatePending = true;
          api.requestChangesetUpdate({}, {
            autosave: true
      ...
      scheduleChangesetUpdate = function scheduleChangesetUpdate() {
        clearTimeout(timeoutId);
      ...
      scheduleChangesetUpdate(); // Save changeset when focus removed from window.
    ...
    api.bind('change', startAutosaving); // Make sure TinyMCE dialogs appear above Customizer UI.
