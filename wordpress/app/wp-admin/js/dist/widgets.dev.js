  ...
  window.wpWidgets = {
    ...
    init: function init() {
      var rem,
          ...
          isRTL = !!('undefined' !== typeof isRtl && isRtl); // Handle the widgets containers in the right column.
      ...
      .on('click', function () {
        ...
        if ($wrap.hasClass('closed')) {
          ...
          $toggle.attr('aria-expanded', 'true'); // Refresh the jQuery UI sortable items.
      ...
      $('#widgets-left .sidebar-name').on('click', function () {
        ...
        $wrap.toggleClass('closed').find('.handlediv').attr('aria-expanded', !$wrap.hasClass('closed')); // Update the admin menu "sticky" state.
      ...
      $(document.body).on('click.widgets-toggle', function (e) {
        ...
        if (target.parents('.widget-top').length && !target.parents('#available-widgets').length) {
          ...
          widgetId = inside.find('.widget-id').val(); // Save button is initially disabled, but is enabled when a field is changed.
      ...
      $('#widget-list').children('.widget').draggable({
        ...
        start: function start(event, ui) {
          ...
          if (chooser.length) {
            ...
            $('#wpbody-content').append(chooser.hide()); // Delete the cloned chooser from the drag helper.
      ...
      sidebars.sortable({
        ...
        stop: function stop(event, ui) {
          var addNew,
              ...
              item,
              $widget = ui.item,
              id = the_id; // Reset the var to hold a previously closed sidebar.
          ...
          if (addNew) {
            if ('multi' === addNew) {
              $widget.html($widget.html().replace(/<[^<>]+>/g, function (tag) {
          ...
          if ($sidebar.parent().hasClass('closed')) {
            $sidebar.parent().removeClass('closed').find('.handlediv').attr('aria-expanded', 'true');
            $children = $sidebar.children('.widget'); // Make sure the dropped widget is at the top.
        ...
        receive: function receive(event, ui) {
          var $sender = $(ui.sender); // Don't add more widgets to orphaned sidebars.
      ...
      $('#available-widgets').droppable({
        ...
        over: function over(e, ui) {
          ...
          if (ui.draggable.hasClass('ui-sortable-helper')) {
            $('#removing-widget').show().children('span').html(ui.draggable.find('div.widget-title').children('h3').html());
          }
        },
      ...
      $('#available-widgets .widget .widget-top').on('click.widgets-chooser', function () {
        ...
        if ($widget.hasClass('widget-in-question') || $('#widgets-left').hasClass('chooser')) {
          ...
          // Open the chooser.
          self.clearWidgetSelection();
          $('#widgets-left').addClass('chooser'); // Add CSS class and insert the chooser after the widget description.

          $widget.addClass('widget-in-question').children('.widget-description').after(chooser); // Open the chooser with a slide down animation.
    ...
    saveOrder: function saveOrder(sidebarId) {
      var data = {
      ...
      $.post(ajaxurl, data, function (r) {
        ...
        if (del) {
          ...
          if (r && r.length > 2) {
            ...
            wpWidgets.appendTitle(widget); // Re-disable the save button.

            widget.find('.widget-control-save').prop('disabled', true).val(wp.i18n.__('Saved'));
            widget.removeClass('widget-dirty'); // Clear the dirty flag from the widget.
    ...
    appendTitle: function appendTitle(widget) {
    ...
    close: function close(widget) {
      widget.children('.widget-inside').slideUp('fast', function () {
        widget.attr('style', '').find('.widget-top button.widget-action').attr('aria-expanded', 'false').focus();
      });
    },
    addWidget: function addWidget(chooser) {
      var widget,
          widgetId,
          add,
          n,
      ...
      n = widget.find('input.multi_number').val(); // Remove the cloned chooser from the widget.

      widget.find('.widgets-chooser').remove();

      if ('multi' === add) {
        widget.html(widget.html().replace(/<[^<>]+>/g, function (m) {
      ...
      wpWidgets.save(widget, 0, 0, 1); // No longer "new" widget.
      ...
      window.setTimeout(function () {
        ...
        widget.find('.widget-title').trigger('click'); // At the end of the animation, announce the widget has been added.
    ...
    closeChooser: function closeChooser() {
      ...
      $('.widgets-chooser').slideUp(200, function () {
        ...
        self.clearWidgetSelection(); // Move focus back to the toggle button.
