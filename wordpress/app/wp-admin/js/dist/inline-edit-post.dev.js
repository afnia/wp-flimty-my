  ...
  window.inlineEditPost = {
    ...
    init: function init() {
      var t = this,
          qeRow = $('#inline-edit'),
          bulkRow = $('#bulk-edit');
      t.type = $('table.widefat').hasClass('pages') ? 'page' : 'post'; // Post ID prefix.
    ...
    setBulk: function setBulk() {
      var te = '',
          ...
          c = true;
      this.revert();
      $('#bulk-edit td').attr('colspan', $('th:visible, td:visible', '.widefat:first thead').length); // Insert the editor at the top of the table with an empty row above to maintain zebra striping.
      ...
      $('tbody th.check-column input[type="checkbox"]').each(function () {
        ...
        if ($(this).prop('checked')) {
          ...
          wp.i18n.__('Remove &#8220;%s&#8221; from Bulk Edit'), theTitle);
      ...
      $('#bulk-titles .ntdelbutton').click(function () {
        ...
        $('#_' + id).parent().remove();
        wp.a11y.speak(wp.i18n.__('Item removed.'), 'assertive'); // Move focus to a proper place when items are removed.
      ...
      $('#bulk-edit .inline-edit-wrapper').attr('tabindex', '-1').focus(); // Scrolls to the top of the table where the editor is rendered.
    ...
    edit: function edit(id) {
      ...
      $('td', editRow).attr('colspan', $('th:visible, td:visible', '.widefat:first thead').length); // Remove the ID from the copied row and let the `for` attribute reference the hidden ID.

      $('td', editRow).find('#quick-edit-legend').removeAttr('id');
      $('td', editRow).find('p[id^="quick-edit-"]').removeAttr('id');
      $(t.what + id).removeClass('is-expanded').hide().after(editRow).after('<tr class="hidden"></tr>'); // Populate fields in the quick edit window.
      ...
      $('.tags_input', rowData).each(function () {
        var terms = $(this),
            ...
            comma = wp.i18n._x(',', 'tag delimiter').trim(); // Ensure the textarea exists.
    ...
    save: function save(id) {
      ...
      params = fields + '&' + $.param(params); // Make Ajax request.
      ...
      }, 'html'); // Prevent submitting the form when pressing Enter on a focused field.
    ...
    revert: function revert() {
      ...
      if (id) {
        ...
        if ('bulk-edit' === id) {
          ...
          $('#bulk-titles').empty(); // Store the empty bulk editor in a hidden element.

          $('#inlineedit').append($('#bulk-edit')); // Move focus back to the Bulk Action button that was activated.

          $('#' + inlineEditPost.whichBulkButtonId).trigger('focus');
        } else {
          // Remove both the inline-editor and its hidden tr siblings.
          $('#' + id).siblings('tr.hidden').addBack().remove();
          id = id.substr(id.lastIndexOf('-') + 1); // Show the post row and move focus back to the Quick Edit button.

          $(this.what + id).show().find('.editinline').attr('aria-expanded', 'false').trigger('focus');
        }
      }
