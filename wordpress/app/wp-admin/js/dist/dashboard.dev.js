...
jQuery(function ($) {
  ...
  updateWelcomePanel = function updateWelcomePanel(visible) {
    $.post(ajaxurl, {
  ...
  window.ajaxPopulateWidgets = function (el) {
    ...
    function show(i, id) {
      var p,
          e = $('#' + id + ' div.inside:visible').find('.widget-loading'); // If the element is found in the dom, queue to load latest representation.
    ...
    if (el) {
      el = el.toString(); // If the element is available as Ajax widget, show it.
  ...
  ajaxPopulateWidgets(); // Register ajax widgets as postbox toggles.
  ...
  window.quickPressLoad = function () {
    var act = $('#quickpost-action'),
        t; // Enable the submit buttons.

    $('#quick-press .submit input[type="submit"], #quick-press .submit input[type="reset"]').prop('disabled', false);
    t = $('#quick-press').on('submit', function (e) {
      e.preventDefault(); // Show a spinner.

      $('#dashboard_quick_press #publishing-action .spinner').show(); // Disable the submit button to prevent duplicate submissions.

      $('#quick-press .submit input[type="submit"], #quick-press .submit input[type="reset"]').prop('disabled', true); // Post the entered data to save it.
      ...
      $.post(t.attr('action'), t.serializeArray(), function (data) {
        ...
        quickPressLoad();
        highlightLatestPost(); // Focus the title to allow for quickly drafting another post.
  ...
  window.quickPressLoad(); // Enable the dragging functionality of the widgets.
  ...
  function autoResizeTextarea() {
    ...
    editor.on('focus input propertychange', function () {
      ...
      cloneHeight = clone.css('width', $this.css('width')).text(textareaContent).outerHeight() + 2; // Default to show a vertical scrollbar, if needed.

      editor.css('overflow-y', 'auto'); // Only change the height if it has changed and both heights are below the max.
...
jQuery(function ($) {
  ...
  /** @lends wp.communityEvents */
  {
     ...
     * @return {void}
     */
    getEvents: function getEvents(requestParams) {
      var initiatedBy,
          app = this,
      ...
      wp.ajax.post('get-community-events', requestParams).always(function () {
     ...
     * @return {void}
     */
    renderEventsTemplate: function renderEventsTemplate(templateParams, initiatedBy) {
      var template,
          elementVisibility,
          $toggleButton = $('.community-events-toggle-location'),
          $locationMessage = $('#community-events-location-message'),
          $results = $('.community-events-results');
      templateParams.events = app.populateDynamicEventFields(templateParams.events, communityEventsData.time_format);
      /*
      ...
      if (templateParams.location.ip) {
        ...
        if ('user' === initiatedBy) {
          ...
          __('City updated. Listing events near %s.'), templateParams.location.description), 'assertive');
        }
     ...
     * @returns {Array}
     */
    populateDynamicEventFields: function populateDynamicEventFields(rawEvents, timeFormat) {
      ...
      $.each(populatedEvents, function (index, event) {
        ...
        event.user_formatted_date = app.getFormattedDate(event.start_unix_timestamp * 1000, event.end_unix_timestamp * 1000, timeZone);
     ...
     */
    getTimeZone: function getTimeZone(startTimestamp) {
      /*
     ...
     * @returns {number}
     */
    getFlippedTimeZoneOffset: function getFlippedTimeZoneOffset(startTimestamp) {
     ...
     * @returns {string}
     */
    getTimeZoneAbbreviation: function getTimeZoneAbbreviation(startTimestamp) {
      ...
      var shortTimeStringParts = eventDateTime.toLocaleTimeString(undefined, {
        timeZoneName: 'short'
      }).split(' ');
      ...
      if ('undefined' === typeof timeZoneAbbreviation) {
        ...
        var timeZoneOffset = app.getFlippedTimeZoneOffset(startTimestamp),
            sign = -1 === Math.sign(timeZoneOffset) ? '' : '+'; // translators: Used as part of a string like `GMT+5` in the Events Widget.
     ...
     * @returns {string}
     */
    getFormattedDate: function getFormattedDate(startDate, endDate, timeZone) {
      var formattedDate;
      /*
