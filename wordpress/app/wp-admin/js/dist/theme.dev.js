...
(function ($) {
  ...
  var themes, l10n;
  themes = wp.themes = wp.themes || {}; // Store the theme data and settings for organized and quick access.
  // themes.data.settings, themes.data.themes, themes.data.l10n.

  themes.data = _wpThemeSettings;
  l10n = themes.data.l10n; // Shortcut for isInstall check.

  themes.isInstall = !!themes.data.settings.isInstall; // Setup app structure.
  ...
  themes.view.Appearance = wp.Backbone.View.extend({
    ...
    initialize: function initialize(options) {
      ...
      this.SearchView = options.SearchView ? options.SearchView : themes.view.Search; // Bind to the scroll event and throttle
    ...
    render: function render() {
      ...
      this.search();
      this.$el.removeClass('search-loading'); // Render and append.
    ...
    search: function search() {
      var view,
          self = this; // Don't render the search if there is only one theme.
      ...
      self.SearchView = view; // Render and append after screen title.

      view.render();
      this.searchContainer.append($.parseHTML('<label class="screen-reader-text" for="wp-filter-search-input">' + l10n.search + '</label>')).append(view.el).on('submit', function (event) {
  ...
  themes.Collection = Backbone.Collection.extend({
    ...
    doSearch: function doSearch(value) {
      ...
      this.terms = value; // If we have terms, run a search...
    ...
    search: function search(term) {
      var match, results, haystack, name, description, author; // Start with a full collection.

      this.reset(themes.data.themes, {
        silent: true
      }); // Trim the term.

      term = term.trim(); // Escape the term string for RegExp meta characters.

      term = term.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&'); // Consider spaces as word delimiters and match the whole string
      // so matching terms can be combined.

      term = term.replace(/ /g, ')(?=.*');
      match = new RegExp('^(?=.*' + term + ').+', 'i'); // Find results.
      // _.filter() and .test().
    ...
    paginate: function paginate(instance) {
      var collection = this;
      instance = instance || 0; // Themes per instance are set at 20.
    ...
    query: function query(request) {
      ...
      var queries = this.queries,
          ...
          query,
          isPaginated,
          count; // Store current query request args
      // for later use with the event `theme:end`.

      this.currentQuery.request = request; // Search the query cache for matches.
      ...
      isPaginated = _.has(request, 'page'); // Reset the internal api page counter for non-paginated queries.
      ...
      if (!query && !isPaginated) {
        query = this.apiCall(request).done(function (data) {
          ...
          if (data.themes) {
            ...
            count = data.info.results; // Store the results and the query request.
        ...
        if (isPaginated) {
          return this.apiCall(request, isPaginated).done(function (data) {
            ...
            self.trigger('query:success'); // We are done loading themes for now.
    ...
    apiCall: function apiCall(request, paginated) {
  ...
  themes.view.Theme = wp.Backbone.View.extend({
    ...
    render: function render() {
      var data = this.model.toJSON(); // Render themes using the html template.

      this.$el.html(this.html(data)).attr('data-slug', data.id); // Renders active theme styles.
    ...
    expand: function expand(event) {
      var self = this;
      event = event || window.event; // 'Enter' and 'Space' keys expand the details view when a theme is :focused.
    ...
    preview: function preview(event) {
      var self = this,
          current,
          preview;
      event = event || window.event; // Bail if the user scrolled on a touch device.
      ...
      event.preventDefault();
      event = event || window.event; // Set focus to current theme.

      themes.focusedTheme = this.$el; // Construct a new Preview view.
      ...
      preview.render();
      this.setNavButtonsState(); // Hide previous/next navigation if there is only one theme.
      ...
      $('div.wrap').append(preview.el); // Listen to our preview object
      // for `theme:next` and `theme:previous` events.

      this.listenTo(preview, 'theme:next', function () {
        // Keep local track of current theme model.
        current = self.model; // If we have ventured away from current model update the current model position.
        ...
        self.current = self.model.collection.at(self.model.collection.indexOf(current) + 1); // If we have no more themes, bail.
        ...
        preview.model = self.current; // Render and append.
        ...
        current = self.model; // Bail early if we are at the beginning of the collection.
    ...
    setNavButtonsState: function setNavButtonsState() {
      var $themeInstaller = $('.theme-install-overlay'),
          ...
          nextThemeButton = $themeInstaller.find('.next-theme'); // Disable previous at the zero position.
  ...
  themes.view.Details = wp.Backbone.View.extend({
    ...
    render: function render() {
      ...
      this.$el.html(this.html(data)); // Renders active theme styles.

      this.activeTheme(); // Set up navigation events.

      this.navigation(); // Checks screenshot size.

      this.screenshotCheck(this.$el); // Contain "tabbing" inside the overlay.
    ...
    containFocus: function containFocus($el) {
      ...
      }, 100); // Constrain tabbing within the modal.


      $el.on('keydown.wp-themes', function (event) {
        var $firstFocusable = $el.find('.theme-header button:not(.disabled)').first(),
            $lastFocusable = $el.find('.theme-actions a:visible').last(); // Check for the Tab key.
    ...
    collapse: function collapse(event) {
      var self = this,
          scroll;
      event = event || window.event; // Prevent collapsing detailed view when there is only one theme available.
      ...
      if ($(event.target).is('.theme-backdrop') || $(event.target).is('.close') || event.keyCode === 27) {
        ...
        $('body').addClass('closing-overlay'); // With a quick fade out animation.

        this.$el.fadeOut(130, function () {
          // Clicking outside the modal box closes the overlay.
          $('body').removeClass('closing-overlay'); // Handle event cleanup.

          self.closeOverlay(); // Get scroll position to avoid jumping to the top.

          scroll = document.body.scrollTop; // Clean the URL structure.

          themes.router.navigate(themes.router.baseUrl('')); // Restore scroll position.

          document.body.scrollTop = scroll; // Return focus to the theme div.
    ...
    autoupdateState: function autoupdateState() {
      var _callback,
          _this = this; // Support concurrent clicks in different Theme Details overlays.


      _callback = function callback(event, data) {
        var autoupdate;
        ...
        if (_this.model.get('id') === data.asset) {
          ...
          $(document).off('wp-auto-update-setting-changed', _callback);
        }
      }; // Triggered in updates.js


      $(document).on('wp-auto-update-setting-changed', _callback);
    },
    updateTheme: function updateTheme(event) {
      var _this = this;
    ...
    deleteTheme: function deleteTheme(event) {
      ...
      event.preventDefault(); // Confirmation dialog for deleting a theme.
      ...
      $(document).one('wp-theme-delete-success', function (event, response) {
        ...
        $('[data-slug="' + response.slug + '"]').css({
          ...
          $(this).remove();
          _themes.data.themes = _.without(_themes.data.themes, _.findWhere(_themes.data.themes, {
            id: response.slug
          }));
    ...
    screenshotCheck: function screenshotCheck(el) {
      var screenshot, image;
      screenshot = el.find('.screenshot img');
      image = new Image();
      image.src = screenshot.attr('src'); // Width check.
  ...
  themes.view.Preview = themes.view.Details.extend({
    ...
    close: function close() {
      this.$el.fadeOut(200, function () {
        $('body').removeClass('theme-installer-active full-overlay-active'); // Return focus to the theme div.

        if (themes.focusedTheme) {
          themes.focusedTheme.find('.more-details').trigger('focus');
        }
      }).removeClass('iframe-ready'); // Restore the previous browse tab if available.
    ...
    previewDevice: function previewDevice(event) {
      var device = $(event.currentTarget).data('device');
      this.$el.removeClass('preview-desktop preview-tablet preview-mobile').addClass('preview-' + device).data('current-preview-device', device);
      this.tooglePreviewDeviceButtons(device);
    },
    tooglePreviewDeviceButtons: function tooglePreviewDeviceButtons(newDevice) {
      var $devices = $('.wp-full-overlay-footer .devices');
      $devices.find('button').removeClass('active').attr('aria-pressed', false);
      $devices.find('button.preview-' + newDevice).addClass('active').attr('aria-pressed', true);
    },
  ...
  themes.view.Themes = wp.Backbone.View.extend({
    ...
    liveThemeCount: 0,
    initialize: function initialize(options) {
      var self = this; // Set up parent.

      this.parent = options.parent; // Set current view to [grid].

      this.setView('grid'); // Move the active theme to the beginning of the collection.

      self.currentTheme(); // When the collection is updated by user input...
    ...
    render: function render() {
      ...
      this.$el.empty(); // If the user doesn't have switch capabilities or there is only one theme
    ...
    renderThemes: function renderThemes(page) {
      var self = this;
      self.instance = self.collection.paginate(page); // If we have no more themes, bail.
      ...
      self.instance.each(function (theme) {
        ...
        self.theme.render(); // ...and append them to div.themes.

        self.$el.append(self.theme.el); // Binds to theme:expand to show the modal box
    ...
    currentTheme: function currentTheme() {
      var self = this,
          current;
      current = self.collection.findWhere({
        active: true
      }); // Move the active theme to the beginning of the collection.

      if (current) {
        self.collection.remove(current);
        self.collection.add(current, {
          at: 0
        });
      }
    },
    ...
    expand: function expand(id) {
      var self = this,
          $card,
          $modal; // Set the current theme model.

      this.model = self.collection.get(id); // Trigger a route update for the current model.

      themes.router.navigate(themes.router.baseUrl(themes.router.themePath + this.model.id)); // Sets this.view to 'detail'.

      this.setView('detail');
      $('body').addClass('modal-open'); // Set up the theme details view.
      ...
      if (this.model.get('hasUpdate')) {
        ...
        if ($card.find('.updating-message').length) {
          ...
          $modal.find('.notice-warning').removeClass('notice-large').addClass('updating-message').find('p').text(wp.updates.l10n.updating);
        } else if ($card.find('.notice-error').length) {
          $modal.find('.notice-warning').remove();
        }
      }

      this.$overlay.html(this.overlay.el); // Bind to theme:next and theme:previous triggered by the arrow keys.
    ...
    next: function next(args) {
      var self = this,
          model,
          nextModel; // Get the current theme.

      model = self.collection.get(args[0]); // Find the next model within the collection.

      nextModel = self.collection.at(self.collection.indexOf(model) + 1); // Sanity check which also serves as a boundary test.

      if (nextModel !== undefined) {
        // We have a new theme...
        // Close the overlay.
        this.overlay.closeOverlay(); // Trigger a route update for the current model.
    ...
    previous: function previous(args) {
      var self = this,
          model,
          previousModel; // Get the current theme.

      model = self.collection.get(args[0]); // Find the previous model within the collection.
    ...
    announceSearchResults: function announceSearchResults(count) {
      if (0 === count) {
  ...
  themes.view.Search = wp.Backbone.View.extend({
    ...
    doSearch: function doSearch(event) {
      var options = {};
      this.collection.doSearch(event.target.value.replace(/\+/g, ' ')); // if search is initiated and key is not return.
  ...
  themes.Run = {
    init: function init() {
      ...
      this.themes = new themes.Collection(themes.data.themes); // Set up the view.

      this.view = new themes.view.Appearance({
        collection: this.themes
      });
      this.render(); // Start debouncing user searches after Backbone.history.start().
    ...
    routes: function routes() {
      var self = this; // Bind to our global thx object
      // so that the object is available to sub-views.

      themes.router = new themes.Router(); // Handles theme details route event.
  ...
  themes.view.InstallerSearch = themes.view.Search.extend({
    ...
    search: function search(event) {
      ...
      this.collection = this.options.parent.view.collection; // Clear on escape.
    ...
    doSearch: function doSearch(value) {
      var request = {}; // Don't do anything if the search terms haven't changed.
      ...
      $('.drawer-toggle').attr('aria-expanded', 'false'); // Get the themes by sending Ajax POST request to api.wordpress.org/themes
      // or searching the local cache.

      this.collection.query(request); // Set route.

      themes.router.navigate(themes.router.baseUrl(themes.router.searchPath + encodeURIComponent(value)), {
        replace: true
      });
    }
  });
  themes.view.Installer = themes.view.Appearance.extend({
    ...
    render: function render() {
      ...
      this.search();
      this.uploader();
      this.collection = new themes.Collection(); // Bump `collection.currentQuery.page` and request more themes if we hit the end of the page.
      ...
      this.listenTo(this, 'theme:end', function () {
        ...
        self.collection.currentQuery.page++; // Use currentQuery.page to build the themes request.
    ...
    browse: function browse(section) {
    ...
    onSort: function onSort(event) {
      ...
      $('.drawer-toggle').attr('aria-expanded', 'false'); // Bail if this is already active.

      if ($el.hasClass(this.activeClass)) {
        return;
      }

      this.sort(sort); // Trigger a router.navigate update.

      themes.router.navigate(themes.router.baseUrl(themes.router.browsePath + sort));
    },
    sort: function sort(_sort) {
      this.clearSearch(); // Track sorting so we can restore the correct tab when closing preview.

      themes.router.selectedTab = _sort;
      $('.filter-links li > a, .theme-filter').removeClass(this.activeClass).removeAttr('aria-current');
      $('[data-sort="' + _sort + '"]').addClass(this.activeClass).attr('aria-current', 'page');
    ...
    onFilter: function onFilter(event) {
      var request,
          $el = $(event.target),
          filter = $el.data('filter'); // Bail if this is already active.

      if ($el.hasClass(this.activeClass)) {
        return;
      }

      $('.filter-links li > a, .theme-section').removeClass(this.activeClass).removeAttr('aria-current');
    ...
    saveUsername: function saveUsername(event) {
    ...
    uploader: function uploader() {
      ...
      uploadViewToggle.on('click', function () {
        ...
        $body.toggleClass('show-upload-view'); // Toggle the `aria-expanded` button attribute.

        uploadViewToggle.attr('aria-expanded', $body.hasClass('show-upload-view'));
      });
    },
    // Toggle the full filters navigation.
    moreFilters: function moreFilters(event) {
      var $body = $('body'),
      ...
      this.clearSearch();
      themes.router.navigate(themes.router.baseUrl('')); // Toggle the feature filters view.

      $body.toggleClass('show-filters'); // Toggle the `aria-expanded` button attribute.
    ...
    backToFilters: function backToFilters(event) {
      if (event) {
        event.preventDefault();
      }
  ...
  themes.RunInstaller = {
    ...
    routes: function routes() {
      var self = this,
          request = {}; // Bind to our global `wp.themes` object
      // so that the router is available to sub-views.

      themes.router = new themes.InstallerRouter(); // Handles `theme` route event.
      ...
      themes.router.on('route:preview', function (slug) {
        ...
        if (self.view.view.theme && self.view.view.theme.preview) {
          ...
          self.view.collection.trigger('update'); // Open the theme preview.
      ...
      themes.router.on('route:sort', function (sort) {
        ...
        self.view.sort(sort); // Close the preview if open.
  ...
  $(function () {
    ...
    $(document.body).on('click', '.load-customize', function () {
      ...
      urlParser.search = $.param(_.extend(wp.customize.utils.parseQueryString(urlParser.search.substr(1)), {
...
})(jQuery); // Align theme browser thickbox.
