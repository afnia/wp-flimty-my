...
(function ($, wp, settings) {
  ...
  wp.updates.addAdminNotice = function (data) {
    ...
    $adminNotice = wp.updates.adminNotice(data); // Check if this admin notice already exists.
  ...
  wp.updates.refreshCount = function () {
    ...
    $adminBarUpdates.find('.updates-available-text').text(sprintf(
    /* translators: %s: Total number of updates available. */
    _n('%s update available', '%s updates available', settings.totals.counts.total), settings.totals.counts.total)); // Remove the update count from the toolbar if it's zero.
  ...
  wp.updates.updatePlugin = function (args) {
    ...
    if ('plugins' === pagenow || 'plugins-network' === pagenow) {
      ...
      _x('Updating %s...', 'plugin'), $updateRow.find('.plugin-title strong').text());
    } else if ('plugin-install' === pagenow || 'plugin-install-network' === pagenow) {
      $card = $('.plugin-card-' + args.slug);
      $message = $card.find('.update-now').addClass('updating-message');
      message = sprintf(
      /* translators: %s: Plugin name and version. */
      _x('Updating %s...', 'plugin'), $message.data('name')); // Remove previous error messages, if any.
  ...
  wp.updates.updatePluginSuccess = function (response) {
    ...
    if ('plugins' === pagenow || 'plugins-network' === pagenow) {
      $pluginRow = $('tr[data-plugin="' + response.plugin + '"]').removeClass('update is-enqueued').addClass('updated');
      $updateMessage = $pluginRow.find('.update-message').removeClass('updating-message notice-warning').addClass('updated-message notice-success').find('p'); // Update the version number in the row.

      newText = $pluginRow.find('.plugin-version-author-uri').html().replace(response.oldVersion, response.newVersion);
      $pluginRow.find('.plugin-version-author-uri').html(newText); // Clear the "time to next auto-update" text.
    ...
    $updateMessage.attr('aria-label', sprintf(
    /* translators: %s: Plugin name and version. */
    _x('%s updated!', 'plugin'), response.pluginName)).text(_x('Updated!', 'plugin'));
  ...
  wp.updates.updatePluginError = function (response) {
    ...
    __('Update failed: %s'), response.errorMessage);
    ...
    if ('plugins' === pagenow || 'plugins-network' === pagenow) {
      ...
      if (response.pluginName) {
        $message.find('p').attr('aria-label', sprintf(
        /* translators: %s: Plugin name and version. */
        _x('%s update failed.', 'plugin'), response.pluginName));
      } else {
        $message.find('p').removeAttr('aria-label');
      }
    } else if ('plugin-install' === pagenow || 'plugin-install-network' === pagenow) {
      $card = $('.plugin-card-' + response.slug).addClass('plugin-card-update-failed').append(wp.updates.adminNotice({
        className: 'update-message notice-error notice-alt is-dismissible',
        message: errorMessage
      }));
      $card.find('.update-now').text(__('Update failed.')).removeClass('updating-message');

      if (response.pluginName) {
        $card.find('.update-now').attr('aria-label', sprintf(
        /* translators: %s: Plugin name and version. */
        _x('%s update failed.', 'plugin'), response.pluginName));
      } else {
  ...
  wp.updates.installPlugin = function (args) {
    ...
    $message.addClass('updating-message').attr('aria-label', sprintf(
    /* translators: %s: Plugin name and version. */
    _x('Installing %s...', 'plugin'), $message.data('name'))).text(__('Installing...'));
    wp.a11y.speak(__('Installing... please wait.')); // Remove previous error messages, if any.
  ...
  wp.updates.installPluginSuccess = function (response) {
    ...
    $message.removeClass('updating-message').addClass('updated-message installed button-disabled').attr('aria-label', sprintf(
    /* translators: %s: Plugin name and version. */
    _x('%s installed!', 'plugin'), response.pluginName)).text(_x('Installed!', 'plugin'));
    ...
    if (response.activateUrl) {
      setTimeout(function () {
        ...
        $message.removeClass('install-now installed button-disabled updated-message').addClass('activate-now button-primary').attr('href', response.activateUrl);

        if ('plugins-network' === pagenow) {
          $message.attr('aria-label', sprintf(
          /* translators: %s: Plugin name. */
          _x('Network Activate %s', 'plugin'), response.pluginName)).text(__('Network Activate'));
        } else {
          $message.attr('aria-label', sprintf(
          /* translators: %s: Plugin name. */
          _x('Activate %s', 'plugin'), response.pluginName)).text(__('Activate'));
        }
      }, 1000);
    }
  };
  ...
  wp.updates.installPluginError = function (response) {
    ...
    __('Installation failed: %s'), response.errorMessage);
    $card.addClass('plugin-card-update-failed').append('<div class="notice notice-error notice-alt is-dismissible"><p>' + errorMessage + '</p></div>');
    $card.on('click', '.notice.is-dismissible .notice-dismiss', function () {
      // Use same delay as the total duration of the notice fadeTo + slideUp animation.
      setTimeout(function () {
        $card.removeClass('plugin-card-update-failed').find('.column-name a').trigger('focus');
    ...
    $button.removeClass('updating-message').addClass('button-disabled').attr('aria-label', sprintf(
    /* translators: %s: Plugin name and version. */
    _x('%s installation failed', 'plugin'), $button.data('name'))).text(__('Installation failed.'));
    wp.a11y.speak(errorMessage, 'assertive');
  ...
  wp.updates.installImporterSuccess = function (response) {
    wp.updates.addAdminNotice({
      ...
      __('Importer installed successfully. <a href="%s">Run importer</a>'), response.activateUrl + '&from=import')
    });
    $('[data-slug="' + response.slug + '"]').removeClass('install-now updating-message').addClass('activate-now').attr({
      'href': response.activateUrl + '&from=import',
      'aria-label': sprintf(
      /* translators: %s: Importer name. */
      __('Run %s'), response.pluginName)
  ...
  wp.updates.installImporterError = function (response) {
    ...
    $installLink.removeClass('updating-message').attr('aria-label', sprintf(
    /* translators: %s: Plugin name. */
    _x('Install %s now', 'plugin'), pluginName)).text(__('Install Now'));
    wp.a11y.speak(errorMessage, 'assertive');
  ...
  wp.updates.deletePluginSuccess = function (response) {
    ...
    $('[data-plugin="' + response.plugin + '"]').css({
      ...
      plugins = settings.plugins,
          remainingCount; // Add a success message after deleting a plugin.

      if (!$pluginRow.hasClass('plugin-update-tr')) {
        $pluginRow.after(pluginDeletedRow({
          slug: response.slug,
      ...
      $pluginRow.remove(); // Remove plugin from update count.
      ...
      if ($itemsCount.length && $currentView.length) {
        ...
        $itemsCount.text(sprintf(
        /* translators: %s: The remaining number of plugins. */
        _nx('%s item', '%s items', 'plugin/plugins', remainingCount), remainingCount));
      }
    });
  ...
  wp.updates.deletePluginError = function (response) {
    ...
    if (!$pluginUpdateRow.length) {
      $plugin.addClass('update').after(pluginUpdateRow({
        slug: response.slug,
  ...
  wp.updates.updateTheme = function (args) {
    ...
    if ('themes-network' === pagenow) {
      ...
      $notice.find('h3').remove(); // Add the top-level UI, and update both.
  ...
  wp.updates.updateThemeSuccess = function (response) {
    ...
    if ('customize' === pagenow) {
      ...
      $notice = $theme.find('.update-message'); // Update the version number in the row.

      newText = $theme.find('.theme-version-author-uri').html().replace(response.oldVersion, response.newVersion);
      $theme.find('.theme-version-author-uri').html(newText); // Clear the "time to next auto-update" text.

      $theme.find('.auto-update-time').empty();
    } else {
      $notice = $('.theme-info .notice').add($theme.find('.update-message')); // Focus on Customize button after updating.
    ...
    wp.updates.addAdminNotice(_.extend({
      selector: $notice
    }, updatedMessage));
    wp.a11y.speak(__('Update completed successfully.'));
    wp.updates.decrementCount('theme');
    $document.trigger('wp-theme-update-success', response); // Show updated message after modal re-rendered.
  ...
  wp.updates.installTheme = function (args) {
    ...
    _x('Installing %s...', 'theme'), $message.data('name'))).text(__('Installing...'));
    wp.a11y.speak(__('Installing... please wait.')); // Remove previous error messages, if any.
  ...
  wp.updates.installThemeSuccess = function (response) {
    ...
    $message = $card.find('.button-primary').removeClass('updating-message').addClass('updated-message disabled').attr('aria-label', sprintf(
    /* translators: %s: Theme name and version. */
    _x('%s installed!', 'theme'), response.themeName)).text(_x('Installed!', 'theme'));
    wp.a11y.speak(__('Installation completed successfully.'));
    setTimeout(function () {
      if (response.activateUrl) {
        // Transform the 'Install' button into an 'Activate' button.
        $message.attr('href', response.activateUrl).removeClass('theme-install updated-message disabled').addClass('activate');

        if ('themes-network' === pagenow) {
          $message.attr('aria-label', sprintf(
          /* translators: %s: Theme name. */
          _x('Network Activate %s', 'theme'), response.themeName)).text(__('Network Enable'));
        } else {
          $message.attr('aria-label', sprintf(
          /* translators: %s: Theme name. */
          _x('Activate %s', 'theme'), response.themeName)).text(__('Activate'));
        }
      }
      ...
      if (response.customizeUrl) {
        ...
        $message.siblings('.preview').replaceWith(function () {
          return $('<a>').attr('href', response.customizeUrl).addClass('button load-customize').text(__('Live Preview'));
        });
      }
    }, 1000);
  };
  ...
  wp.updates.installThemeError = function (response) {
    ...
    $button.removeClass('updating-message').attr('aria-label', sprintf(
    /* translators: %s: Theme name and version. */
    _x('%s installation failed', 'theme'), $button.data('name'))).text(__('Installation failed.'));
    wp.a11y.speak(errorMessage, 'assertive');
  ...
  wp.updates.deleteTheme = function (args) {
    ...
    wp.a11y.speak(__('Deleting...')); // Remove previous error messages, if any.
  ...
  wp.updates.deleteThemeSuccess = function (response) {
    ...
    if ('themes-network' === pagenow) {
      ...
      $themeRows.css({
        ...
        $themeRow.remove(); // Remove theme from update count.
        ...
        themes.all = _.without(themes.all, response.slug); // There is always at least one theme available.
  ...
  wp.updates.deleteThemeError = function (response) {
    ...
    __('Deletion failed: %s'), response.errorMessage),
        $message = wp.updates.adminNotice({
      className: 'update-message notice-error notice-alt',
      message: errorMessage
    });
    ...
    if ('themes-network' === pagenow) {
      if (!$updateRow.length) {
        $themeRow.addClass('update').after(updateRow({
          slug: response.slug,
          colspan: $('#bulk-action-form').find('thead th:not(.hidden), thead td').length,
          content: $message
        }));
  ...
  wp.updates.queueChecker = function () {
    ...
    job = wp.updates.queue.shift(); // Handle a queue job.
  ...
  wp.updates.showErrorInCredentialsForm = function (message) {
    var $filesystemForm = $('#request-filesystem-credentials-form'); // Remove any existing error.
  ...
  wp.updates.isValidResponse = function (response, action) {
    var error = __('Something went wrong.'),
        errorMessage; // Make sure the response is a valid data object and not a Promise object.
    ...
    errorMessage = errorMessage.replace('%s', error); // Add admin notice.
    ...
    wp.updates.ajaxLocked = false;
    wp.updates.queue = []; // Change buttons of all running updates.

    $('.button.updating-message').removeClass('updating-message').removeAttr('aria-label').prop('disabled', true).text(__('Update failed.'));
    $('.updating-message:not(.button):not(.thickbox)').removeClass('updating-message notice-warning').addClass('notice-error').find('p').removeAttr('aria-label').text(errorMessage);
    wp.a11y.speak(errorMessage, 'assertive');
    return false;
  };
  /**
  ...
  $(function () {
    ...
    $filesystemModal.on('submit', 'form', function (event) {
      event.preventDefault(); // Persist the credentials input by the user for the duration of the page load.
      ...
      wp.updates.filesystemCredentials.available = true; // Unlock and invoke the queue.
      ...
      if ($message && $message.hasClass('updating-message')) {
        ...
        if ('plugin-install' === pagenow || 'plugin-install-network' === pagenow) {
          if ('update-plugin' === job.action) {
            $message.attr('aria-label', sprintf(
            /* translators: %s: Plugin name and version. */
            _x('Update %s now', 'plugin'), $message.data('name')));
          } else if ('install-plugin' === job.action) {
            $message.attr('aria-label', sprintf(
            /* translators: %s: Plugin name. */
            _x('Install %s now', 'plugin'), $message.data('name')));
          }
        }
      }
    ...
    $bulkActionForm.on('click', '[data-plugin] .update-link', function (event) {
      ...
      wp.updates.maybeRequestFilesystemCredentials(event); // Return the user to the input box of the plugin's table row after closing the modal.
    ...
    $bulkActionForm.on('click', '[data-plugin] a.delete', function (event) {
      ...
      if ($pluginRow.hasClass('is-uninstallable')) {
        ...
        /* translators: %s: Plugin name. */
        __('Are you sure you want to delete %s and its data?'), $pluginRow.find('.plugin-title strong').text());
      } else {
        confirmMessage = sprintf(
        /* translators: %s: Plugin name. */
        __('Are you sure you want to delete %s?'), $pluginRow.find('.plugin-title strong').text());
      }
    ...
    $document.on('click', '.themes-php.network-admin .update-link', function (event) {
      ...
      wp.updates.maybeRequestFilesystemCredentials(event); // Return the user to the input box of the theme's table row after closing the modal.
    ...
    $document.on('click', '.themes-php.network-admin a.delete', function (event) {
      var $themeRow = $(event.target).parents('tr'),
          confirmMessage = sprintf(
      /* translators: %s: Theme name. */
      __('Are you sure you want to delete %s?'), $themeRow.find('.theme-title strong').text());
      event.preventDefault();
    ...
    $bulkActionForm.on('click', '[type="submit"]:not([name="clear-recent-list"])', function (event) {
      var bulkAction = $(event.target).siblings('select').val(),
          ...
          errorMessages = [],
          type,
          action; // Determine which type of item we're dealing with.
      ...
      switch (bulkAction) {
        ...
        case 'delete-selected':
          var confirmMessage = 'plugin' === type ? __('Are you sure you want to delete the selected plugins and their data?') : __('Caution: These themes may be active on other sites in the network. Are you sure you want to proceed?');
      ...
      wp.updates.maybeRequestFilesystemCredentials(event);
      event.preventDefault(); // Un-check the bulk checkboxes.

      $bulkActionForm.find('.manage-column [type="checkbox"]').prop('checked', false);
      $document.trigger('wp-' + type + '-bulk-' + bulkAction, itemsSelected); // Find all the checkboxes which have been checked.

      itemsSelected.each(function (index, element) {
        var $checkbox = $(element),
            $itemRow = $checkbox.parents('tr'); // Only add update-able items to the update queue.
        ...
        $itemRow.addClass('is-enqueued'); // Add it to the queue.
      ...
      $document.on('wp-plugin-update-success wp-plugin-update-error wp-theme-update-success wp-theme-update-error', function (event, response) {
        ...
        $bulkActionNotice = $('#bulk-action-notice').on('click', 'button', function () {
          ...
          $(this).toggleClass('bulk-action-errors-collapsed').attr('aria-expanded', !$(this).hasClass('bulk-action-errors-collapsed')); // Show the errors list.
    ...
    $pluginInstallSearch.on('keyup input', _.debounce(function (event, eventtype) {
      ...
      searchLocation = location.href.split('?')[0] + '?' + $.param(_.omit(data, ['_ajax_nonce', 'pagenow'])); // Clear on escape.
      ...
      if (!$searchTab.length) {
        $searchTab = $('<li class="plugin-install-search" />').append($('<a />', {
          'class': 'current',
          'href': searchLocation,
          'text': __('Search Results')
        }));
        $('.wp-filter .filter-links .current').removeClass('current').parents('.filter-links').prepend($searchTab);
      ...
      wp.updates.searchRequest = wp.ajax.post('search-install-plugins', data).done(function (response) {
        ...
        if (0 === response.count) {
          ...
          __('Number of plugins found: %d'), response.count));
        }
      });
    }, 1000));
    ...
    $pluginSearch.on('keyup input', _.debounce(function (event) {
      ...
      wp.updates.searchRequest = wp.ajax.post('search-plugins', data).done(function (response) {
        ...
        var $subTitle = $('<span />').addClass('subtitle').html(sprintf(
        /* translators: %s: Search query. */
        __('Search results for: %s'), '<strong>' + _.escape(decodeURIComponent(data.s)) + '</strong>')),
    ...
    $document.on('click keyup', '.column-auto-updates .toggle-auto-update, .theme-overlay .toggle-auto-update', function (event) {
      ...
      event.preventDefault(); // Prevent multiple simultaneous requests.
      ...
      $parent.find('.notice.notice-error').addClass('hidden'); // Show loading status.
      ...
      $.post(window.ajaxurl, data).done(function (response) {
        ...
        $parent.find('.notice.notice-error').removeClass('hidden').find('p').text(__('The request could not be completed.'));
