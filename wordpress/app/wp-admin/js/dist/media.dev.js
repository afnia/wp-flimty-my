...
(function ($) {
  window.findPosts = {
     ...
     */
    open: function open(af_name, af_val) {
      ...
      $('#find-posts').show(); // Close the dialog when the escape key is pressed.
  ...
  $(function () {
    var settings,
        ...
        copyAttachmentURLSuccessTimeout; // Opens a manage media frame into the grid.
    ...
    if ($mediaGridWrap.length && window.wp && window.wp.media) {
      ...
      }).open(); // Fire a global ready event.
    ...
    $('#find-posts-search').on('click', findPosts.send); // Binds the close dialog click event.

    $('#find-posts-close').on('click', findPosts.close); // Binds the bulk action events to the submit buttons.
    ...
    copyAttachmentURLClipboard.on('success', function (event) {
      ...
      successElement.removeClass('hidden'); // Hide success visual feedback after 3 seconds since last success and unfocus the trigger.
