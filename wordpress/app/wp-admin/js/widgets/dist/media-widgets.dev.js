...
wp.mediaWidgets = function ($) {
      ...
      if ('image' === this.controller.options.mimeType) {
        ...
        /** @lends wp.mediaWidgets~Constructor.prototype */
        {
          ...
          updateoEmbed: function updateoEmbed() {
            ...
            url = embedLinkView.model.get('url'); // Abort if the URL field was emptied out.
          ...
          fetch: function fetch() {
            ...
            if (youTubeEmbedMatch) {
              url = 'https://www.youtube.com/watch?v=' + youTubeEmbedMatch[1]; // silently change url to proper oembed-able version.
  ...
  /** @lends wp.mediaWidgets.MediaWidgetControl.prototype */
  {
    ...
    initialize: function initialize(options) {
      ...
      control.$el.addClass('media-widget-control'); // Allow methods to be passed in with control context preserved.
      ...
      control.previewTemplateProps = new Backbone.Model(control.mapModelToPreviewTemplateProps()); // Re-render the preview when the attachment changes.

      control.selectedAttachment = new wp.media.model.Attachment();
      control.renderPreview = _.debounce(control.renderPreview);
      control.listenTo(control.previewTemplateProps, 'change', control.renderPreview); // Make sure a copy of the selected attachment is always fetched.
      ...
      control.listenTo(control.model, 'change', control.render); // Update the title.
      ...
      control.displaySettings = new Backbone.Model(_.pick(control.mapModelToMediaFrameProps(_.extend(control.model.defaults(), control.model.toJSON())), _.keys(wp.media.view.settings.defaultProps)));
    },
    ...
    updateSelectedAttachment: function updateSelectedAttachment() {
      ...
      if (0 === control.model.get('attachment_id')) {
        ...
        attachment.fetch().done(function done() {
          control.model.set('error', false);
    ...
    selectMedia: function selectMedia() {
      ...
      mediaFrame.on('insert', function onInsert() {
        var attachment = {},
            state = mediaFrame.state(); // Update cached attachment object to avoid having to re-fetch. This also triggers re-rendering of preview.
        ...
        control.model.set('error', false); // Update widget instance.
      ...
      mediaFrame.open(); // Clear the selected attachment when it is deleted in the media select frame.
    ...
    getModelPropsFromMediaFrame: function getModelPropsFromMediaFrame(mediaFrame) {
      ...
      if ('insert' === state.get('id')) {
        ...
        if (control.showDisplaySettings) {
          _.extend(mediaFrameProps, mediaFrame.content.get('.attachments-browser').sidebar.get('display').model.toJSON());
        }
        ...
        mediaFrameProps = _.extend(state.props.toJSON(), {
          attachment_id: 0
        }, // Because some media frames use `attachment_id` not `id`.
      ...
      modelProps = control.mapMediaToModelProps(mediaFrameProps); // Clear the extension prop so sources will be reset for video and audio media.
      ...
      _.each(attrs, function (value, name) {
        ...
        if ('array' === type) {
          ...
          if (model.schema[name].items && 'integer' === model.schema[name].items.type) {
            castedAttrs[name] = _.filter(_.map(castedAttrs[name], function (id) {
  ...
  component.handleWidgetAdded = function handleWidgetAdded(event, widgetContainer) {
    var fieldContainer,
        ...
        _renderWhenAnimationDone;

    widgetForm = widgetContainer.find('> .widget-inside > .form, > .widget-inside > form'); // Note: '.form' appears in the customizer, whereas 'form' on the widgets admin screen.

    idBase = widgetForm.find('> .id_base').val();
    widgetId = widgetForm.find('> .widget-id').val(); // Prevent initializing already-added widgets.
    ...
    _renderWhenAnimationDone = function renderWhenAnimationDone() {
      if (!widgetContainer.hasClass('open')) {
        setTimeout(_renderWhenAnimationDone, animatedCheckDelay);
      } else {
