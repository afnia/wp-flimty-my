...
(function ($) {
  ...
  /** @lends $.widget.wp.pointer.prototype */
  {
    options: {
      ...
      position: 'top',
      show: function show(event, t) {
        t.pointer.show();
        t.opened();
      },
      hide: function hide(event, t) {
        t.pointer.hide();
        t.closed();
      },
      document: document
    ...
    _create: function _create() {
      ...
      }).length) positioning = 'fixed';
      this.pointer = $('<div />').append(this.content).append(this.arrow).attr('id', 'wp-pointer-' + identifier++).addClass(this.options.pointerClass).css({
    ...
    _setOption: function _setOption(key, value) {
      var o = this.options,
          tip = this.pointer; // Handle document transfer.

      if (key === 'document' && value !== o.document) {
        tip.detach().appendTo(value.body); // Handle class change.
      } else if (key === 'pointerClass') {
        tip.removeClass(o.pointerClass).addClass(value);
      } // Call super method.


      $.Widget.prototype._setOption.apply(this, arguments); // Reposition automatically.


      if (key === 'position') {
        this.reposition(); // Update content automatically if pointer is open.
    ...
    update: function update(event) {
      ...
      if (typeof o.content === 'string') {
        content = o.content; // ...or o.content is a callback.
      } else {
     ...
     */
    _update: function _update(event, content) {
      var buttons,
          o = this.options;
      if (!content) return; // Kill any animations on the pointer.
    ...
    reposition: function reposition() {
      ...
      position = this._processPosition(this.options.position); // Reposition pointer.
    ...
    repoint: function repoint() {
      var o = this.options,
          edge;
      if (o.disabled) return;
      edge = typeof o.position == 'string' ? o.position : o.position.edge; // Remove arrow classes.

      this.pointer[0].className = this.pointer[0].className.replace(/wp-pointer-[^\s'"]*/, ''); // Add arrow class.
     ...
     */
    _processPosition: function _processPosition(position) {
      var opposite = {
        top: 'bottom',
        bottom: 'top',
        left: 'right',
        right: 'left'
          ...
          result; // If the position object is a string, it is shorthand for position.edge.
    ...
    toggle: function toggle(event) {
      if (this.pointer.is(':hidden')) this.open(event);else this.close(event);
    },
     ...
     */
    _handoff: function _handoff(extend) {
      return $.extend({
