...
(function () {
  ...
  var _domReady = function _domReady(func) {
    var t, i, _DOMContentLoaded2, _tryReady2;
    ...
    if (typeof jQuery !== 'undefined') {
      ...
      if (!t.eventAttached) {
        if (document.addEventListener) {
          ...
          _tryReady2 = function _tryReady() {
            try {
              document.documentElement.doScroll('left');
            } catch (e) {
              setTimeout(_tryReady2, 50);
              return;
            }
    ...
    zeroise = function zeroise(number) {
  ...
  var qt = window.QTags = function (settings) {
    ...
    t.toolbar = tb; // Listen for click events.

    onclick = function onclick(e) {
      e = e || window.event;
      var target = e.target || e.srcElement,
          visible = target.clientWidth || target.offsetWidth,
          i; // Don't call the callback on pressing the accesskey when the button is not visible.
    ...
    setActiveEditor = function setActiveEditor() {
  ...
  qt.Button.prototype.callback = function () {}; // A button that inserts HTML tag.
  ...
  qt.SpellButton = function () {}; // The close tags button.
