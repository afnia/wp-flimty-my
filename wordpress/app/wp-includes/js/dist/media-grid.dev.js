  ...
  var __webpack_modules__ = {
    ...
    function _(module) {
      ...
      /** @lends wp.media.view.MediaFrame.Manage.Router.prototype */
      {
        ...
        showItem: function showItem(query) {
          var media = wp.media,
              frame = media.frames.browse,
              library = frame.state().get('library'),
              item; // Trigger the media frame to open the correct item.
        ...
        editItem: function editItem(query) {
          this.showItem(query);
    ...
    function _(module) {
      ...
      /** @lends wp.media.view.DeleteSelectedPermanentlyButton.prototype */
      {
        ...
        filterChange: function filterChange(model) {
      ...
      /** @lends wp.media.view.SelectModeToggle.prototype */
      {
        ...
        toggleBulkEditHandler: function toggleBulkEditHandler() {
          var toolbar = this.controller.content.get().toolbar,
              children;
          children = toolbar.$('.media-toolbar-secondary > *, .media-toolbar-primary > *'); // @todo The Frame should be doing all of this.
      ...
      /** @lends wp.media.view.MediaFrame.EditAttachments.prototype */
      {
        ...
        createModal: function createModal() {
          ...
          if (this.options.modal) {
            ...
            }, this)); // Completely destroy the modal DOM element when closing it.

            this.modal.on('close', _.bind(function () {
              // Remove the keydown event.
              $('body').off('keydown.media-modal'); // Move focus back to the original item in the grid if possible.

              $('li.attachment[data-id="' + this.model.get('id') + '"]').trigger('focus');
              this.resetRoute();
            }, this)); // Set this frame as the modal's content.
         ...
         */
        editMetadataMode: function editMetadataMode(contentRegion) {
         ...
         *                               should be set with the proper region view.
         */
        editImageMode: function editImageMode(contentRegion) {
        ...
        editImageModeRender: function editImageModeRender(view) {
        ...
        rerender: function rerender(model) {
          ...
          this.model = model;
          this.bindModelHandlers(); // Only rerender the `content` region.
        ...
        previousMediaItem: function previousMediaItem() {
          ...
          this.trigger('refresh', this.library.at(this.getCurrentIndex() - 1)); // Move focus to the Previous button. When there are no more items, to the Next button.
        ...
        nextMediaItem: function nextMediaItem() {
          ...
          this.trigger('refresh', this.library.at(this.getCurrentIndex() + 1)); // Move focus to the Next button. When there are no more items, to the Previous button.
         ...
         */
        focusNavButton: function focusNavButton(which) {
          $(which).trigger('focus');
        },
      ...
      /** @lends wp.media.view.MediaFrame.Manage.prototype */
      {
        ...
        initialize: function initialize() {
          _.defaults(this.options, {
            ...
            selection: [],
            library: {},
            // Options hash for the query to the media library.
            multiple: 'add',
            state: 'library',
            uploader: true,
            mode: ['grid', 'edit']
          ...
          this.$window = $(window);
          this.$adminBar = $('#wpadminbar'); // Store the Add New button for later reuse in wp.media.view.UploaderInline.

          this.$uploaderToggler = $('.page-title-action').attr('aria-expanded', 'false').on('click', _.bind(this.addNewClickHandler, this));
          this.$window.on('scroll resize', _.debounce(_.bind(this.fixPosition, this), 15)); // Ensure core and media grid view UI is enabled.

          this.$el.addClass('wp-core-ui'); // Force the uploader off if the upload limit has been exceeded or
          ...
          this.gridRouter = new wp.media.view.MediaFrame.Manage.Router(); // Call 'initialize' directly on the parent class.

          MediaFrame.prototype.initialize.apply(this, arguments); // Append the frame view directly the supplied container.
        ...
        bindSearchHandler: function bindSearchHandler() {
          ...
          }, 1000); // Update the URL when entering search string (at most once per second).
        ...
        bindRegionModeHandlers: function bindRegionModeHandlers() {
          this.on('content:create:browse', this.browseContent, this); // Handle a frame-level event for editing an attachment.
        ...
        fixPosition: function fixPosition() {
          ...
          $toolbar = $browser.find('.media-toolbar'); // Offset doesn't appear to take top margin into account, hence +16.
         ...
         */
        addNewClickHandler: function addNewClickHandler(event) {
          event.preventDefault();
         ...
         */
        openEditAttachmentModal: function openEditAttachmentModal(model) {
