...
(function (exports, $) {
  var api = {},
      ctor,
      inherits,
      slice = Array.prototype.slice; // Shared empty constructor function to aid in prototype-chain creation.
  ...
  api.Class = function (applicator, argsArray, options) {
    ...
    if (this.instance) {
      _magic = function magic() {
        return _magic.instance.apply(_magic, arguments);
      };

      $.extend(_magic, this);
    }

    _magic.initialize.apply(_magic, args);
  ...
  /** @lends wp.customize.Value.prototype */
  {
     ...
     */
    initialize: function initialize(initial, options) {
  ...
  /** @lends wp.customize.Values.prototype */
  {
     ...
     */
    add: function add(item, itemObject) {
      var collection = this,
          id,
          instance;
      ...
      instance.parent = collection; // Propagate a 'change' event on an item up to the collection.

      if (instance.extended(api.Value)) {
        instance.bind(collection._change);
      }

      collection.trigger('add', instance); // If a deferred object exists for this item,
      // resolve it.
    ...
    remove: function remove(id) {
      ...
      delete this._deferreds[id]; // Trigger removed event after the item has been eliminated from the collection.
    ...
    when: function when() {
      var self = this,
          ...
          dfd = $.Deferred(); // If the last argument is a callback, bind it to .done().
  ...
  /** @lends wp.customize.Messenger.prototype */
  {
     ...
     */
    add: function add(key, initial, options) {
    ...
    initialize: function initialize(params, options) {
      ...
      this.add('origin', this.url()).link(this.url).setter(function (to) {
        ...
        urlParser.href = to; // Port stripping needed by IE since it adds to host but not to event.origin.

        return urlParser.protocol + '//' + urlParser.host.replace(/:(80|443)$/, '');
      }); // First add with no value.

      this.add('targetWindow', null); // This avoids SecurityErrors when setting a window object in x-origin iframe'd scenarios.
    ...
    receive: function receive(event) {
      ...
      message = JSON.parse(event.data); // Check required message properties.
