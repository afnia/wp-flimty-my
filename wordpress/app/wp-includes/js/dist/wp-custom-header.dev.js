...
(function (window, settings) {
  ...
  window.wp = window.wp || {}; // Fail gracefully in unsupported browsers.
      ...
      if (this.supportsVideo()) {
        for (var id in this.handlers) {
          ...
          if ('test' in handler && handler.test(settings)) {
            this.activeHandler = handler.initialize.call(handler, settings); // Dispatch custom event when the video is loaded.

            _trigger(document, 'wp-custom-header-video-loaded');
  ...
  BaseHandler.prototype = {
    ...
    initialize: function initialize(settings) {
      ...
      button.innerHTML = settings.l10n.play; // Toggle video playback when the button is clicked.
     ...
     */
    setVideo: function setVideo(node) {
  ...
  document.addEventListener('DOMContentLoaded', window.wp.customHeader.initialize.bind(window.wp.customHeader), false); // Selective refresh support in the Customizer.
