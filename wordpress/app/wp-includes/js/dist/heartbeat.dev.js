...
(function ($, window, undefined) {
  ...
  var Heartbeat = function Heartbeat() {
    ...
    function initialize() {
      ...
      if (_typeof(window.heartbeatSettings) === 'object') {
        options = window.heartbeatSettings; // The XHR URL can be passed as option when window.ajaxurl is not set.
      ...
      $(window).on('pagehide.wp-heartbeat', function () {
        ...
        suspend(); // Abort the last request if not completed.
      ...
      window.setInterval(checkUserActivity, 30000); // Start one tick after DOM ready.
    ...
    function connect() {
      var ajaxData, heartbeatData; // If the connection to the server is slower than the interval,
      ...
      settings.lastTick = time();
      heartbeatData = $.extend({}, settings.queue); // Clear the data queue. Anything added after this point will be sent on the next tick.
      ...
      ajaxData = {
        ...
        _nonce: _typeof(window.heartbeatSettings) === 'object' ? window.heartbeatSettings.nonce : '',
        action: 'heartbeat',
      ...
      settings.xhr = $.ajax({
        ...
        type: 'post',
        timeout: 30000,
        // Throw an error if not completed after 30 seconds.
        data: ajaxData,
        dataType: 'json'
      }).always(function () {
        ...
        if (response.heartbeat_nonce && _typeof(window.heartbeatSettings) === 'object') {
        ...
        if (response.rest_nonce && _typeof(window.wpApiSettings) === 'object') {
          window.wpApiSettings.nonce = response.rest_nonce; // This nonce is required for api-fetch through heartbeat.tick.
          // delete response.rest_nonce;
        }

        $document.trigger('heartbeat-tick', [response, textStatus, jqXHR]);
        wp.hooks.doAction('heartbeat.tick', response, textStatus, jqXHR); // Do this last. Can trigger the next XHR if connection time > 5 seconds and newInterval == 'fast'.
    ...
    function focused() {
      settings.userActivity = time(); // Resume if suspended.
    ...
    function checkUserActivity() {
      var lastActive = settings.userActivity ? time() - settings.userActivity : 0; // Throttle down when no mouse or keyboard activity for 5 minutes.
    ...
    initialize(); // Expose public methods.
