...
(function ($) {
  ...
  wpList = {
     ...
     */
    nonce: function nonce(element, settings) {
     ...
     */
    parseData: function parseData(element, type) {
      var data = [],
          wpListsData;
     ...
     */
    pre: function pre(list, settings, action) {
     ...
     */
    ajaxAdd: function ajaxAdd(element, settings) {
      var list = this,
          $element = $(element),
     ...
     */
    ajaxDel: function ajaxDel(element, settings) {
      var list = this,
          $element = $(element),
     ...
     */
    ajaxDim: function ajaxDim(element, settings) {
      var list = this,
          $element = $(element),
          ...
          dimColor,
          parsedResponse,
          returnedResponse; // Prevent hidden links from being clicked by hotkeys.
      ...
      settings.success = function (response) {
        ...
        if ('undefined' !== typeof parsedResponse.responses[0].supplemental.comment_link) {
          var $submittedOn = $element.find('.submitted-on'),
              $commentLink = $submittedOn.find('a'); // Comment is approved; link the date field.

          if ('' !== parsedResponse.responses[0].supplemental.comment_link) {
            $submittedOn.html($('<a></a>').text($submittedOn.text()).prop('href', parsedResponse.responses[0].supplemental.comment_link)); // Comment is not approved; unlink the date field.
     ...
     * @return {boolean} Whether the item was added.
     */
    add: function add(element, settings) {
      var $list = $(this),
          $element = $(element),
          old = false,
          position,
    ...
    clear: function clear(elementId) {
      var list = this,
          $element = $(elementId),
          type,
          tagName; // Bail if we're within the list.
     ...
     */
    process: function process(elementId) {
      var list = this,
    ...
    recolor: function recolor() {
      var list = this,
          evenOdd = [':even', ':odd'],
          items; // Bail if there is no alternate class name specified.
  ...
  $.fn.wpList = function (settings) {
    this.each(function (index, list) {
      list.wpList = {
        settings: $.extend({}, wpList.settings, {
