...
(function (window, wp, shortcode, $) {
  ...
  wp.mce.views = {
     ...
     */
    register: function register(type, extend) {
      views[type] = wp.mce.View.extend(_.extend(extend, {
        type: type
      }));
    },
     ...
     */
    setMarkers: function setMarkers(content, editor) {
      var pieces = [{
        content: content
      }],
          self = this,
      ...
      _.each(views, function (view, type) {
        ...
        _.each(current, function (piece) {
          var remaining = piece.content,
              result,
              text; // Ignore processed pieces, but retain their location.
          ...
          while (remaining && (result = view.prototype.match(remaining))) {
            ...
            text = instance.loader ? '.' : instance.text; // Add the processed piece for the match.
     ...
     */
    createInstance: function createInstance(type, text, options, force) {
     ...
     */
    getInstance: function getInstance(object) {
     ...
     * @param {boolean}        force  Recreate the instance. Optional.
     */
    update: function update(text, editor, node, force) {
     ...
     */
    edit: function edit(editor, node) {
      var instance = this.getInstance(node);
     ...
     */
    remove: function remove(editor, node) {
      var instance = this.getInstance(node);
  ...
  /** @lends wp.mce.View.prototype */
  {
     ...
     */
    render: function render(content, force) {
      if (content != null) {
        this.content = content;
      }

      content = this.getContent(); // If there's nothing to render an no loader needs to be shown, stop.
      ...
      force && this.unbind(); // Replace any left over markers.
     ...
     */
    getEditors: function getEditors(callback) {
     ...
     */
    getNodes: function getNodes(callback, rendered) {
      this.getEditors(function (editor) {
        var self = this;
        $(editor.getBody()).find('[data-wpview-text="' + self.encodedText + '"]').filter(function () {
          var data;
    ...
    getMarkers: function getMarkers(callback) {
      this.getEditors(function (editor) {
        var self = this;
        $(editor.getBody()).find('[data-wpview-marker="' + this.encodedText + '"]').each(function () {
    ...
    replaceMarkers: function replaceMarkers() {
      this.getMarkers(function (editor, node) {
        ...
        $viewNode = editor.$('<div class="wpview wpview-wrap" data-wpview-text="' + this.encodedText + '" data-wpview-type="' + this.type + '" contenteditable="false"></div>');
     ...
     */
    setContent: function setContent(content, callback, rendered) {
     ...
     * @param {boolean}  rendered Only set for (un)rendered nodes. Optional.
     */
    setIframes: function setIframes(head, body, callback, rendered) {
      var self = this;

      if (body.indexOf('[') !== -1 && body.indexOf(']') !== -1) {
        var shortcodesRegExp = new RegExp('\\[\\/?(?:' + window.mceViewL10n.shortcodes.join('|') + ')[^\\]]*?\\]', 'g'); // Escape tags inside shortcode previews.
      ...
      this.getNodes(function (editor, node) {
        ...
        tinymce.each(dom.$('link[rel="stylesheet"]', editorHead), function (link) {
          if (link.href && link.href.indexOf('skins/lightgray/content.min.css') === -1 && link.href.indexOf('skins/wordpress/wp-content.css') === -1) {
        ...
        iframeDoc.open();
        iframeDoc.write('<!DOCTYPE html>' + '<html>' + '<head>' + '<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />' + head + styles + '<style>' + 'html {' + 'background: transparent;' + 'padding: 0;' + 'margin: 0;' + '}' + 'body#wpview-iframe-sandbox {' + 'background: transparent;' + 'padding: 1px 0 !important;' + 'margin: -1px 0 0 !important;' + '}' + 'body#wpview-iframe-sandbox:before,' + 'body#wpview-iframe-sandbox:after {' + 'display: none;' + 'content: "";' + '}' + 'iframe {' + 'max-width: 100%;' + '}' + '</style>' + '</head>' + '<body id="wpview-iframe-sandbox" class="' + bodyClasses + '">' + body + '</body>' + '</html>');
        iframeDoc.close();
     ...
     */
    setLoader: function setLoader(dashicon) {
      this.setContent('<div class="loading-placeholder">' + '<div class="dashicons dashicons-' + (dashicon || 'admin-media') + '"></div>' + '<div class="wpview-loading"><ins></ins></div>' + '</div>');
    },
     ...
     */
    setError: function setError(message, dashicon) {
      this.setContent('<div class="wpview-error">' + '<div class="dashicons dashicons-' + (dashicon || 'no') + '"></div>' + '<p>' + message + '</p>' + '</div>');
    },
...
(function (window, views, media, $) {
  ...
  function verifyHTML(string) {
    ...
    return serializer.serialize(parser.parse(string, {
  ...
  base = {
    state: [],
    edit: function edit(text, update) {
      var type = this.type,
