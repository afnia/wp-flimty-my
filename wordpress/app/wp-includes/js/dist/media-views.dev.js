...
(function () {
  ...
  var __webpack_modules__ = {
    ...
    function (_2) {
      ...
      /** @lends wp.media.controller.CollectionAdd.prototype */
      {
        ...
        initialize: function initialize() {
          ...
          this.set('menu', collectionType); // If we haven't been provided a `library`, create a `Selection`.
    ...
    function _(module) {
      ...
      /** @lends wp.media.controller.CollectionEdit.prototype */
      {
        ...
        initialize: function initialize() {
          ...
          this.set('toolbar', collectionType + '-edit'); // If we haven't been provided a `library`, create a `Selection`.
         ...
         */
        activate: function activate() {
          var library = this.get('library'); // Limit the library to images only.

          library.props.set('type', this.get('type')); // Watch for uploaded attachments.
         ...
         */
        renderSettings: function renderSettings(attachmentsBrowserView) {
          var library = this.get('library'),
    ...
    function (_3) {
      ...
      /** @lends wp.media.controller.Cropper.prototype */
      {
         ...
         */
        doCrop: function doCrop(attachment) {
      ...
      /** @lends wp.media.controller.CustomizeImageCropper.prototype */
      {
        ...
        doCrop: function doCrop(attachment) {
          var cropDetails = attachment.get('cropDetails'),
              ...
              ratio = cropDetails.width / cropDetails.height; // Use crop measurements when flexible in both directions.

          if (control.params.flex_width && control.params.flex_height) {
            cropDetails.dst_width = cropDetails.width;
            cropDetails.dst_height = cropDetails.height; // Constrain flexible side based on image ratio and size of the fixed side.
          } else {
    ...
    function (_5) {
      ...
      /** @lends wp.media.controller.Embed.prototype */
      {
        ...
        initialize: function initialize(options) {
          ...
          this.props = new Backbone.Model(this.metadata || {
            url: ''
          });
         ...
         */
        scanImage: function scanImage(attributes) {
          var frame = this.frame,
              state = this,
              url = this.props.get('url'),
              image = new Image(),
              deferred = $.Deferred();
          attributes.scanners.push(deferred.promise()); // Try to load the image and find its width/height.
    ...
    function (_6) {
      ...
      /** @lends wp.media.controller.FeaturedImage.prototype */
      {
         ...
         */
        initialize: function initialize() {
          var library, comparator; // If we haven't been provided a `library`, create a `Selection`.
          ...
          comparator = library.comparator; // Overload the library's comparator to push items that are not in
    ...
    function _(module) {
      ...
      /** @lends wp.media.controller.GalleryEdit.prototype */
      {
        ...
        activate: function activate() {
          ...
          library.props.set('type', 'image'); // Watch for uploaded attachments.
    ...
    function (_9) {
      ...
      /** @lends wp.media.controller.Library.prototype */
      {
        ...
        deactivate: function deactivate() {
          ...
          this.frame.off('content:activate', this.saveContentMode, this); // Unbind all event handlers that use this state as the context
          // from the selection.
         ...
         */
        display: function display(attachment) {
         ...
         * @return {Object}
         */
        defaultDisplaySettings: function defaultDisplaySettings(attachment) {
         ...
         * @return {boolean}
         */
        isImageAttachment: function isImageAttachment(attachment) {
         ...
         * @return {boolean}
         */
        canEmbed: function canEmbed(attachment) {
          // If uploading, we know the filename but not the mime type.
          if (!attachment.get('uploading')) {
         ...
         * @param {wp.media.model.Attachment} attachment
         */
        uploading: function uploading(attachment) {
    ...
    function (_11) {
      ...
      /** @lends wp.media.controller.Region.prototype */
      {
         ...
         */
        set: function set(views, options) {
          if (options) {
            options.add = false;
          }
        ...
        trigger: function trigger(event) {
          ...
          base = this.id + ':' + event; // Trigger `{this.id}:{event}:{this._mode}` event on the frame.

          args[0] = base + ':' + this._mode;
          this.view.trigger.apply(this.view, args); // Trigger `{this.id}:{event}` event on the frame.
    ...
    function (_12) {
      ...
      /** @lends wp.media.controller.ReplaceImage.prototype */
      {
        ...
        initialize: function initialize(options) {
          ...
          this.image = options.image; // If we haven't been provided a `library`, create a `Selection`.
    ...
    function (_13) {
      ...
      /** @lends wp.media.controller.StateMachine.prototype */
      {
        ...
        state: function state(id) {
          this.states = this.states || new Backbone.Collection(); // Default to the active state.
      ...
      _.each(['on', 'off', 'trigger'], function (method) {
        ...
        StateMachine.prototype[method] = function () {
          // Ensure that the `states` collection exists so the `StateMachine`
          // can be used as a mixin.
          this.states = this.states || new Backbone.Collection(); // Forward the method to the `states` collection.
    ...
    function (_14) {
      ...
      /** @lends wp.media.controller.State.prototype */
      {
        ...
        */
         ...
         * @access private
         */
        _renderTitle: function _renderTitle(view) {
         ...
         */
        _renderMenu: function _renderMenu(view) {
    ...
    function (_17) {
      ...
      /** @lends wp.media.view.AttachmentFilters.prototype */
      {
        ...
        initialize: function initialize() {
          ...
          _.extend(this.filters, this.options.filters); // Build `<option>` elements.
    ...
    function (_20) {
      ...
      /** @lends wp.media.view.Attachment.prototype */
      {
        ...
        initialize: function initialize() {
          ...
          this.listenTo(this.model, 'change:album', this._syncAlbum); // Update the selection.

          this.listenTo(this.model, 'add', this.select);
          this.listenTo(this.model, 'remove', this.deselect);

          if (selection) {
            selection.on('reset', this.updateSelect, this); // Update the model's details view.
         ...
         */
        dispose: function dispose() {
          var selection = this.options.selection; // Make sure all settings are saved before removing the view.
        ...
        render: function render() {
          ...
          this.updateSelect(); // Update the save status.
        ...
        toggleSelectionHandler: function toggleSelectionHandler(event) {
          var method; // Don't do anything inside inputs and on the attachment check and remove buttons.
          ...
          event.preventDefault(); // In the grid view, bubble up an edit:attachment event to the controller.
         ...
         * @param {Object} options
         */
        toggleSelection: function toggleSelection(options) {
          ...
          method = _.isUndefined(method) ? selection.multiple : method; // If the `method` is set to `between`, select all models that
          ...
          if ('between' === method && single && selection.multiple) {
            ...
            return; // If the `method` is set to `toggle`, just flip the selection
         ...
         */
        select: function select(model, collection) {
          var selection = this.options.selection,
          ...
          this.$el.addClass('selected').attr('aria-checked', true); //  Make the checkbox tabable, except in media grid (bulk select mode).
         ...
         * @param {Backbone.Collection} collection
         */
        deselect: function deselect(model, collection) {
          var selection = this.options.selection;
          /*
           * Check if a selection exists and if it's the collection provided.
           * If they're not the same collection, bail; we're in another
         ...
         * @param {Backbone.Collection} collection
         */
        details: function details(model, collection) {
          var selection = this.options.selection,
              details;
        ...
        imageSize: function imageSize(size) {
          var sizes = this.model.get('sizes'),
              matched = false;
          size = size || 'medium'; // Use the provided image size if possible.
         ...
         * @param {Object} event
         */
        updateSetting: function updateSetting(event) {
        ...
        save: function save() {
          var view = this,
            ...
            status: 'ready'
              ...
              requests = save.requests ? $.when(request, save.requests) : request; // If we're waiting to remove 'Saved.', stop.
         ...
         * @return {wp.media.view.Attachment} Returns itself to allow chaining.
         */
        updateSave: function updateSave(status) {
        ...
        updateAll: function updateAll() {
          ...
          changed = _.chain($settings).map(function (el) {
            ...
            value = $input.val(); // Record the value if it changed.
         ...
         * @param {Object} event
         */
        removeFromLibrary: function removeFromLibrary(event) {
          // Catch enter and space events.
          if ('keydown' === event.type && 13 !== event.keyCode && 32 !== event.keyCode) {
            return;
         ...
         */
        checkClickHandler: function checkClickHandler(event) {
          var selection = this.options.selection;
          ...
          if (selection.where({
            ...
            selection.remove(this.model); // Move focus back to the attachment tile (from the check).
    ...
    function (_21) {
      ...
      /** @lends wp.media.view.Attachment.Details.prototype */
      {
        ...
        copyAttachmentDetailsURLClipboard: function copyAttachmentDetailsURLClipboard() {
          ...
          clipboard.on('success', function (event) {
            var triggerElement = $(event.trigger),
                successElement = $('.success', triggerElement.closest('.copy-to-clipboard-container')); // Clear the selection and move focus back to the trigger.

            event.clearSelection(); // Handle ClipboardJS focus bug, see https://github.com/zenorocha/clipboard.js/issues/680

            triggerElement.trigger('focus'); // Show success visual feedback.

            clearTimeout(successTimeout);
            successElement.removeClass('hidden'); // Hide success visual feedback after 3 seconds since last success.

            successTimeout = setTimeout(function () {
              successElement.addClass('hidden');
            }, 3000); // Handle success audible feedback.
         ...
         * @return {void}
         */
        deleteAttachment: function deleteAttachment(event) {
          event.preventDefault();
         ...
         * @return {void}
         */
        trashAttachment: function trashAttachment(event) {
          var library = this.controller.library,
              self = this;
          event.preventDefault();
          this.getFocusableElements(); // When in the Media Library and the Media Trash is enabled.

          if (wp.media.view.settings.mediaTrash && 'edit-metadata' === this.controller.content.mode()) {
         ...
         * @return {void}
         */
        untrashAttachment: function untrashAttachment(event) {
          var library = this.controller.library;
          event.preventDefault();
    ...
    function (_22) {
      ...
      /** @lends wp.media.view.Attachments.prototype */
      {
         ...
         * @return {void}
         */
        arrowEvent: function arrowEvent(event) {
          ...
          this.$el.sortable(_.extend({
            ...
            update: function update(event, ui) {
              var model = collection.at(ui.item.data('sortableIndexStart')),
                  comparator = collection.comparator; // Temporarily disable the comparator to prevent `add`
              // from re-sorting.

              delete collection.comparator; // Silently shift the model to its new index.
              ...
              collection.comparator = comparator; // Fire the `reset` event to ensure other collections sync.

              collection.trigger('reset', collection); // If the collection is sorted by menu order, update the menu order.
         ...
         */
        createAttachmentView: function createAttachmentView(attachment) {
        ...
        scroll: function scroll() {
          ...
          toolbar = this.views.parent.toolbar; // Show the spinner only if we are close to the bottom.
    ...
    function (_23) {
      ...
      /** @lends wp.media.view.AttachmentsBrowser.prototype */
      {
        ...
        initialize: function initialize() {
          ...
          this.controller.on('edit:selection', this.editSelection); // In the Media Library, the sidebar is used to display errors before the attachments grid.
          ...
          this.createAttachmentsHeading(); // Create the attachments wrapper view.
        ...
        editSelection: function editSelection(modal) {
          ...
          */
          ...
          if (this.controller.isModeActive('grid')) {
            ...
            this.toolbar.set('libraryViewSwitcher', new LibraryViewSwitcher({
              controller: this.controller,
              priority: -90
            }).render()); // DateFilter is a <select>, a visually hidden label element needs to be rendered before.
            ...
            this.toolbar.set('dateFilter', new wp.media.view.DateFilter({
              ...
              model: this.collection.props,
              priority: -75
            }).render()); // BulkSelection is a <div> with subviews, including screen reader text.
            ...
            this.toolbar.set('deleteSelectedButton', new wp.media.view.DeleteSelectedButton({
              ...
              click: function click() {
                ...
                if (mediaTrash && 'trash' !== selection.at(0).get('status') && !window.confirm(l10n.warnBulkTrash)) {
                  return;
                }
        ...
        updateLoadMoreView: _.debounce(function () {
          ...
          if (this.collection.length) {
            ...
            sprintf(__('Showing %1$s of %2$s media items'), this.collection.length, this.collection.getTotalAttachments()));
          ...
          this.firstAddedMediaItem = this.$el.find('.attachment').eq(this.firstAddedMediaItemIndex); // If there's a media item to move focus to, make the "Jump to" button available.
        ...
        disposeSingle: function disposeSingle() {
          ...
          sidebar.unset('display'); // Hide the sidebar on mobile.
    ...
    function (_26) {
      ...
      /** @lends wp.media.view.Button.prototype */
      {
        ...
        initialize: function initialize() {
           ...
           */
          this.model = new Backbone.Model(this.defaults); // If any of the `options` have a key from `defaults`, apply its
        ...
        onImageLoad: function onImageLoad() {
          ...
          imgOptions = _.extend(imgOptions, {
            ...
            onInit: function onInit() {
              // Store the set ratio.
              var setRatio = imgSelect.getOptions().aspectRatio; // On mousedown, if no ratio is set and the Shift key is down, use a 1:1 ratio.
    ...
    function (_29) {
      ...
      /** @lends wp.media.view.EmbedLink.prototype */
      {
        ...
        updateoEmbed: _.debounce(function () {
          var url = this.model.get('url'); // Clear out previous results.

          this.$('.embed-container').hide().find('.embed-preview').empty();
          this.$('.setting').hide(); // Only proceed with embed if the field contains more than 11 characters.
        ...
        fetch: function fetch() {
          var url = this.model.get('url'),
              re,
              youTubeEmbedMatch; // Check if they haven't typed in 500 ms.
        ...
        renderFail: function renderFail(response, status) {
          if ('abort' === status) {
            return;
          }
        ...
        renderoEmbed: function renderoEmbed(response) {
    ...
    function (_30) {
      ...
      /** @lends wp.media.view.EmbedUrl.prototype */
      {
        ...
        initialize: function initialize() {
          this.$input = $('<input id="embed-url-field" type="url" />').attr('aria-label', l10n.insertFromUrlTitle).val(this.model.get('url'));
    ...
    function (_31) {
      ...
      /** @lends wp.media.view.FocusManager.prototype */
      {
        ...
        * @return {void}
        */
        focusManagementMode: function focusManagementMode(event) {
         ...
         * @return {void}
         */
        constrainTabbing: function constrainTabbing(event) {
          var tabbables; // Look for the tab key.

          if (9 !== event.keyCode) {
            return;
          }

          tabbables = this.getTabbables(); // Keep tab focus within media modal while it's open.
         ...
         * @return {void}
         */
        setAriaHiddenOnBodyChildren: function setAriaHiddenOnBodyChildren(visibleElement) {
          var bodyChildren,
              self = this;
          ...
          bodyChildren = document.body.children; // Loop through the body children and hide the ones that should be hidden.
          ...
          _.each(bodyChildren, function (element) {
            ...
            if (self.elementShouldBeHidden(element)) {
              element.setAttribute('aria-hidden', 'true'); // Store the hidden elements.
         ...
         */
        elementShouldBeHidden: function elementShouldBeHidden(element) {
          ...
          return !(element.tagName === 'SCRIPT' || element.hasAttribute('aria-hidden') || element.hasAttribute('aria-live') || liveRegionsRoles.indexOf(role) !== -1);
        },
        ...
        setupAriaTabs: function setupAriaTabs() {
          this.tabs = this.$('[role="tab"]'); // Set up initial attributes.
         ...
         * @return {void}
         */
        tabsNavigation: function tabsNavigation(event) {
          var orientation = 'horizontal',
              keys = [32, 35, 36, 37, 38, 39, 40]; // Return if not Spacebar, End, Home, or Arrow keys.
        ...
        activateTab: function activateTab(tab) {
          ...
          tab.focus(); // Handle automatic activation.
    ...
    function (_32) {
      ...
      /** @lends wp.media.view.Frame.prototype */
      {
        ...
        _createRegions: function _createRegions() {
          ...
          this.regions = this.regions ? this.regions.slice() : []; // Initialize regions.
         ...
         */
        triggerModeEvents: function triggerModeEvents(model, collection, options) {
          var collectionEvent,
              modeEventMap = {
            add: 'activate',
            remove: 'deactivate'
          },
              eventToTrigger; // Probably a better way to do this.
         ...
         * @return {this} Returns itself to allow chaining.
         */
        deactivateMode: function deactivateMode(mode) {
          // Bail if the mode isn't active.
          if (!this.isModeActive(mode)) {
            return this;
          }

          this.activeModes.remove(this.activeModes.where({
            id: mode
          }));
         ...
         * @return bool
         */
        isModeActive: function isModeActive(mode) {
    ...
    function _(module) {
      ...
      /** @lends wp.media.view.MediaFrame.ImageDetails.prototype */
      {
        ...
        bindHandlers: function bindHandlers() {
          ...
          this.on('toolbar:render:image-details', this.renderImageDetailsToolbar, this); // Override the select toolbar.
        ...
        imageDetailsContent: function imageDetailsContent(options) {
            ...
            items: {
              select: {
                ...
                click: function click() {
                  ...
                  controller.close(); // Not sure if we want to use wp.media.string.image which will create a shortcode or
                  // perhaps wp.html.string to at least to build the <img />.

                  state.trigger('update', controller.image.toJSON()); // Restore and reset the default state.
            ...
            items: {
              ...
              replace: {
                ...
                click: function click() {
                  ...
                  controller.close();
                  controller.image.changeAttachment(attachment, state.display(attachment)); // Not sure if we want to use wp.media.string.image which will create a shortcode or
                  // perhaps wp.html.string to at least to build the <img />.

                  state.trigger('replace', controller.image.toJSON()); // Restore and reset the default state.
      ...
      /** @lends wp.media.view.MediaFrame.Post.prototype */
      {
        ...
        createStates: function createStates() {
          ...
          new wp.media.controller.GalleryEdit({
            ...
            menu: 'gallery'
          }), new wp.media.controller.GalleryAdd(), new Library({
            id: 'playlist',
        ...
        bindHandlers: function bindHandlers() {
          ...
          Select.prototype.bindHandlers.apply(this, arguments);
          this.on('activate', this.activate, this); // Only bother checking media type counts if one of the counts is zero.
        ...
        mediaTypeCounts: function mediaTypeCounts(model, attr) {
        ...
        menuItemVisibility: function menuItemVisibility(state, visibility) {
         ...
         * @param {wp.Backbone.View} view
         */
        galleryMenu: function galleryMenu(view) {
        ...
        playlistMenu: function playlistMenu(view) {
          var lastState = this.lastState(),
              previous = lastState && lastState.id,
              frame = this;
          view.set({
        ...
        videoPlaylistMenu: function videoPlaylistMenu(view) {
          var lastState = this.lastState(),
              previous = lastState && lastState.id,
              frame = this;
          view.set({
        ...
        editSelectionContent: function editSelectionContent() {
          ...
          view.toolbar.set('backToLibrary', {
            ...
            priority: -100,
            click: function click() {
              this.controller.content.mode('browse'); // Move focus to the modal when jumping back from Edit Selection to Add Media view.

              this.controller.modal.focusManager.focus();
            }
          }); // Browse our library of attachments.

          this.content.set(view); // Trigger the controller to set focus.
         ...
         * @param {wp.Backbone.View} view
         */
        mainGalleryToolbar: function mainGalleryToolbar(view) {
          var controller = this;
          this.selectionStatusToolbar(view);
          view.set('gallery', {
            style: 'primary',
            ...
            click: function click() {
              ...
              this.controller.setState('gallery-edit'); // Move focus to the modal after jumping to Edit Gallery view.

              this.controller.modal.focusManager.focus();
            }
          });
        },
        mainPlaylistToolbar: function mainPlaylistToolbar(view) {
          var controller = this;
          this.selectionStatusToolbar(view);
          view.set('playlist', {
            style: 'primary',
            ...
            click: function click() {
              ...
              this.controller.setState('playlist-edit'); // Move focus to the modal after jumping to Edit Audio Playlist view.

              this.controller.modal.focusManager.focus();
            }
          });
        },
        mainVideoPlaylistToolbar: function mainVideoPlaylistToolbar(view) {
          var controller = this;
          this.selectionStatusToolbar(view);
          view.set('video-playlist', {
            ...
            click: function click() {
              ...
              this.controller.setState('video-playlist-edit'); // Move focus to the modal after jumping to Edit Video Playlist view.
            ...
            items: {
              insert: {
                ...
                click: function click() {
                  ...
                  state.trigger('reset');
                  controller.setState('gallery-edit'); // Move focus to the modal when jumping back from Add to Gallery to Edit Gallery view.
            ...
            items: {
              insert: {
                ...
                click: function click() {
                  ...
                  edit.get('library').add(state.get('selection').models);
                  state.trigger('reset');
                  controller.setState('playlist-edit'); // Move focus to the modal when jumping back from Add to Audio Playlist to Edit Audio Playlist view.
            ...
            items: {
              insert: {
                ...
                click: function click() {
                  ...
                  library.type = 'video';
                  controller.close();
                  state.trigger('update', library); // Restore and reset the default state.
            ...
            items: {
              insert: {
                ...
                click: function click() {
                  ...
                  edit.get('library').add(state.get('selection').models);
                  state.trigger('reset');
                  controller.setState('video-playlist-edit'); // Move focus to the modal when jumping back from Add to Video Playlist to Edit Video Playlist view.
    ...
    function (_34) {
      ...
      /** @lends wp.media.view.MediaFrame.Select.prototype */
      {
         ...
         */
        browseContent: function browseContent(contentRegion) {
          var state = this.state();
          this.$el.removeClass('hide-toolbar'); // Browse our library of attachments.
      ...
      /** @lends wp.media.view.ImageDetails.prototype */
      {
        ...
        render: function render() {
          ...
          if (this.model.attachment && 'pending' === this.model.dfd.state()) {
            this.model.dfd.done(_.bind(function () {
        ...
        onCustomSize: function onCustomSize(event) {
          var dimension = $(event.target).data('setting'),
              num = $(event.target).val(),
              value; // Ignore bogus input.
        ...
        onToggleAdvanced: function onToggleAdvanced(event) {
          event.preventDefault();
          this.toggleAdvanced();
        },
        toggleAdvanced: function toggleAdvanced(show) {
        ...
        replaceAttachment: function replaceAttachment(event) {
          event.preventDefault();
    ...
    function (_36) {
      ...
      /** @lends wp.media.view.MediaFrame.prototype */
      {
        ...
        initialize: function initialize() {
          ...
          this.$el.addClass('wp-core-ui'); // Initialize modal container view.
          ...
          this.on('attach', _.bind(this.views.ready, this.views), this); // Bind default title creation.

          this.on('title:create:default', this.createTitle, this);
          this.title.mode('default'); // Bind default menu.

          this.on('menu:create:default', this.createMenu, this); // Set the menu ARIA tab panel attributes when the modal opens.

          this.on('open', this.setMenuTabPanelAriaAttributes, this); // Set the router ARIA tab panel attributes when the modal opens.

          this.on('open', this.setRouterTabPanelAriaAttributes, this); // Update the menu ARIA tab panel attributes when the content updates.

          this.on('content:render', this.setMenuTabPanelAriaAttributes, this); // Update the router ARIA tab panel attributes when the content updates.
        ...
        setMenuTabPanelAriaAttributes: function setMenuTabPanelAriaAttributes() {
          ...
          if (this.state().get('menu') && this.menuView && this.menuView.isVisible) {
            ariaLabelledby = 'menu-item-' + stateId; // Set the tab panel attributes only if the tabs are visible.
        ...
        setRouterTabPanelAriaAttributes: function setRouterTabPanelAriaAttributes() {
          var tabPanelEl = this.$el.find('.media-frame-content'),
              ariaLabelledby;
          tabPanelEl.removeAttr('role aria-labelledby tabindex'); // Set the tab panel attributes only if the tabs are visible.
         ...
         * @param {Object} options
         */
        createIframeStates: function createIframeStates(options) {
        ...
        iframeMenu: function iframeMenu(view) {
          var views = {};
    ...
    function _(module) {
      ...
      /** @lends wp.media.view.MenuItem.prototype */
      {
        ...
        click: function click() {
          ...
          if (state) {
            this.controller.setState(state); // Toggle the menu visibility in the responsive view.
    ...
    function (_37) {
      ...
      /** @lends wp.media.view.Menu.prototype */
      {
         ...
         * @return {wp.media.View}
         */
        toView: function toView(options, id) {
          options = options || {};
        ...
        ready: function ready() {
          ...
          this.visibility(); // Set up aria tabs initial attributes.
        ...
        visibility: function visibility() {
          ...
          if (this === view) {
            ...
            this.isVisible = !hide; // Set or remove a CSS class to hide the menu.
        ...
        select: function select(id) {
          ...
          this.deselect();
          view.$el.addClass('active'); // Set up again the aria tabs initial attributes after the menu updates.
    ...
    function (_38) {
      ...
      /** @lends wp.media.view.Modal.prototype */
      {
        ...
        attach: function attach() {
          ...
          this.$el.appendTo(this.options.container); // Manually mark the view as attached and trigger ready.
        ...
        open: function open() {
          ...
          $el.show(); // Try to close the onscreen keyboard.
          ...
          this.$('.media-modal').trigger('focus'); // Hide the page content from assistive technologies.
        ...
        close: function close(options) {
          ...
          $('.mejs-pause button').trigger('click'); // Enable page scrolling.

          $('body').removeClass('modal-open'); // Hide the modal element by adding display:none.
          ...
          this.focusManager.removeAriaHiddenFromBodyChildren(); // Move focus back in useful location once modal is closed.
        ...
        content: function content(_content) {
          this.views.set('.media-modal-content', _content);
          return this;
        },
         ...
         * @return {wp.media.view.Modal} Returns itself to allow chaining.
         */
        propagate: function propagate(id) {
          this.trigger(id);
    ...
    function (_39) {
      ...
      /** @lends wp.media.view.PriorityList.prototype */
      {
        ...
        set: function set(id, view, options) {
          ...
          options = options || {}; // Accept an object with an `id` : `view` mapping.
    ...
    function _(module) {
      ...
      /** @lends wp.media.view.Router.prototype */
      {
        ...
        initialize: function initialize() {
          this.controller.on('content:render', this.update, this); // Call 'initialize' directly on the parent class.
    ...
    function (_40) {
      ...
      /** @lends wp.media.view.Search.prototype */
      {
        ...
        search: _.debounce(function (event) {
          var searchTerm = event.target.value.trim(); // Trigger the search only after 2 ASCII characters.
    ...
    function (_41) {
      ...
      /** @lends wp.media.view.Selection.prototype */
      {
        ...
        refresh: function refresh() {
          ...
          var collection = this.collection,
              editing = 'edit-selection' === this.controller.content.mode(); // If nothing is selected, display nothing.
        ...
        clear: function clear(event) {
          event.preventDefault();
          this.collection.reset(); // Move focus to the modal.
    ...
    function (_42) {
      ...
      /** @lends wp.media.view.Settings.prototype */
      {
         ...
         */
        render: function render() {
          View.prototype.render.apply(this, arguments); // Select the correct values.
        ...
        update: function update(key) {
          var value = this.model.get(key),
              ...
              $buttons,
              $value; // Bail if we didn't find a matching setting.
          ...
          if ($setting.is('select')) {
            ...
            $buttons = $setting.find('button').removeClass('active').attr('aria-pressed', 'false');
            $buttons.filter('[value="' + value + '"]').addClass('active').attr('aria-pressed', 'true'); // Handle text inputs and textareas.
        ...
        updateHandler: function updateHandler(event) {
          ...
          this.model.set($setting.data('setting'), value); // If the setting has a corresponding user setting,
          // update that as well.
        ...
        updateChanges: function updateChanges(model) {
          if (model.hasChanged()) {
    ...
    function _(module) {
      ...
      /** @lends wp.media.view.SiteIconPreview.prototype */
      {
        ...
        updatePreview: function updatePreview(img, coords) {
    ...
    function (_46) {
      ...
      /** @lends wp.media.view.Toolbar.prototype */
      {
        ...
        initialize: function initialize() {
          ...
          this._views = {}; // The toolbar is composed of two `PriorityList` views.
         ...
         * @return {wp.media.view.Toolbar} Returns itself to allow chaining.
         */
        unset: function unset(id, options) {
          delete this._views[id];
        ...
        refresh: function refresh() {
          ...
          _.each(this._views, function (button) {
            ...
            var requires = button.options.requires,
                disabled = false; // Prevent insertion of attachments if any of them are still uploading.
    ...
    function (_49) {
      ...
      /** @lends wp.media.view.EditorUploader.prototype */
      {
         ...
         */
        initialize: function initialize() {
          this.initialized = false; // Bail if not enabled or UA does not support drag'n'drop or File API.
        ...
        isDraggingFile: function isDraggingFile(event) {
          ...
          this.draggingFile = _.indexOf(event.originalEvent.dataTransfer.types, 'Files') > -1 && _.indexOf(event.originalEvent.dataTransfer.types, 'text/plain') === -1;
          return this.draggingFile;
        },
        ...
        attach: function attach(index, editor) {
        ...
        containerDragover: function containerDragover(event) {
        ...
        containerDragleave: function containerDragleave() {
          this.overContainer = false; // Throttle dragleave because it's called when bouncing from some elements to others.

          _.delay(_.bind(this.refresh, this), 50);
        },
        dropzoneDragover: function dropzoneDragover(event) {
          if (this.localDrag || !this.isDraggingFile(event)) {
            return;
          }
        ...
        dropzoneDragleave: function dropzoneDragleave(e) {
    ...
    function (_50) {
      ...
      /** @lends wp.media.view.UploaderInline.prototype */
      {
        ...
        ready: function ready() {
          ...
          if (this.controller.uploader) {
            $placeholder = this.$('.browser'); // Check if we've already replaced the placeholder.
      ...
      /** @lends wp.media.view.UploaderStatus.prototype */
      {
        ...
        error: function error(_error) {
          var statusError = new wp.media.view.UploaderStatusError({
            filename: this.filename(_error.get('file').name),
            message: _error.get('message')
          });
          var buttonClose = this.$el.find('button'); // Can show additional info here while retrying to create image sub-sizes.
          ...
          _.delay(function () {
            ...
            wp.a11y.speak(_error.get('message'), 'assertive');
          }, 1000);
        },
        dismiss: function dismiss() {
          ...
          wp.Uploader.errors.reset(); // Move focus to the modal after the dismiss button gets removed from the DOM.
      ...
      /** @lends wp.media.view.UploaderWindow.prototype */
      {
        ...
        ready: function ready() {
          var postId = wp.media.view.settings.post.id,
              dropzone; // If the uploader already exists, bail.
        ...
        show: function show() {
          var $el = this.$el.show(); // Ensure that the animation is triggered by waiting until
  ...
  !function () {
    ...
    var media = wp.media,
        $ = jQuery,
        l10n;
    media.isTouchDevice = 'ontouchend' in document; // Link any localized strings.

    l10n = media.view.l10n = window._wpMediaViewsL10n || {}; // Link any settings.

    media.view.settings = l10n.settings || {};
    delete l10n.settings; // Copy the `post` setting over to the model settings.

    media.model.settings.post = media.view.settings.post; // Check if the browser supports CSS 3.0 transitions.
    ...
    media.transition = function (selector, sensitivity) {
      ...
      if ($.support.transition) {
        ...
        selector.first().one($.support.transition.end, deferred.resolve); // Just in case the event doesn't trigger, fire a callback.

        _.delay(deferred.resolve, sensitivity); // Otherwise, execute on the spot.
