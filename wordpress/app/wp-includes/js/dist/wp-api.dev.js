...
(function (window, undefined) {
  ...
  wp.api.versionString = wp.api.versionString || 'wp/v2/'; // Alias _includes to _.contains, ensuring it is available if lodash is used.
...
(function (window, undefined) {
  ...
  wp.api.utils.extractRoutePart = function (route, part, versionString, reverse) {
    var routeParts;
    part = part || 1;
    versionString = versionString || wp.api.versionString; // Remove versions string from route to avoid returning it.
  ...
  wp.api.utils.addMixinsAndHelpers = function (model, modelClassName, loadingObjects) {
    ...
    TimeStampedMixin = {
      ...
      setDate: function setDate(date, field) {
        var theField = field || 'date'; // Don't alter non-parsable date fields.
      ...
      getDate: function getDate(field) {
        var theField = field || 'date',
            theISODate = this.get(theField); // Only get date fields and non-null values.
    ...
    buildModelGetter = function buildModelGetter(parentModel, modelId, modelName, embedSourcePoint, embedCheckField) {
      ...
      embeddedObjects = parentModel.get('_embedded') || {}; // Verify that we have a valid object id.
      ...
      if (!getModel.get(embedCheckField)) {
        getModel.fetch({
          success: function success(getModel) {
            deferred.resolve(getModel);
          },
          error: function error(getModel, response) {
     ...
     */
    buildCollectionGetter = function buildCollectionGetter(parentModel, collectionName, embedSourcePoint, embedIndex) {
      /**
      ...
      embeddedObjects = parentModel.get('_embedded') || {}; // Verify that we have a valid post ID.
      ...
      getObjects = new wp.api.collections[collectionName](properties, classProperties); // If we didn’t have embedded getObjects, fetch the getObjects data.

      if (_.isUndefined(getObjects.models[0])) {
        getObjects.fetch({
          success: function success(getObjects) {
     ...
     */
    setHelperParentPost = function setHelperParentPost(collection, postId) {
    ...
    MetaMixin = {
       ...
       */
      setMetas: function setMetas(meta) {
        var metas = this.get('meta');
    ...
    TagsMixin = {
      ...
      getTags: function getTags() {
        var tagIds = this.get('tags'),
            tags = new wp.api.collections.Tags(); // Resolve with an empty array if no tags.
      ...
      setTags: function setTags(tags) {
        ...
        if (_.isArray(tags)) {
          ...
          allTags.fetch({
            ...
            success: function success(alltags) {
              // Find the passed tags and set them up.
              _.each(tags, function (tag) {
                newTag = new wp.api.models.Tag(alltags.findWhere({
                  slug: tag
                })); // Tie the new tag to the post.

                newTag.set('parent_post', self.get('id')); // Add the new tag to the collection.
       ...
       *
       */
      setTagsWithCollection: function setTagsWithCollection(tags) {
    ...
    CategoriesMixin = {
      ...
      getCategories: function getCategories() {
        var categoryIds = this.get('categories'),
            categories = new wp.api.collections.Categories(); // Resolve with an empty array if no categories.
      ...
      setCategories: function setCategories(categories) {
        ...
        if (_.isArray(categories)) {
          ...
          allCategories.fetch({
            ...
            success: function success(allcats) {
              // Find the passed categories and set them up.
              _.each(categories, function (category) {
                newCategory = new wp.api.models.Category(allcats.findWhere({
                  slug: category
                })); // Tie the new category to the post.

                newCategory.set('parent_post', self.get('id')); // Add the new category to the collection.
       ...
       *
       */
      setCategoriesWithCollection: function setCategoriesWithCollection(categories) {
        // Pluck out the category IDs.
...
(function () {
  ...
  /** @lends WPApiBaseModel.prototype  */
  {
    ...
    sync: function sync(method, model, options) {
      var beforeSend;
      options = options || {}; // Remove date_gmt if null.
      ...
      if (_.isFunction(model.nonce) && !_.isEmpty(model.nonce())) {
        beforeSend = options.beforeSend; // @todo Enable option for jsonp endpoints.
...
(function () {
  ...
  /** @lends BaseCollection.prototype  */
  {
    ...
    sync: function sync(method, model, options) {
      ...
      if (_.isFunction(model.nonce) && !_.isEmpty(model.nonce())) {
        beforeSend = options.beforeSend; // Include the nonce with requests.
    ...
    hasMore: function hasMore() {
      if (null === this.state.totalPages || null === this.state.totalObjects || null === this.state.currentPage) {
        return null;
      } else {
...
(function () {
  ...
  wp.api = wp.api || {}; // If wpApiSettings is unavailable, try the default.
  ...
  /** @lends Endpoint.prototype */
  {
    ...
    initialize: function initialize() {
      ...
      if (model.get('schema')) {
      ...
      } else if (!_.isUndefined(sessionStorage) && (_.isUndefined(wpApiSettings.cacheSchema) || wpApiSettings.cacheSchema) && sessionStorage.getItem('wp-api-schema-model' + model.get('apiRoot') + model.get('versionString'))) {
        ...
        model.schemaModel.fetch({
           ...
           * @ignore
           */
          success: function success(newSchemaModel) {
            ...
            if (!_.isUndefined(sessionStorage) && (_.isUndefined(wpApiSettings.cacheSchema) || wpApiSettings.cacheSchema)) {
              ...
              } catch (error) {// Fail silently, fixes errors in safari private mode.
              }
            }
          },
    ...
    constructFromSchema: function constructFromSchema() {
      ...
      _.each(routeModel.schemaModel.get('routes'), function (route, index) {
        ...
        if (index !== routeModel.get(' versionString') && index !== schemaRoot && index !== '/' + routeModel.get('versionString').slice(0, -1)) {
      ...
      _.each(modelRoutes, function (modelRoute) {
        ...
        var modelClassName,
            ...
            routeEnd = wp.api.utils.extractRoutePart(modelRoute.index, 1, routeModel.get('versionString'), true); // Clear the parent part of the rouite if its actually the version string.
        ...
        if ('' !== parentName && parentName !== routeName) {
          ...
          loadingObjects.models[modelClassName] = wp.api.WPApiBaseModel.extend({
            ...
            url: function url() {
              var url = routeModel.get('apiRoot') + routeModel.get('versionString') + parentName + '/' + (_.isUndefined(this.get('parent')) || 0 === this.get('parent') ? _.isUndefined(this.get('parent_post')) ? '' : this.get('parent_post') + '/' : this.get('parent') + '/') + routeName;
          ...
          loadingObjects.models[modelClassName] = wp.api.WPApiBaseModel.extend({
            ...
            url: function url() {
              var url = routeModel.get('apiRoot') + routeModel.get('versionString') + ('me' === routeName ? 'users/me' : routeName);
        ...
        wp.api.utils.decorateFromRoute(modelRoute.route.endpoints, loadingObjects.models[modelClassName], routeModel.get('versionString'));
      });
      /**
      ...
      _.each(collectionRoutes, function (collectionRoute) {
        ...
        var collectionClassName,
            ...
            parentName = wp.api.utils.extractRoutePart(collectionRoute.index, 1, routeModel.get('versionString'), false); // If the collection has a parent in its route, add that to its class name.
        ...
        if ('' !== parentName && parentName !== routeName && routeModel.get('versionString') !== parentName) {
          ...
          loadingObjects.collections[collectionClassName] = wp.api.WPApiBaseCollection.extend({
            ...
            url: function url() {
              return routeModel.get('apiRoot') + routeModel.get('versionString') + parentName + '/' + (_.isUndefined(this.parent) || '' === this.parent ? _.isUndefined(this.get('parent_post')) ? '' : this.get('parent_post') + '/' : this.parent + '/') + routeName;
            },
  ...
  wp.api.init = function (args) {
    ...
    if (!initializedDeferreds[attributes.apiRoot + attributes.versionString]) {
      ...
      endpoint.schemaConstructed.done(function (resolvedEndpoint) {
        wp.api.endpoints.add(resolvedEndpoint); // Map the default endpoints, extending any already present items (including Schema model).
