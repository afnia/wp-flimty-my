...
(function (exports, $) {
  ...
  _Uploader = function Uploader(options) {
    var self = this,
        isIE,
        // Not used, back-compat.
    elements = {
      container: 'container',
    ...
    this.supports = {
      upload: _Uploader.browser.supported
    };
    ...
    $.extend(true, this, options); // Proxy all methods so this always refers to the current instance.
    ...
    for (key in elements) {
      ...
      if (!this[key].prop('id')) {
        this[key].prop('id', '__wp-uploader-id-' + _Uploader.uuid++);
      }
    ...
    delete this.plupload; // Set default params and remove this.params alias.
    ...
    _tryAgain = function tryAgain(message, data, file) {
      var times, id;
    ...
    error = function error(message, data, file, retry) {
      var isImage = file.type && file.type.indexOf('image/') === 0,
          status = data && data.status; // If the file is an image and the error is HTTP 5xx try to create sub-sizes again.
    ...
    fileUploaded = function fileUploaded(up, file, response) {
      var complete; // Remove the "uploading" UI elements.
      ...
      complete = _Uploader.queue.all(function (attachment) {
        return !attachment.get('uploading');
      });

      if (complete) {
        _Uploader.queue.reset();
      }
    ...
    this.uploader.bind('init', function (uploader) {
      ...
      dragdrop = self.supports.dragdrop = uploader.features.dragdrop && !_Uploader.browser.mobile; // Generate drag/drop helper classes.
    ...
    this.uploader.bind('FilesAdded', function (up, files) {
      _.each(files, function (file) {
        var attributes, image; // Ignore failed uploads.
        ...
        }, _.pick(file, 'loaded', 'size', 'percent')); // Handle early mime type scanning for images.

        image = /(?:jpe?g|png|gif)$/i.exec(file.name); // For images set the model's type and subtype attributes.

        if (image) {
          attributes.type = 'image'; // `jpeg`, `png` and `gif` are valid subtypes.
        ...
        _Uploader.queue.add(file.attachment);
    ...
    this.uploader.bind('Error', function (up, pluploadError) {
      var message = pluploadL10n.default_error,
          key; // Check for plupload errors.

      for (key in _Uploader.errorMap) {
        if (pluploadError.code === plupload[key]) {
          message = _Uploader.errorMap[key];
  ...
  $.extend(_Uploader, _wpPluploadSettings);
  _Uploader.uuid = 0; // Map Plupload error codes to user friendly error messages.
  ...
  _Uploader.errorMap = {
    ...
    'FILE_SIZE_ERROR': function FILE_SIZE_ERROR(file) {
      return pluploadL10n.file_exceeds_size_limit.replace('%s', file.name);
    },
    'HTTP_ERROR': function HTTP_ERROR(file) {
  ...
  /** @lends wp.Uploader.prototype */
  {
    ...
    refresh: function refresh() {
      ...
      if (this.browser) {
        node = this.browser[0]; // Check if the browser node is in the DOM.
  ...
  _Uploader.queue = new wp.media.model.Attachments([], {
    query: false
  }); // Create a collection to collect errors incurred while attempting upload.

  _Uploader.errors = new Backbone.Collection();
  exports.Uploader = _Uploader;
})(wp, jQuery);