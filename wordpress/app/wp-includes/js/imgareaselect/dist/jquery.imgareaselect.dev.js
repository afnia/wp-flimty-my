  ...
  $.imgAreaSelect = function (img, options) {
    ...
    function fixAspectRatio(xFirst) {
      if (aspectRatio) if (xFirst) {
        ...
        y2 = floor(max(top, min(top + imgHeight, y1 + abs(x2 - x1) / aspectRatio * (y2 > y1 || -1))));
        x2 = floor(x2);
      } else {
        y2 = max(top, min(top + imgHeight, y1 + abs(x2 - x1) / aspectRatio * (y2 > y1 || -1)));
        x2 = floor(max(left, min(left + imgWidth, x1 + abs(y2 - y1) * aspectRatio * (x2 > x1 || -1))));
        y2 = floor(y2);
      }
    }
    /**
    ...
    function setOptions(newOptions) {
      ...
      if (newOptions.handles != null) {
        ...
        if (!parseInt($handles.css('width')) >= 0) $handles.width(10).height(10);
        /*
