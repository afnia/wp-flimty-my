...
(self["__WordPressPrivateInteractivityAPI__"] = self["__WordPressPrivateInteractivityAPI__"] || []).push([[354], {
  ...
  function _(__unused_webpack_module, __unused_webpack___webpack_exports__, __webpack_require__) {
    /* harmony import */
    var _wordpress_interactivity__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(754);
    /**
        ...
        core: {
          image: {
            showLightbox: function showLightbox(_ref) {
              ...
              context.core.image.scrollTopReset = window.pageYOffset || document.documentElement.scrollTop; // In most cases, this value will be 0, but this is included
              // in case a user has created a page with horizontal scrolling.

              context.core.image.scrollLeftReset = window.pageXOffset || document.documentElement.scrollLeft; // We define and bind the scroll callback here so
              ...
              scrollCallback = handleScroll.bind(null, context); // We need to add a scroll event listener to the window
                  ...
                  switch (_context.prev = _context.next) {
                    case 0:
                      ...
                      if (context.core.image.lightboxEnabled) {
                        ...
                        setTimeout(function () {
                          window.removeEventListener('scroll', scrollCallback); // If we don't delay before changing the focus,
        ...
        core: {
          image: {
            ...
            initLightbox: function initLightbox(_ref12) {
              var context, ref, focusableElements;
              return regeneratorRuntime.async(function initLightbox$(_context2) {
                while (1) {
                  switch (_context2.prev = _context2.next) {
                    case 0:
                      context = _ref12.context, ref = _ref12.ref;

                      if (context.core.image.lightboxEnabled) {
                        focusableElements = ref.querySelectorAll(focusableSelectors);
                        context.core.image.firstFocusableElement = focusableElements[0];
                        context.core.image.lastFocusableElement = focusableElements[focusableElements.length - 1]; // Move focus to the dialog when opening it.
            ...
            setButtonStyles: function setButtonStyles(_ref13) {
              ...
              var naturalWidth = ref.naturalWidth,
                  ...
                  offsetHeight = ref.offsetHeight; // If the image isn't loaded yet, we can't
              ...
              var figure = ref.parentElement;
              var figureWidth = ref.parentElement.clientWidth; // We need special handling for the height because
              // a caption will cause the figure to be taller than
              // the image, which means we need to account for that
              // when calculating the placement of the button in the
              // top right corner of the image.

              var figureHeight = ref.parentElement.clientHeight;
              ...
              var buttonOffsetTop = figureHeight - offsetHeight;
              var buttonOffsetRight = figureWidth - offsetWidth; // In the case of an image with object-fit: contain, the
              // size of the <img> element can be larger than the image itself,
              // so we need to calculate where to place the button.

              if (context.core.image.scaleAttr === 'contain') {
                // Natural ratio of the image.
                var naturalRatio = naturalWidth / naturalHeight; // Offset ratio of the image.

                var offsetRatio = offsetWidth / offsetHeight;

                if (naturalRatio >= offsetRatio) {
                  // If it reaches the width first, keep
                  // the width and compute the height.
                  var referenceHeight = offsetWidth / naturalRatio;
                  ...
                  var referenceWidth = offsetHeight * naturalRatio;
    ...
    function setStyles(context, ref) {
      ...
      var naturalWidth = ref.naturalWidth,
          naturalHeight = ref.naturalHeight,
          originalWidth = ref.offsetWidth,
          originalHeight = ref.offsetHeight;

      var _ref$getBoundingClien = ref.getBoundingClientRect(),
          screenPosX = _ref$getBoundingClien.x,
          screenPosY = _ref$getBoundingClien.y; // Natural ratio of the image clicked to open the lightbox.


      var naturalRatio = naturalWidth / naturalHeight; // Original ratio of the image clicked to open the lightbox.

      var originalRatio = originalWidth / originalHeight; // If it has object-fit: contain, recalculate the original sizes
      // and the screen position without the blank spaces.

      if (context.core.image.scaleAttr === 'contain') {
        if (naturalRatio > originalRatio) {
          var heightWithoutSpace = originalWidth / naturalRatio; // Recalculate screen position without the top space.

          screenPosY += (originalHeight - heightWithoutSpace) / 2;
          originalHeight = heightWithoutSpace;
        } else {
          var widthWithoutSpace = originalHeight * naturalRatio; // Recalculate screen position without the left space.
      ...
      originalRatio = originalWidth / originalHeight; // Typically, we use the image's full-sized dimensions. If those
      // dimensions have not been set (i.e. an external image with only one size),
      // the image's dimensions in the lightbox are the same
      // as those of the image in the content.

      var imgMaxWidth = parseFloat(context.core.image.targetWidth !== 'none' ? context.core.image.targetWidth : naturalWidth);
      var imgMaxHeight = parseFloat(context.core.image.targetHeight !== 'none' ? context.core.image.targetHeight : naturalHeight); // Ratio of the biggest image stored in the database.

      var imgRatio = imgMaxWidth / imgMaxHeight;
      var containerMaxWidth = imgMaxWidth;
      var containerMaxHeight = imgMaxHeight;
      var containerWidth = imgMaxWidth;
      var containerHeight = imgMaxHeight; // Check if the target image has a different ratio than the original one (thumbnail).
      ...
      if (naturalRatio.toFixed(2) !== imgRatio.toFixed(2)) {
        if (naturalRatio > imgRatio) {
          ...
          var reducedHeight = imgMaxWidth / naturalRatio;
          ...
          var reducedWidth = imgMaxHeight * naturalRatio;
        ...
        imgRatio = imgMaxWidth / imgMaxHeight; // Calculate the max size of the container.
      ...
      var targetMaxWidth = Math.min(window.innerWidth - horizontalPadding, containerWidth);
      var targetMaxHeight = Math.min(window.innerHeight - verticalPadding, containerHeight);
      var targetContainerRatio = targetMaxWidth / targetMaxHeight;
      ...
      var containerScale = originalWidth / containerWidth;
      var lightboxImgWidth = imgMaxWidth * (containerWidth / containerMaxWidth);
      var lightboxImgHeight = imgMaxHeight * (containerHeight / containerMaxHeight); // Add the CSS variables needed.

      var styleTag = document.getElementById('wp-lightbox-styles');
      ...
      styleTag.innerHTML = "\n\t\t:root {\n\t\t\t--wp--lightbox-initial-top-position: ".concat(screenPosY, "px;\n\t\t\t--wp--lightbox-initial-left-position: ").concat(screenPosX, "px;\n\t\t\t--wp--lightbox-container-width: ").concat(containerWidth + 1, "px;\n\t\t\t--wp--lightbox-container-height: ").concat(containerHeight + 1, "px;\n\t\t\t--wp--lightbox-image-width: ").concat(lightboxImgWidth, "px;\n\t\t\t--wp--lightbox-image-height: ").concat(lightboxImgHeight, "px;\n\t\t\t--wp--lightbox-scale: ").concat(containerScale, ";\n\t\t}\n\t");
    }
    /*
