...
use Elementor\Api;
use Elementor\Api_Landingpresstemplates;
...
class Manager {
	...
	*/
	...
	public function get_library_data( array $args ) {
		if ( ! empty( $args['sync'] ) ) {
			...
			Api_Landingpresstemplates::get_templates_data( true );
			Api_Landingpresssections::get_templates_data( true );
		}
	...
	*/
	 ...
	 * @access public
	*/
	public function on_import_template_success( array $result ) {
		$args = reset( $result );
		if ( count( $result ) > 1 ) {
			wp_redirect( admin_url( 'edit.php?post_type=' . $args['post_type'] ) );
		}
		else {
			wp_redirect( admin_url( 'post.php?post=' . $args['template_id'] . '&action=edit' ) );
		}
	}
	...
	*/
	private function register_default_sources() {
		$sources = [
			'local',
			'remote',
			'landingpresstemplates',
			'landingpresssections',
			'landingpresspages',
		];
