...
class LP_Button_Tel extends Widget_Base {
	...
	public function get_title() {
		return __( 'LP - Button Phone', 'landingpress-wp' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => __( 'Call Me', 'landingpress-wp' ),
				'placeholder' => __( 'Call Me', 'landingpress-wp' ),
				'label_block' => true,
			]
		);
		...
		$this->add_control(
			'popover_description',
			[
				'raw' => __( 'Attention: Smart Popover for Telp/WA Button is displayed (by click) only on desktop and iPad.', 'landingpress-wp' ),
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::TEXT,
				'default' => '',
				'placeholder' => __( 'Cara Pesan Via Telp / Whatsapp', 'landingpress-wp' ),
				'condition' => [
					'popover!' => '',
				],
			]
		...
		$this->add_control(
			...
			[
				...
				'rows' => '2',
				'default' => __( 'Silahkan simpan nomor %phone% untuk menghubungi via telp / whatsapp', 'landingpress-wp' ),
				'placeholder' => __( 'Silahkan simpan nomor %phone% untuk menghubungi via telp / whatsapp', 'landingpress-wp' ),
				'condition' => [
					'popover!' => '',
				],
			]
		...
		if ( ! empty( $settings['phone'] ) ) {
			...
			if ( $settings['popover'] == 'yes' ) {
				...
				if ( $popover_content ) {
					...
					$this->add_render_attribute( 'button', 'class', 'elementor-button-popover-tel' );
					if ( $popover_title ) {
