...
class Settings extends Settings_Page {
	...
	*/
	public function go_elementor_pro() {
		echo '<div class="wrap">';
			echo '<h2>Elementor Pro</h2>';
			echo '<p>Elementor Pro adalah plugin resmi dan <strong>premium (berbayar)</strong> dari creator/developer plugin Elementor.</p> <p>Plugin ini <strong>tidak termasuk</strong> dalam pembelian LandingPress WordPress Theme v2.</p> <p>Plugin ini sifatnya adalah <strong>opsional</strong>, tidak wajib, beli dan gunakan jika memang butuh fitur yang ada di Elementor Pro saja.</p> <p>LandingPress v2 dapat bekerja dengan baik dengan <strong>Elementor Pro v1 (versi lama)</strong>.</p> <p>Jika ingin bisa menggunakan Elementor Pro v2 (versi baru) dengan LandingPress, silahkan upgrade ke LandingPress v3 yang sudah tersedia di member area LandingPress.</p> ';
			$utm_source = isset( $_GET['utm_source'] ) ? esc_attr( $_GET['utm_source'] ) : 'wp-menu';
			$utm_campaign = isset( $_GET['utm_campaign'] ) ? esc_attr( $_GET['utm_campaign'] ) : 'gopro';
			$utm_medium = isset( $_GET['utm_medium'] ) ? esc_attr( $_GET['utm_medium'] ) : 'wp-dash';
			echo '<a href="'.Utils::get_pro_link( 'https://elementor.com/pro/?utm_source='.$utm_source.'&utm_campaign='.$utm_campaign.'&utm_medium='.$utm_medium ).'" class="button button-primary" target="_blank">Beli Elementor Pro</a>';
		echo '</div>';
	}
