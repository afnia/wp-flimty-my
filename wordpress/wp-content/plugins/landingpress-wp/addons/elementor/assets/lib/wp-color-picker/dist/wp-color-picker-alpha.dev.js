...
(function ($) {
  ...
  $.widget('wp.wpColorPicker', $.wp.wpColorPicker, {
    ...
    _create: function _create() {
      ...
      self.toggler = $(_before).insertBefore(self.wrappingLabel); // Set the toggle button span element text.
      ...
      self.button = $(_button); // Set up the Clear/Default button.

      if (self.options.defaultColor) {
        self.button.addClass('wp-picker-default').val(wpColorPickerL10n.defaultString).attr('aria-label', wpColorPickerL10n.defaultAriaLabel);
      } else {
        self.button.addClass('wp-picker-clear').val(wpColorPickerL10n.clear).attr('aria-label', wpColorPickerL10n.clearAriaLabel);
      ...
      var setTogglerColor = function setTogglerColor(color) {
  ...
  $.widget('a8c.iris', $.a8c.iris, {
    _create: function _create() {
      ...
      if (typeof this.options.alpha !== 'undefined' && this.options.alpha) {
        ...
        el.width(parseInt(self.options.defaultWidth + self.options.customWidth)); // Push new controls
        ...
        self.setAlphaSliderBackground(); // Add new slider
      ...
      if (self.options.alpha) {
        ...
        if (target.hasClass('wp-picker-open')) {
          ...
          if (self._color._alpha < 1) {
            controls.strip.attr('style', controls.strip.attr('style').replace(/rgba\(([0-9]+,)(\s+)?([0-9]+,)(\s+)?([0-9]+)(,(\s+)?[0-9\.]+)\)/g, 'rgb($1$3$5)')); //el.width( parseInt( defaultWidth + customWidth ) );
          } else {//el.width( defaultWidth );
            }
        }
      }
