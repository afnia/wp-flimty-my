...
class CMB_Meta_Box {
	...
	function __construct( $meta_box ) {
		...
		add_action( 'add_meta_boxes', array( &$this, 'add' ), 50 );
	...
	public function init_fields( $post_id = 0 ) {
		...
		foreach ( $this->_meta_box['fields'] as $key => $field ) {
			...
			if ( $post_id ) {
				$single = isset( $field['repeatable'] ) && $field['repeatable'] ? false : true;
	...
	function enqueue_scripts() {
		...
		wp_enqueue_script( 'cmb-scripts', trailingslashit( CMB_URL ) . 'js/cmb.js', array( 'jquery' ), LANDINGPRESS_THEME_VERSION );

		wp_localize_script( 'cmb-scripts', 'CMBData', array(
			'strings' => array(
				'confirmDeleteField' => esc_html__( 'Are you sure you want to delete this field?', 'landingpress-wp' ),
			)
		) );
	...
	function enqueue_styles() {
		...
		if ( version_compare( get_bloginfo( 'version' ), '3.8', '>=' ) )
			wp_enqueue_style( 'cmb-styles', trailingslashit( CMB_URL ) . "css/dist/cmb$suffix.css", array(), LANDINGPRESS_THEME_VERSION );
		else
			wp_enqueue_style( 'cmb-styles', trailingslashit( CMB_URL ) . 'css/legacy.css', array(), LANDINGPRESS_THEME_VERSION );
