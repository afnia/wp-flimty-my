...
CMB.addCallbackForDeletedField('CMB_wysiwyg', function (el) {
  ...
  el.find('.cmb-wysiwyg textarea').each(function () {
    ...
    if (instance !== null && typeof instance !== 'undefined') instance.remove();
  });
});
CMB.addCallbackForClonedField('CMB_wysiwyg', function (newT) {
  newT.find('.cmb-wysiwyg').each(function (i) {
    ...
    idRegex = new RegExp('cmb-placeholder-id-' + fieldId, 'g'); // Placeholder markup for the new wysiwyg is stored as a prop on var cmb_wysiwyg_editors
    // Copy, update ids & names & insert.

    el.html(cmb_wysiwyg_editors[fieldId].replace(nameRegex, name).replace(idRegex, id)); // If no settings for this field. Clone from placeholder.

    if (typeof tinyMCEPreInit.mceInit[id] === 'undefined') {
      var newSettings = jQuery.extend({}, tinyMCEPreInit.mceInit['cmb-placeholder-id-' + fieldId]);

      for (var prop in newSettings) {
        if ('string' === typeof newSettings[prop]) newSettings[prop] = newSettings[prop].replace(idRegex, id).replace(nameRegex, name);
      }
    ...
    if (typeof tinyMCEPreInit.qtInit[id] === 'undefined') {
      ...
      for (var prop in newQTS) {
        if ('string' === typeof newQTS[prop]) newQTS[prop] = newQTS[prop].replace(idRegex, id).replace(nameRegex, name);
      }

      tinyMCEPreInit.qtInit[id] = newQTS;
    }

    var mode = el.find('.wp-editor-wrap').hasClass('tmce-active') ? 'tmce' : 'html'; // If current mode is visual, create the tinyMCE.
