...
class LP_Button_BBM extends Widget_Base {
	...
	public function get_title() {
		return __( 'LP - Button BBM', 'landingpress-wp' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => __( 'Invite Me on BBM', 'landingpress-wp' ),
				'placeholder' => __( 'Invite Me on BBM', 'landingpress-wp' ),
				'label_block' => true,
			]
		);
		...
		$this->add_control(
			'pin',
			[
				'label' => __( 'Blackberry PIN', 'landingpress-wp' ),
				'type' => Controls_Manager::TEXT,
				'default' => '',
		...
		$this->add_control(
			'popover_description',
			[
				'raw' => __( 'Attention: Smart Popover for BBM Button is displayed (by click) only on desktop', 'landingpress-wp' ),
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::TEXT,
				'default' => '',
				'placeholder' => __( 'Cara Pesan Via BBM', 'landingpress-wp' ),
				'condition' => [
					'popover!' => '',
				],
				'label_block' => true,
		...
		$this->add_control(
			...
			[
				...
				'rows' => '2',
				'default' => __( 'Silahkan invite PIN BBM %pin% untuk menghubungi via BBM', 'landingpress-wp' ),
				'placeholder' => __( 'Silahkan invite PIN BBM %pin% untuk menghubungi via BBM', 'landingpress-wp' ),
				'condition' => [
					'popover!' => '',
				],
			]
		...
		$this->add_control(
			'icon_show',
			[
				'label' => __( 'Show BBM Icon', 'landingpress-wp' ),
				'type' => Controls_Manager::SWITCHER,
	...
	protected function render() {
		...
		if ( ! empty( $settings['pin'] ) ) {
			$link = 'bbmi://' . trim( $settings['pin'] );
			...
			if ( $settings['popover'] == 'yes' ) {
				...
				if ( $popover_content ) {
					$popover_content = str_replace( '%pin%', $settings['pin'], $popover_content );
					$this->add_render_attribute( 'button', 'class', 'elementor-button-popover' );
					$this->add_render_attribute( 'button', 'class', 'elementor-button-popover-bbm' );
					if ( $popover_title ) {
				...
				<span <?php echo $this->get_render_attribute_string( 'content-wrapper' ); ?>>
					<?php if ( $settings['icon_show'] ) : ?>
						<span <?php echo $this->get_render_attribute_string( 'icon-align' ); ?>>
							<img src="<?php echo get_template_directory_uri(); ?>/assets/images/bbm.svg" alt="" height="15px" width="15px" />
						</span>
					<?php endif; ?>
