...
(function ($) {
  $('#wfssl-tabs').tabs({
    create: function create() {
      if (window.location.hash && window.location.hash != '#' && window.location.hash != '#open-pro-dialog') {
        $('#wfssl-tabs').tabs('option', 'active', $('a[href="' + location.hash + '"]').parent().index());
  ...
  $('.settings_page_wpfs-settings').on('click', '.change-tab', function (e) {
    e.preventDefault();
    $('#wfssl-tabs').tabs('option', 'active', $(this).data('tab')); // get the link anchor and scroll to it
  ...
  $('.settings_page_wpfs-settings').on('click', '.scrollto', function (e) {
    e.preventDefault(); // get the link anchor and scroll to it
  ...
  function load_test_results(force) {
    ...
    $('.run-tests').hide();
    $('#status_tasks').hide();
    $('#test-results-wrapper').html('<div class="loading-wrapper"><img class="wfssl_flicker" src="' + wpfs.loading_icon_url + '" alt="Loading. Please wait." title="Loading. Please wait."><p>Loading. Please wait.</p></div>');
    $.ajax({
      url: ajaxurl,
      data: {
      ...
      if (data.success) {
        ...
        $('#status_tasks').html('<div class="status-tasks status-tasks-selected" data-tasks="all">All Tests (' + tests_total + ')</div><div class="status-tasks" data-tasks="pass">Passed (' + tests_pass + ')</div><div class="status-tasks" data-tasks="warning">Need Attention (' + tests_warning + ')</div><div class="status-tasks" data-tasks="fail">Failed (' + tests_fail + ')</div>');
        $('#status_tasks').show();
  ...
  function load_ssl_cert_info(force) {
    ...
    }).always(function (data) {}).done(function (data) {
      if (data.success) {
