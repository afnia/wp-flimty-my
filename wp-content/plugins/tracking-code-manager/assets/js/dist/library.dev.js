...
TCMP.formatColorOption = function (option) {
  ...
  var $option = jQuery('<div></div>').html(option.text).css('background-color', color).css('color', font).addClass('lbColorSelectItem');
  return $option;
};
...
TCMP.attr = function ($self, name, v) {
  ...
  if (_typeof(result) === (typeof undefined === "undefined" ? "undefined" : _typeof(undefined)) || result === false) {
    result = v;
  }
...
TCMP.changeShowOptions = function ($self) {
  ...
  var $options = $self.children('option'); //console.log('NAME=[%s] OPTIONS=N.%s', name, $options.length);
...
TCMP.getDateCookie = function (name) {
  ...
  if (result !== null) {
    ...
    result = moment(result).toDate(); //console.log('getDateCookie MOMENT=%s', result);
  }
...
TCMP.setDateCookie = function (name, value, days) {
  if (value !== null && value !== '') {
    ...
    value = moment(value).format(); //console.log('setDateCookie MOMENT=%s', value);
  }
