...
class LP_Button_WAGroup extends Widget_Base {

	public function get_name() {
		return 'button_wagroup';
	}

	public function get_title() {
		return __( 'LP - Button WhatsappGroup', 'landingpress-wp' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => __( 'Join Our Whatsapp Group', 'landingpress-wp' ),
				'placeholder' => __( 'Join Our Whatsapp Group', 'landingpress-wp' ),
				'label_block' => true,
			]
		);
		...
		$this->add_control(
			'wagroup',
			[
				'label' => __( 'Whatsapp Group Join Link', 'landingpress-wp' ),
				'type' => Controls_Manager::TEXT,
				'default' => '',
	...
	protected function render() {
		...
		if ( ! empty( $settings['wagroup'] ) ) {
			$link = esc_url( trim( $settings['wagroup'] ) );
				...
				<span <?php echo $this->get_render_attribute_string( 'content-wrapper' ); ?>>
					<?php if ( $settings['icon_show'] ) : ?>
						<span <?php echo $this->get_render_attribute_string( 'icon-align' ); ?>>
							<img src="<?php echo get_template_directory_uri(); ?>/assets/images/whatsapp.svg" alt="" height="15px" width="15px" />
						</span>
					<?php endif; ?>
