...
var CMB = {
  ...
  init: function init() {
    ...
    jQuery(document).on('click.CMB', '.repeat-field', CMB.repeatField); // When toggling the display of the meta box container - reinitialize
  ...
  repeatField: function repeatField(e) {
    var templateField, newT, field, index, attr;
    field = jQuery(this).closest('.field');
    e.preventDefault();
    jQuery(this).blur();
    if (CMB.isMaxFields(field, 1)) return;
    ...
    newT.insertBefore(templateField); // Recalculate group ids & update the name fields..
    ...
    field.children('.field-item').not(templateField).each(function () {
      ...
      jQuery(this).find('[' + attr.join('],[') + ']').each(function () {
        for (var i = 0; i < attr.length; i++) {
          if (typeof jQuery(this).attr(attr[i]) !== 'undefined') jQuery(this).attr(attr[i], jQuery(this).attr(attr[i]).replace(search, replace));
        }
      });
      index += 1;
    });
    CMB.clonedField(newT);
    if (field.hasClass('cmb-sortable')) CMB.sortableInit(field);
  },
   ...
   * @return null
   */
  isMaxFields: function isMaxFields(field, modifier) {
    ...
    max = field.attr('data-rep-max'); // Show all the remove field buttons.

    field.find('> .field-item > .cmb-delete-field, > .field-item > .group > .cmb-delete-field').show();
    if (typeof max === 'undefined') return false; // Disable the add new field button?

    if (count >= parseInt(max, 10)) addBtn.attr('disabled', 'disabled');else addBtn.removeAttr('disabled');
    if (count > parseInt(max, 10)) return true;
  },
  addCallbackForInit: function addCallbackForInit(callback) {
  ...
  addCallbackForClonedField: function addCallbackForClonedField(fieldName, callback) {
    if (jQuery.isArray(fieldName)) for (var i = 0; i < fieldName.length; i++) {
   ...
   */
  clonedField: function clonedField(el) {
    ...
    el.add(el.find('div[data-class]')).each(function (i, el) {
      ...
      if (callbacks) for (var a = 0; a < callbacks.length; a++) {
        callbacks[a](el);
      }
    });
  },
  addCallbackForDeletedField: function addCallbackForDeletedField(fieldName, callback) {
    if (jQuery.isArray(fieldName)) for (var i = 0; i < fieldName.length; i++) {
   ...
   * Called when a field has been cloned.
   */
  deletedField: function deletedField(el) {
    // also check child elements
    el.add(el.find('div[data-class]')).each(function (i, el) {
      el = jQuery(el);
  ...
  sortableInit: function sortableInit(field) {
    ...
    field.sortable({
      ...
      beforeStop: function beforeStop(event, ui) {
        CMB.sortStart(jQuery(ui.item[0]));
      },
      deactivate: function deactivate(event, ui) {
        CMB.sortEnd(jQuery(ui.item[0]));
      }
    });
  },
  sortStart: function sortStart(el) {
    // also check child elements
    el.add(el.find('div[data-class]')).each(function (i, el) {
      el = jQuery(el);
  ...
  addCallbackForSortStart: function addCallbackForSortStart(fieldName, callback) {
    if (jQuery.isArray(fieldName)) for (var i = 0; i < fieldName.length; i++) {
  ...
  addCallbackForSortEnd: function addCallbackForSortEnd(fieldName, callback) {
    if (jQuery.isArray(fieldName)) for (var i = 0; i < fieldName.length; i++) {
