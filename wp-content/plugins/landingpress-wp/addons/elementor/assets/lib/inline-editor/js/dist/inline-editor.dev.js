...
(function (root, doc) {
  ...
  function onToolbarClick(ctx, target) {
    ...
    if (!ctx._urlInput) return; // create link

    var input = ctx._urlInput;
    if (toolbar === ctx._menu) showLinkInput(ctx);else {
  ...
  function initToolbar(ctx) {
    var icons = '',
        inputStr = createLinkInput(ctx).outerHTML;
    ...
    if (!ctx._toolbar) {
      ...
      if (toolListValues.indexOf('createlink') >= 0 || toolListValues.indexOf('insertimage') >= 0) icons += inputStr;
  ...
  function initEvents(ctx) {
    ...
    if (ctx._menu) {
      ...
      addListener(ctx, root, 'scroll', setpos); // toggle toolbar on mouse select
  ...
  function getNode(ctx, byRoot) {
    ...
    node = ctx._range.commonAncestorContainer; // Fix selection detection for Firefox
