...
(function ($) {
  ...
  **/
  ...
  var PickerHandler = function PickerHandler($picker, $input) {
  ...
  PickerHandler.prototype._relocate = function () {
    ...
    if ($input != null && $picker.data('isInline') === false) {
      ...
      $picker.parent().css('left', $input.offset().left + 'px'); // Display on most top of the z-index
  ...
  var getParentPickerObject = function getParentPickerObject(obj) {
    return $(obj).closest('.datepicker');
  };

  var getPickersInputObject = function getPickersInputObject($obj) {
  ...
  var setToNow = function setToNow($obj) {
    var $picker = getParentPickerObject($obj);
    var date = new Date();
    draw($picker, {
      "isAnim": true,
  ...
  var beforeMonth = function beforeMonth($obj) {
    var $picker = getParentPickerObject($obj);
  ...
  var nextMonth = function nextMonth($obj) {
    var $picker = getParentPickerObject($obj);
    var date = getPickedDate($picker);
  ...
  **/
  ...
  var getDateFormat = function getDateFormat(format, locale, is_date_only) {
    if (format == "default") {
      // Default format
  ...
  var parseDate = function parseDate(str, opt_date_format) {
    var re, m, date;
  ...
  var getFormattedDate = function getFormattedDate(date, date_format) {
    if (date == null) {
      date = new Date();
    }
    ...
    /* century */
    .replace(/MM/g, zpadding(m)).replace(/M/g, m).replace(/DD/g, zpadding(d)).replace(/D/g, d).replace(/hh/g, zpadding(hou)).replace(/h/g, hou).replace(/mm/g, zpadding(min)).replace(/m/g, min);
    return date_format;
  };

  var outputToInputObject = function outputToInputObject($picker) {
  ...
  var getPickedDate = function getPickedDate($obj) {
    var $picker = getParentPickerObject($obj);
    return $picker.data("pickedDate");
  };

  var zpadding = function zpadding(num) {
    num = ("0" + num).slice(-2);
    return num;
  };

  var draw_date = function draw_date($picker, option, date) {
  ...
  var translate = function translate(locale, s) {
  ...
  var draw = function draw($picker, option, year, month, day, hour, min) {
    var date = new Date();
    ...
    for (var zz = 0; i < cellNum; i++) {
      ...
      if ( // compare to 23:59:59 on the current day (if MIN is 1pm, then we still need to show this day
    ...
    if (opt.calendarMouseScroll) {
      if (window.sidebar) {
        ...
        $calendar.bind('DOMMouseScroll', function (e) {
          // Change a month with mouse wheel scroll for Fx
          var $picker = getParentPickerObject($(this)); // up,left [delta < 0] down,right [delta > 0]
        ...
        $calendar.bind('mousewheel', function (e) {
          // Change a month with mouse wheel scroll
          var $picker = getParentPickerObject($(this)); // up [delta > 0] down [delta < 0]
    ...
    return this.each(function (i) {
      ...
      $(input).keyup(function () {
        ...
        if ($input.val() != null && ($input.data('beforeVal') == null || $input.data('beforeVal') != null && $input.data('beforeVal') != $input.val())) {
          ...
          var date = parseDate($input.val(), format); //console.log("dtpicker - inputKeyup - format: " + format + ", date: " + $input.val() + " -> " + date);
      ...
      if (options.inline === true) {
        ...
        $(input).on('click, focus', function () {
          var $input = $(this);
          var $picker = $(PickerObjects[$input.data('pickerId')]); // Generate the handler of a picker

          var handler = new PickerHandler($picker, $input); // Get the display state of a picker

          var is_showed = handler.isShow();

          if (!is_showed) {
            // Show a picker
            handler.show(); // Call a event-hanlder
      ...
      $(input).bind('destroyed', function () {
        ...
        var handler = new PickerHandler($picker, $input); // Destroy a picker
  ...
  var methods = {
    show: function show() {
      ...
      if ($picker != null) {
        var handler = new PickerHandler($picker, $input); // Show a picker
    ...
    hide: function hide() {
      ...
      if ($picker != null) {
        var handler = new PickerHandler($picker, $input); // Hide a picker
    ...
    setDate: function setDate(date) {
      ...
      if ($picker != null) {
        var handler = new PickerHandler($picker, $input); // Set a date
    ...
    getDate: function getDate() {
      ...
      if ($picker != null) {
        var handler = new PickerHandler($picker, $input); // Get a date
  ...
  if (!window.console) {
    // Not available a console on this environment.
    window.console = {};
      ...
      for (var i = 0; i < PickerObjects.length; i++) {
        ...
        if (ActivePickerId != i) {
          ...
          if ($picker.data("inputObjectId") != null && $picker.data("isInline") === false && $picker.css('display') != 'none') {
            ...
            var handler = new PickerHandler($picker, $input);
            handler.hide(); // Call a event-hanlder
