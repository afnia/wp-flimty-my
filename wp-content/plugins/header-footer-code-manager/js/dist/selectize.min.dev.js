...
!function (root, factory) {
  "function" == typeof define && define.amd ? define("sifter", factory) : "object" == (typeof exports === "undefined" ? "undefined" : _typeof(exports)) ? module.exports = factory() : root.Sifter = factory();
}(void 0, function () {
  ...
  Sifter.prototype.tokenize = function (query) {
    ...
    for (var regex, letter, tokens = [], words = query.split(/ +/), i = 0, n = words.length; i < n; i++) {
      if (regex = escape_regex(words[i]), this.settings.diacritics) for (letter in DIACRITICS) {
    ...
    if (sort) for (i = 0, n = sort.length; i < n; i++) {
    ...
    } else for (i = 0, n = fields.length; i < n; i++) {
      if ("$score" === fields[i].field) {
        fields.splice(i, 1);
        break;
      }
    ...
    if ("object" == _typeof(query)) return query;
  ...
  var cmp = function cmp(a, b) {
    ...
    for (var k, object, i = 1, n = arguments.length; i < n; i++) {
      if (object = arguments[i]) for (k in object) {
    ...
    return a;
      ...
      getattr = function getattr(obj, name, nesting) {
    if (obj && name) {
    ...
    for (k in DIACRITICS) {
      if (DIACRITICS.hasOwnProperty(k)) for (foreignletters += chunk = DIACRITICS[k].substring(2, DIACRITICS[k].length - 1), i = 0, n = chunk.length; i < n; i++) {
  ...
  "function" == typeof define && define.amd ? define("microplugin", factory) : "object" == (typeof exports === "undefined" ? "undefined" : _typeof(exports)) ? module.exports = factory() : root.MicroPlugin = factory();
}(void 0, function () {
  var MicroPlugin = {
    mixin: function mixin(Interface) {
      Interface.plugins = {}, Interface.prototype.initializePlugins = function (plugins) {
        ...
        if (this.plugins = {
          ...
          requested: {},
          loaded: {}
        }, utils.isArray(plugins)) for (i = 0, n = plugins.length; i < n; i++) {
        ...
        } else if (plugins) for (key in plugins) {
  ...
  "function" == typeof define && define.amd ? define("selectize", ["jquery", "sifter", "microplugin"], factory) : "object" == (typeof module === "undefined" ? "undefined" : _typeof(module)) && "object" == _typeof(module.exports) ? module.exports = factory(require("jquery"), require("sifter"), require("microplugin")) : root.Selectize = factory(root.jQuery, root.Sifter, root.MicroPlugin);
  ...
  MicroEvent.prototype = {
    ...
    trigger: function trigger(event) {
      if (this._events = this._events || {}, event in this._events != !1) for (var i = 0; i < this._events[event].length; i++) {
  ...
  function debounce_events(self, types, fn) {
    ...
    }, fn.apply(self, []), self.trigger = trigger, event_args) {
  ...
  function measureString(str, $parent) {
    return str ? (Selectize.$testInput || (Selectize.$testInput = $("<span />").css({
      ...
      var i,
          n,
          styles = {};
      if (properties) for (i = 0, n = properties.length; i < n; i++) {
  ...
  var message,
      ...
      hook = {
    before: function before(self, method, fn) {
    ...
    after: function after(self, method, fn) {
      var original = self[method];
      ...
      Selectize = function Selectize($input, settings) {
    var i,
        n,
        input = $input[0];
  ...
  return MicroEvent.mixin(Selectize), void 0 !== MicroPlugin ? MicroPlugin.mixin(Selectize) : (message = "Dependency MicroPlugin is missing", options = (options = {
    ...
    setup: function setup() {
      ...
      }), event = "mousedown", selector = "*:not(input)", fn = function fn() {
    ...
    onPaste: function onPaste(e) {
      ...
      self.isFull() || self.isInputHidden || self.isLocked ? e.preventDefault() : self.settings.splitOn && setTimeout(function () {
        ...
        if (pastedText.match(self.settings.splitOn)) for (var splitInput = $.trim(pastedText).split(self.settings.splitOn), i = 0, n = splitInput.length; i < n; i++) {
    ...
    onBlur: function onBlur(deactivate, dest) {
      var self = this;
    ...
    setMaxItems: function setMaxItems(value) {
    ...
    setActiveItem: function setActiveItem($item, e) {
      ...
      if ("single" !== this.settings.mode) {
        ...
        if ("mousedown" === (idx = e && e.type.toLowerCase()) && this.isShiftDown && this.$activeItems.length) {
          for (swap = this.$control.children(".active:last"), begin = Array.prototype.indexOf.apply(this.$control[0].childNodes, [swap[0]]), (end = Array.prototype.indexOf.apply(this.$control[0].childNodes, [$item[0]])) < begin && (swap = begin, begin = end, end = swap), i = begin; i <= end; i++) {
    ...
    setActiveOption: function setActiveOption(scroll_bottom, scroll, animate) {
    ...
    search: function search(query) {
      ...
      })), this.currentResults = result) : result = $.extend(!0, {}, this.currentResults), settings.hideSelected) for (i = result.items.length - 1; 0 <= i; i--) {
    ...
    refreshOptions: function refreshOptions(triggerDropdown) {
      ...
      for ("number" == typeof self.settings.maxOptions && (n = Math.min(n, self.settings.maxOptions)), groups = {}, groups_order = [], i = 0; i < n; i++) {
        for (option = self.options[results.items[i].id], option_html = self.render("option", option), optgroup = option[self.settings.optgroupField] || "", j = 0, k = (optgroups = Array.isArray(optgroup) ? optgroup : [optgroup]) && optgroups.length; j < k; j++) {
          optgroup = optgroups[j], self.optgroups.hasOwnProperty(optgroup) || (optgroup = ""), groups.hasOwnProperty(optgroup) || (groups[optgroup] = document.createDocumentFragment(), groups_order.push(optgroup)), groups[optgroup].appendChild(option_html);
        }
      }
      ...
      }), html = document.createDocumentFragment(), i = 0, n = groups_order.length; i < n; i++) {
        optgroup = groups_order[i], self.optgroups.hasOwnProperty(optgroup) && groups[optgroup].childNodes.length ? ((html_children = document.createDocumentFragment()).appendChild(self.render("optgroup_header", self.optgroups[optgroup])), html_children.appendChild(groups[optgroup]), html.appendChild(self.render("optgroup", $.extend({}, self.optgroups[optgroup], {
          html: (d = html_children, tmp = void 0, (tmp = document.createElement("div")).appendChild(d.cloneNode(!0)), tmp.innerHTML),
          dom: html_children
        })))) : html.appendChild(groups[optgroup]);
      }

      if ($dropdown_content.html(html), self.settings.highlight && ($dropdown_content.removeHighlight(), results.query.length && results.tokens.length)) for (i = 0, n = results.tokens.length; i < n; i++) {
        !function ($element, pattern) {
          if ("string" != typeof pattern || pattern.length) {
            var regex = "string" == typeof pattern ? new RegExp(pattern, "i") : pattern,
              ...
              } else if (1 === node.nodeType && node.childNodes && !/(script|style)/i.test(node.tagName) && ("highlight" !== node.className || "SPAN" !== node.tagName)) for (var i = 0; i < node.childNodes.length; ++i) {
      ...
      if (!self.settings.hideSelected) for (self.$dropdown.find(".selected").removeClass("selected"), i = 0, n = self.items.length; i < n; i++) {
    ...
    addOption: function addOption(data) {
      var i, n, value;
      if (Array.isArray(data)) for (i = 0, n = data.length; i < n; i++) {
    ...
    updateOption: function updateOption($item, $item_new) {
    ...
    clearOptions: function clearOptions(silent) {
      var self = this;
    ...
    getAdjacentOption: function getAdjacentOption($option, index) {
    ...
    getElementWithValue: function getElementWithValue(value, $els) {
      if (null != (value = hash_key(value))) for (var i = 0, n = $els.length; i < n; i++) {
        if ($els[i].getAttribute("data-value") === value) return $($els[i]);
      }
      return $();
    },
    getElementWithTextContent: function getElementWithTextContent(textContent, ignoreCase, $els) {
    ...
    getFirstItemMatchedByTextContent: function getFirstItemMatchedByTextContent(textContent, ignoreCase) {
    ...
    addItems: function addItems(control, silent) {
    ...
    createItem: function createItem(output, triggerDropdown) {
      var self = this,
          caret = self.caretPos;
      output = output || $.trim(self.$control_input.val() || "");
      var callback = arguments[arguments.length - 1];
      if ("function" != typeof callback && (callback = function callback() {}), "boolean" != typeof triggerDropdown && (triggerDropdown = !0), !self.canCreate(output)) return callback(), !1;
      self.lock();
      var fn,
          called,
        ...
        return data[self.settings.labelField] = input, data[self.settings.valueField] = input, data;
          ...
          create = (called = !(fn = function fn(data) {
        if (self.unlock(), !data || "object" != _typeof(data)) return callback();
    ...
    insertAtCaret: function insertAtCaret(target) {
    ...
    deleteSelection: function deleteSelection(e) {
      ...
      if (this.$activeOption && !this.settings.hideSelected && (option_select = this.getAdjacentOption(this.$activeOption, -1).attr("data-value")), values = [], this.$activeItems.length) {
        for (caret = this.$control.children(".active:" + (0 < direction ? "last" : "first")), caret = this.$control.children(":not(input)").index(caret), 0 < direction && caret++, i = 0, n = this.$activeItems.length; i < n; i++) {
    ...
    setCaret: function setCaret(i) {
      if (i = "single" === this.settings.mode ? this.items.length : Math.max(0, Math.min(this.items.length, i)), !this.isPending) for (var $child, $children = this.$control.children(":not(input)"), j = 0, n = $children.length; j < n; j++) {
    ...
    function init_select($input, settings_element) {
      ...
      for (settings_element.maxItems = $input.attr("multiple") ? null : 1, i = 0, n = ($children = $input.children()).length; i < n; i++) {
        "optgroup" === (tagName = $children[i].tagName.toLowerCase()) ? function ($optgroup) {
          ...
          for ((id = ($optgroup = $($optgroup)).attr("label")) && ((optgroup = readData($optgroup) || {})[field_optgroup_label] = id, optgroup[field_optgroup_value] = id, optgroup[field_disabled] = $optgroup.prop("disabled"), settings_element.optgroups.push(optgroup)), i = 0, n = ($options = $("option", $optgroup)).length; i < n; i++) {
    ...
    return this.each(function () {
      ...
      this.selectize || ($input = $(this), tag_name = this.tagName.toLowerCase(), settings_element = {
        ...
        if (data_raw) for (settings_element.options = JSON.parse(data_raw), i = 0, n = settings_element.options.length; i < n; i++) {
